...
describe("isUnitConfigTestFile", () => {
  it("accepts unit-config src tests", () => {
    expect(isUnitConfigTestFile("src/utils/path.test.ts")).toBe(true);
  });
  ...
  it("rejects files excluded from the unit config", () => {
    ...
    expect(isUnitConfigTestFile("src/infra/assistant-exec-env.test.ts")).toBe(false);
    expect(
      isUnitConfigTestFile(bundledPluginFile("matrix", "src/migration-snapshot.test.ts")),
    ).toBe(false);
    expect(isUnitConfigTestFile("src/infra/assistant-root.test.ts")).toBe(false);

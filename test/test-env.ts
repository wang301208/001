...
function loadProfileEnv(homeDir = os.homedir()): void {
  ...
  try {
    ...
    if (applied > 0 && !isTruthyEnvValue(process.env.ZHUSHOU_LIVE_TEST_QUIET)) {
...
function resolveRestoreEntries(): RestoreEntry[] {
  return [
    { key: "ZHUSHOU_TEST_FAST", value: process.env.ZHUSHOU_TEST_FAST },
    {
      key: "ZHUSHOU_STRICT_FAST_REPLY_CONFIG",
      value: process.env.ZHUSHOU_STRICT_FAST_REPLY_CONFIG,
    },
    {
      key: "ZHUSHOU_ALLOW_SLOW_REPLY_TESTS",
      value: process.env.ZHUSHOU_ALLOW_SLOW_REPLY_TESTS,
    },
    {
      key: "ZHUSHOU_LIVE_TEST_NORMALIZE_CONFIG",
      value: process.env.ZHUSHOU_LIVE_TEST_NORMALIZE_CONFIG,
    },
    ...
    { key: "ZHUSHOU_BRIDGE_ENABLED", value: process.env.ZHUSHOU_BRIDGE_ENABLED },
    { key: "ZHUSHOU_BRIDGE_HOST", value: process.env.ZHUSHOU_BRIDGE_HOST },
    { key: "ZHUSHOU_BRIDGE_PORT", value: process.env.ZHUSHOU_BRIDGE_PORT },
    { key: "ZHUSHOU_CANVAS_HOST_PORT", value: process.env.ZHUSHOU_CANVAS_HOST_PORT },
    { key: "ZHUSHOU_TEST_HOME", value: process.env.ZHUSHOU_TEST_HOME },
    { key: "ZHUSHOU_AGENT_DIR", value: process.env.ZHUSHOU_AGENT_DIR },
...
function createIsolatedTestHome(restore: RestoreEntry[]): {
  ...
  process.env.ZHUSHOU_TEST_HOME = tempHome;
  process.env.ZHUSHOU_TEST_FAST = "1";
  process.env.ZHUSHOU_STRICT_FAST_REPLY_CONFIG = "1";
  delete process.env.ZHUSHOU_ALLOW_SLOW_REPLY_TESTS;
  ...
  delete process.env.ZHUSHOU_BRIDGE_ENABLED;
  delete process.env.ZHUSHOU_BRIDGE_HOST;
  delete process.env.ZHUSHOU_BRIDGE_PORT;
  delete process.env.ZHUSHOU_CANVAS_HOST_PORT;
...
function sanitizeLiveConfig(raw: string): string {
  try {
    ...
    if (!isTruthyEnvValue(process.env.ZHUSHOU_LIVE_TEST_NORMALIZE_CONFIG)) {
...
function stageLiveTestState(params: {
  ...
  const priorIsolatedHome = params.env.ZHUSHOU_TEST_HOME?.trim();
...
export function installTestEnv(options?: { loadProfileEnv?: boolean }): {
  ...
  const live =
    process.env.LIVE === "1" ||
    process.env.ZHUSHOU_LIVE_TEST === "1" ||
    process.env.ZHUSHOU_LIVE_GATEWAY === "1";
  const allowRealHome = isTruthyEnvValue(process.env.ZHUSHOU_LIVE_USE_REAL_HOME);

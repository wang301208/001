...
function createTempHome(): string {
  return makeTempDir(tempDirs, "assistant-test-env-real-home-");
}
...
describe("installTestEnv", () => {
  it("keeps live tests on a temp HOME while copying config and auth state", () => {
    ...
    writeFile(
      path.join(realHome, "custom-assistant.json5"),
      `{
        // Preserve provider config, strip host-bound paths.
        agents: {
          defaults: {
            workspace: "/Users/peter/Projects",
            agentDir: "/Users/peter/.assistant/agents/main/agent",
          },
          list: [
            {
              id: "dev",
              workspace: "/Users/peter/dev-workspace",
              agentDir: "/Users/peter/.assistant/agents/dev/agent",
            },
          ],
        },
        models: {
    ...
    writeFile(path.join(realHome, ".assistant", "credentials", "token.txt"), "secret\n");
    writeFile(
      path.join(realHome, ".assistant", "external-plugins", "glueclaw", "assistant.plugin.json"),
      '{"id":"glueclaw"}\n',
    );
    writeFile(
      path.join(realHome, ".assistant", "agents", "main", "agent", "auth-profiles.json"),
    ...
    process.env.ASSISTANT_LIVE_TEST_QUIET = "1";
    process.env.ASSISTANT_CONFIG_PATH = "~/custom-assistant.json5";
    process.env.ASSISTANT_TEST_HOME = priorIsolatedHome;
    process.env.ASSISTANT_STATE_DIR = path.join(priorIsolatedHome, ".assistant");
    ...
    expect(process.env.ASSISTANT_TEST_HOME).toBe(testEnv.tempHome);
    expect(process.env.TEST_PROFILE_ONLY).toBe("from-profile");

    const copiedConfigPath = path.join(testEnv.tempHome, ".assistant", "assistant.json");
    ...
    expect(
      fs.existsSync(path.join(testEnv.tempHome, ".assistant", "credentials", "token.txt")),
    ).toBe(true);
    expect(
      fs.existsSync(
        path.join(
    ...
    expect(
      fs.existsSync(
        path.join(testEnv.tempHome, ".assistant", "agents", "main", "agent", "auth-profiles.json"),
      ),
    ).toBe(true);
  ...
  it("allows explicit live runs against the real HOME", () => {
    ...
    process.env.USERPROFILE = realHome;
    process.env.ASSISTANT_LIVE_TEST = "1";
    process.env.ASSISTANT_LIVE_USE_REAL_HOME = "1";
    process.env.ASSISTANT_LIVE_TEST_QUIET = "1";
  ...
  it("does not load ~/.profile for normal isolated test runs", () => {
    ...
    delete process.env.ASSISTANT_LIVE_TEST;
    delete process.env.ASSISTANT_LIVE_GATEWAY;
    delete process.env.ASSISTANT_LIVE_USE_REAL_HOME;
    delete process.env.ASSISTANT_LIVE_TEST_QUIET;

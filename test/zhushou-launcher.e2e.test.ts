...
describe("zhushou launcher", () => {
  ...
  it("surfaces transitive entry import failures instead of masking them as missing dist", async () => {
    ...
    const result = spawnSync(process.execPath, [path.join(fixtureRoot, "zhushou.mjs"), "status"], {
      cwd: fixtureRoot,
      encoding: "utf8",
    });
  ...
  it("explains how to recover from an unbuilt source install", async () => {
    ...
    expect(result.stderr).toContain("github:wang301208/zhushou#<ref>");
  });
  ...
  it("prints source-tree quickstart help for root --help before dist exists", async () => {
    ...
    expect(result.stdout).toContain("Usage: zhushou [options] [command]");

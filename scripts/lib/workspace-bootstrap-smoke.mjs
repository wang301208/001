...
export function runInstalledWorkspaceBootstrapSmoke(params) {
  const tempRoot = mkdtempSync(join(tmpdir(), "assistant-workspace-bootstrap-smoke-"));
  ...
  try {
    try {
      execFileSync(
        process.execPath,
        [
          join(params.packageRoot, "assistant.mjs"),
          "agent",
          "--message",
    ...
    const workspaceDir = join(homeDir, ".assistant", "workspace");

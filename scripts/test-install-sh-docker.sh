...
resolve_default_smoke_platform() {
  local host_os
  local host_arch
  if [[ -n "${ASSISTANT_INSTALL_SMOKE_PLATFORM:-}" ]]; then
    printf "%s" "$ASSISTANT_INSTALL_SMOKE_PLATFORM"
    return
  fi
...
const parsed = JSON.parse(raw);
const budgetOverride = process.env.ASSISTANT_INSTALL_SMOKE_PACK_UNPACKED_BUDGET_BYTES;
const budgetBytes = budgetOverride ? Number(budgetOverride) : undefined;
if (budgetOverride && !Number.isFinite(budgetBytes)) {
  throw new Error(
    `ASSISTANT_INSTALL_SMOKE_PACK_UNPACKED_BUDGET_BYTES must be numeric, got ${JSON.stringify(
      budgetOverride,
    )}`,
  );
}
...
SMOKE_IMAGE="${ASSISTANT_INSTALL_SMOKE_IMAGE:-assistant-install-smoke:local}"
NONROOT_IMAGE="${ASSISTANT_INSTALL_NONROOT_IMAGE:-assistant-install-nonroot:local}"
SMOKE_PLATFORM="$(resolve_default_smoke_platform)"
NONROOT_PLATFORM="${ASSISTANT_INSTALL_NONROOT_PLATFORM:-$SMOKE_PLATFORM}"
INSTALL_URL="${ASSISTANT_INSTALL_URL:-https://assistant.bot/install.sh}"
CLI_INSTALL_URL="${ASSISTANT_INSTALL_CLI_URL:-https://assistant.bot/install-cli.sh}"
PACKAGE_NAME="${ASSISTANT_INSTALL_PACKAGE:-assistant}"
SKIP_NONROOT="${ASSISTANT_INSTALL_SMOKE_SKIP_NONROOT:-0}"
SKIP_SMOKE_IMAGE_BUILD="${ASSISTANT_INSTALL_SMOKE_SKIP_IMAGE_BUILD:-0}"
SKIP_NONROOT_IMAGE_BUILD="${ASSISTANT_INSTALL_NONROOT_SKIP_IMAGE_BUILD:-0}"
SKIP_UPDATE="${ASSISTANT_INSTALL_SMOKE_SKIP_UPDATE:-0}"
SKIP_NPM_GLOBAL="${ASSISTANT_INSTALL_SMOKE_SKIP_NPM_GLOBAL:-0}"
UPDATE_BASELINE_VERSION="${ASSISTANT_INSTALL_SMOKE_UPDATE_BASELINE:-2026.4.10}"
UPDATE_PACKAGE_SPEC="${ASSISTANT_INSTALL_SMOKE_UPDATE_PACKAGE_SPEC:-}"
UPDATE_DIST_IMAGE="${ASSISTANT_INSTALL_SMOKE_UPDATE_DIST_IMAGE:-}"
UPDATE_SKIP_LOCAL_BUILD="${ASSISTANT_INSTALL_SMOKE_UPDATE_SKIP_LOCAL_BUILD:-0}"
UPDATE_HOST_ALIAS="${ASSISTANT_INSTALL_SMOKE_UPDATE_HOST:-host.docker.internal}"
UPDATE_PORT="${ASSISTANT_INSTALL_SMOKE_UPDATE_PORT:-}"
...
if [[ "$SKIP_UPDATE" == "1" ]]; then
  echo "==> Skip update smoke (ASSISTANT_INSTALL_SMOKE_SKIP_UPDATE=1)"
else
  prepare_update_tarball
  prepare_update_host_access
  start_update_server
  ...
  docker run --rm -t \
    ...
    -e ASSISTANT_INSTALL_PACKAGE="$PACKAGE_NAME" \
    -e ASSISTANT_INSTALL_METHOD=npm \
    -e ASSISTANT_INSTALL_FRESH_VERSION="$UPDATE_EXPECT_VERSION" \
    -e ASSISTANT_INSTALL_FRESH_TAG_URL="$FRESH_TAG_URL" \
    -e ASSISTANT_INSTALL_LATEST_OUT="/out/latest" \
    -e ASSISTANT_NO_ONBOARD=1 \
    -e ASSISTANT_NO_PROMPT=1 \
  ...
  docker run --rm -t \
    ...
    "${UPDATE_DOCKER_HOST_ARGS[@]}" \
    -e ASSISTANT_INSTALL_PACKAGE="$PACKAGE_NAME" \
    -e ASSISTANT_INSTALL_SMOKE_MODE=update \
    -e ASSISTANT_INSTALL_UPDATE_BASELINE="$UPDATE_BASELINE_VERSION" \
    -e ASSISTANT_INSTALL_UPDATE_BASELINE_TAG_URL="$BASELINE_TAG_URL" \
    -e ASSISTANT_INSTALL_UPDATE_EXPECT_VERSION="$UPDATE_EXPECT_VERSION" \
    -e ASSISTANT_INSTALL_UPDATE_TAG_URL="$UPDATE_TAG_URL" \
    -e ASSISTANT_NO_ONBOARD=1 \
    -e ASSISTANT_NO_PROMPT=1 \
    -e DEBIAN_FRONTEND=noninteractive \
    "$SMOKE_IMAGE"
  ...
  if [[ "$SKIP_NPM_GLOBAL" == "1" ]]; then
    echo "==> Skip direct npm global smoke (ASSISTANT_INSTALL_SMOKE_SKIP_NPM_GLOBAL=1)"
  else
    ...
    docker run --rm -t \
      ...
      "${UPDATE_DOCKER_HOST_ARGS[@]}" \
      -e ASSISTANT_INSTALL_PACKAGE="$PACKAGE_NAME" \
      -e ASSISTANT_INSTALL_SMOKE_MODE=npm-global \
      -e ASSISTANT_INSTALL_UPDATE_BASELINE="$UPDATE_BASELINE_VERSION" \
      -e ASSISTANT_INSTALL_UPDATE_BASELINE_TAG_URL="$BASELINE_TAG_URL" \
      -e ASSISTANT_INSTALL_UPDATE_EXPECT_VERSION="$UPDATE_EXPECT_VERSION" \
      -e ASSISTANT_INSTALL_UPDATE_TAG_URL="$UPDATE_TAG_URL" \
...
if [[ "$SKIP_NONROOT" == "1" ]]; then
  echo "==> Skip non-root installer smoke (ASSISTANT_INSTALL_SMOKE_SKIP_NONROOT=1)"
else
  ...
  docker run --rm -t \
    ...
    -e ASSISTANT_INSTALL_PACKAGE="$PACKAGE_NAME" \
    -e ASSISTANT_INSTALL_METHOD=npm \
    -e ASSISTANT_INSTALL_EXPECT_VERSION="$LATEST_VERSION" \
    -e ASSISTANT_NO_ONBOARD=1 \
    -e ASSISTANT_NO_PROMPT=1 \
    -e DEBIAN_FRONTEND=noninteractive \
    "$NONROOT_IMAGE"
...
if [[ "${ASSISTANT_INSTALL_SMOKE_SKIP_CLI:-0}" == "1" ]]; then
  echo "==> Skip CLI installer smoke (ASSISTANT_INSTALL_SMOKE_SKIP_CLI=1)"
  exit 0
fi
...
docker run --rm -t \
  ...
  --entrypoint /bin/bash \
  -e ASSISTANT_INSTALL_URL="$INSTALL_URL" \
  -e ASSISTANT_INSTALL_CLI_URL="$CLI_INSTALL_URL" \
  -e ASSISTANT_NO_ONBOARD=1 \
  -e ASSISTANT_NO_PROMPT=1 \
  -e DEBIAN_FRONTEND=noninteractive \

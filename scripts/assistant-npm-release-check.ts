...
const EXPECTED_REPOSITORY_URL = "https://github.com/assistant/assistant";
...
const skipPackValidationEnv = "assistant_NPM_RELEASE_SKIP_PACK_CHECK";
...
export function collectReleasePackageMetadataErrors(pkg: PackageJson): string[] {
  ...
  if (pkg.name !== "assistant") {
    errors.push(`package.json name must be "assistant"; found "${pkg.name ?? ""}".`);
  }
  ...
  if (pkg.bin?.assistant !== "assistant.mjs") {
    errors.push(
      `package.json bin.assistant must be "assistant.mjs"; found "${pkg.bin?.assistant ?? ""}".`,
    );
  }
...
async function main(): Promise<number> {
  ...
  if (errors.length > 0) {
    for (const error of errors) {
      console.error(`assistant-npm-release-check: ${error}`);
    }
    return 1;
  }
  ...
  console.log(
    `assistant-npm-release-check: validated ${channel} release ${pkg.version} (${dayDistance} day UTC delta${skipPackValidation ? "; metadata-only" : ""}).`,
  );
  return 0;
}

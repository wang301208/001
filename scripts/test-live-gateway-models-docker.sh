...
elif [[ -n "${ASSISTANT_LIVE_GATEWAY_PROVIDERS:-}" ]]; then
  while IFS= read -r auth_dir; do
    [[ -n "$auth_dir" ]] || continue
    AUTH_DIRS+=("$auth_dir")
  done < <(assistant_live_collect_auth_dirs_from_csv "${ASSISTANT_LIVE_GATEWAY_PROVIDERS:-}")
  while IFS= read -r auth_file; do
    [[ -n "$auth_file" ]] || continue
    AUTH_FILES+=("$auth_file")
  done < <(assistant_live_collect_auth_files_from_csv "${ASSISTANT_LIVE_GATEWAY_PROVIDERS:-}")
else
  while IFS= read -r auth_dir; do
    [[ -n "$auth_dir" ]] || continue
    AUTH_DIRS+=("$auth_dir")
...
docker run --rm -t \
  ...
  -e ASSISTANT_LIVE_GATEWAY_MODELS="${ASSISTANT_LIVE_GATEWAY_MODELS:-modern}" \
  -e ASSISTANT_LIVE_GATEWAY_PROVIDERS="${ASSISTANT_LIVE_GATEWAY_PROVIDERS:-}" \
  -e ASSISTANT_LIVE_GATEWAY_SMOKE="${ASSISTANT_LIVE_GATEWAY_SMOKE:-1}" \
  -e ASSISTANT_LIVE_GATEWAY_MAX_MODELS="${ASSISTANT_LIVE_GATEWAY_MAX_MODELS:-8}" \
  -e ASSISTANT_LIVE_GATEWAY_STEP_TIMEOUT_MS="${ASSISTANT_LIVE_GATEWAY_STEP_TIMEOUT_MS:-45000}" \
  -e ASSISTANT_LIVE_GATEWAY_MODEL_TIMEOUT_MS="${ASSISTANT_LIVE_GATEWAY_MODEL_TIMEOUT_MS:-90000}" \

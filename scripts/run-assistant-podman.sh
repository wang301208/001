#!/usr/bin/env bash
# Rootless assistant in Podman: run after one-time setup.
#
# One-time setup (from repo root): ./scripts/podman/setup.sh
# Then:
#   ./scripts/run-assistant-podman.sh launch        # Start gateway
#   ./scripts/run-assistant-podman.sh launch setup  # Onboarding wizard
#
# Manage the running container from the host CLI:
#   assistant --container assistant tui
#   assistant --container assistant channels login
#
...
load_podman_env_file() {
  ...
  while IFS= read -r line <&9 || [[ -n "$line" ]]; do
    ...
    case "$key" in
      assistant_GATEWAY_TOKEN|assistant_PODMAN_CONTAINER|assistant_PODMAN_IMAGE|assistant_IMAGE|assistant_PODMAN_PULL|assistant_PODMAN_GATEWAY_HOST_PORT|assistant_GATEWAY_PORT|assistant_PODMAN_BRIDGE_HOST_PORT|assistant_BRIDGE_PORT|assistant_GATEWAY_BIND|assistant_PODMAN_USERNS|assistant_BIND_MOUNT_OPTIONS|assistant_PODMAN_PUBLISH_HOST)
        ;;
      *)
        continue
        ;;
...
if [[ "$(id -u)" -eq 0 ]]; then
  fail "Run run-assistant-podman.sh as your normal user so Podman stays rootless."
fi
...
CONFIG_DIR="${assistant_CONFIG_DIR:-$EFFECTIVE_HOME/.assistant}"
ENV_FILE="${assistant_PODMAN_ENV:-$CONFIG_DIR/.env}"
...
CONFIG_DIR="${assistant_CONFIG_DIR:-$EFFECTIVE_HOME/.assistant}"
ENV_FILE="${assistant_PODMAN_ENV:-$CONFIG_DIR/.env}"
WORKSPACE_DIR="${assistant_WORKSPACE_DIR:-$CONFIG_DIR/workspace}"
CONTAINER_NAME="${assistant_PODMAN_CONTAINER:-assistant}"
assistant_IMAGE="${assistant_PODMAN_IMAGE:-${assistant_IMAGE:-assistant:local}}"
PODMAN_PULL="${assistant_PODMAN_PULL:-never}"
HOST_GATEWAY_PORT="${assistant_PODMAN_GATEWAY_HOST_PORT:-${assistant_GATEWAY_PORT:-18789}}"
HOST_BRIDGE_PORT="${assistant_PODMAN_BRIDGE_HOST_PORT:-${assistant_BRIDGE_PORT:-18790}}"
PUBLISH_HOST="${assistant_PODMAN_PUBLISH_HOST:-127.0.0.1}"
...
validate_single_line_value "image name" "$assistant_IMAGE"
...
resolve_config_gateway_bind() {
  ...
  assistant_CONTAINER="" assistant_CONFIG_DIR="$config_dir" \
    assistant config get gateway.bind 2>/dev/null || true
}
...
# assistant_GATEWAY_BIND first, then gateway.bind in local config.
CONFIG_GATEWAY_BIND="$(resolve_config_gateway_bind "$CONFIG_DIR")"
GATEWAY_BIND="${assistant_GATEWAY_BIND:-${CONFIG_GATEWAY_BIND:-lan}}"
...
create_token_env_file() {
  ...
  chmod 600 "$tmp"
  printf 'assistant_GATEWAY_TOKEN=%s\n' "$token" >"$tmp"
  printf '%s' "$tmp"
}
...
sync_local_control_ui_origins_via_cli() {
  ...
  if ! command -v assistant >/dev/null 2>&1; then
    echo "Warning: assistant not found; unable to sync gateway.controlUi.allowedOrigins in $file." >&2
    return 0
  fi
  if ! command -v python3 >/dev/null 2>&1; then
    assistant_CONTAINER="" assistant_CONFIG_DIR="$config_dir" \
      assistant config set gateway.controlUi.allowedOrigins \
  ...
  allowed_json="$(
    assistant_CONTAINER="" assistant_CONFIG_DIR="$config_dir" \
      assistant config get gateway.controlUi.allowedOrigins --json 2>/dev/null || true
  )"
...
PY
  ...
  assistant_CONTAINER="" assistant_CONFIG_DIR="$config_dir" \
    assistant config set gateway.controlUi.allowedOrigins "$merged_json" --strict-json >/dev/null
}
...
if [[ -z "${assistant_GATEWAY_TOKEN:-}" ]]; then
  export assistant_GATEWAY_TOKEN="$(generate_token_hex_32)"
  mkdir -p "$(dirname "$ENV_FILE")"
  ensure_safe_existing_dir "env file directory" "$(dirname "$ENV_FILE")"
  upsert_env_var "$ENV_FILE" "assistant_GATEWAY_TOKEN" "$assistant_GATEWAY_TOKEN"
  echo "Generated assistant_GATEWAY_TOKEN and wrote it to $ENV_FILE." >&2
fi

CONFIG_JSON="$CONFIG_DIR/assistant.json"
...
PODMAN_USERNS="${assistant_PODMAN_USERNS:-keep-id}"
USERNS_ARGS=()
RUN_USER_ARGS=()
case "$PODMAN_USERNS" in
  ...
  *)
    echo "Unsupported assistant_PODMAN_USERNS=$PODMAN_USERNS (expected: keep-id, auto, host)." >&2
    exit 2
    ;;
esac
...
if [[ "$PODMAN_USERNS" == "keep-id" ]]; then
  ...
  echo "Starting container without --user (assistant_PODMAN_USERNS=$PODMAN_USERNS), mounts may require ownership fixes." >&2
fi

SELINUX_MOUNT_OPTS=""
if [[ -z "${assistant_BIND_MOUNT_OPTIONS:-}" ]]; then
  ...
  SELINUX_MOUNT_OPTS="${assistant_BIND_MOUNT_OPTIONS#:}"
  [[ -n "$SELINUX_MOUNT_OPTS" ]] && SELINUX_MOUNT_OPTS=",$SELINUX_MOUNT_OPTS"
fi

if [[ "$RUN_SETUP" == true ]]; then
  TOKEN_ENV_FILE="$(create_token_env_file "$ENV_FILE" "$assistant_GATEWAY_TOKEN")"
  podman run --pull="$PODMAN_PULL" --rm -it \
    --init \
    "${USERNS_ARGS[@]}" "${RUN_USER_ARGS[@]}" \
    -e HOME=/home/node -e TERM=xterm-256color -e BROWSER=echo \
    -e NPM_CONFIG_CACHE=/home/node/.assistant/.npm \
    -e assistant_NO_RESPAWN=1 \
    --env-file "$TOKEN_ENV_FILE" \
    -v "$CONFIG_DIR:/home/node/.assistant:rw${SELINUX_MOUNT_OPTS}" \
    -v "$WORKSPACE_DIR:/home/node/.assistant/workspace:rw${SELINUX_MOUNT_OPTS}" \
    "$assistant_IMAGE" \
...
echo "podman exec -it $CONTAINER_NAME assistant tui"
echo "podman exec -it $CONTAINER_NAME assistant devices approve --latest  # if pairing required"

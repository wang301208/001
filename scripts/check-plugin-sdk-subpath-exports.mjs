...
function parsePluginSdkSubpath(specifier) {
  ...
  const subpath = specifier.slice("assistant/plugin-sdk/".length);
  return subpath || null;
}
...
async function main() {
  ...
  if (violations.length === 0) {
    console.log("OK: all referenced assistant/plugin-sdk/<subpath> imports are exported.");
    return;
  }

  console.error(
    "Rule: every referenced assistant/plugin-sdk/<subpath> must exist in the public package exports.",
  );

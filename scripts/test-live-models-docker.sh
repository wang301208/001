...
if zhushou_live_truthy "${ZHUSHOU_DOCKER_PROFILE_ENV_ONLY:-}"; then
  CONFIG_DIR="$(mktemp -d)"
  WORKSPACE_DIR="$(mktemp -d)"
  TEMP_DIRS+=("$CONFIG_DIR" "$WORKSPACE_DIR")
  ZHUSHOU_DOCKER_AUTH_DIRS=none
else
...
elif [[ -n "${ZHUSHOU_LIVE_PROVIDERS:-}" || -n "${ZHUSHOU_LIVE_GATEWAY_PROVIDERS:-}" ]]; then
  while IFS= read -r auth_dir; do
    [[ -n "$auth_dir" ]] || continue
    AUTH_DIRS+=("$auth_dir")
  done < <(
    {
      zhushou_live_collect_auth_dirs_from_csv "${ZHUSHOU_LIVE_PROVIDERS:-}"
      zhushou_live_collect_auth_dirs_from_csv "${ZHUSHOU_LIVE_GATEWAY_PROVIDERS:-}"
    } | awk '!seen[$0]++'
  )
  while IFS= read -r auth_file; do
    [[ -n "$auth_file" ]] || continue
    ...
    {
      zhushou_live_collect_auth_files_from_csv "${ZHUSHOU_LIVE_PROVIDERS:-}"
      zhushou_live_collect_auth_files_from_csv "${ZHUSHOU_LIVE_GATEWAY_PROVIDERS:-}"
    } | awk '!seen[$0]++'
  )
else
  while IFS= read -r auth_dir; do
...
echo "==> Profile env only: ${ZHUSHOU_DOCKER_PROFILE_ENV_ONLY:-0}"
...
docker run --rm -t \
  ...
  -e ZHUSHOU_LIVE_MODELS="${ZHUSHOU_LIVE_MODELS:-modern}" \
  -e ZHUSHOU_LIVE_PROVIDERS="${ZHUSHOU_LIVE_PROVIDERS:-}" \
  -e ZHUSHOU_LIVE_MAX_MODELS="${ZHUSHOU_LIVE_MAX_MODELS:-12}" \
  -e ZHUSHOU_LIVE_MODEL_TIMEOUT_MS="${ZHUSHOU_LIVE_MODEL_TIMEOUT_MS:-}" \
  -e ZHUSHOU_LIVE_REQUIRE_PROFILE_KEYS="${ZHUSHOU_LIVE_REQUIRE_PROFILE_KEYS:-}" \
  -e ZHUSHOU_LIVE_GATEWAY_MODELS="${ZHUSHOU_LIVE_GATEWAY_MODELS:-}" \
  -e ZHUSHOU_LIVE_GATEWAY_PROVIDERS="${ZHUSHOU_LIVE_GATEWAY_PROVIDERS:-}" \
  -e ZHUSHOU_LIVE_GATEWAY_MAX_MODELS="${ZHUSHOU_LIVE_GATEWAY_MAX_MODELS:-}" \

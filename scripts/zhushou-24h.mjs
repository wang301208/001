...
function hasFlag(name) {
  return optionArgs.includes(`--${name}`) || process.env[`ZHUSHOU_24H_${name.toUpperCase().replaceAll("-", "_")}`] === "1";
}
...
function buildConfig() {
  const gatewayPort = readNumberOption(
    "gateway-port",
    Number(process.env.ZHUSHOU_GATEWAY_PORT ?? process.env.ZHUSHOU_WEB_PORT ?? 3000),
  );
  ...
  const webHost = readOption("web-host", process.env.ZHUSHOU_WEB_HOST ?? "127.0.0.1");
  ...
  return {
    ...
    startupGraceMs,
    healthFailLimit,
    gatewayBind: readOption("gateway-bind", process.env.ZHUSHOU_GATEWAY_BIND ?? "loopback"),
  ...
  const services = [
    {
      ...
      cwd: repoRoot,
      env: {
        ...(config.gatewayToken ? { ZHUSHOU_GATEWAY_TOKEN: config.gatewayToken } : {}),
...
async function startCommand() {
  console.error("Zhushou 24h automatic supervisor has been removed. Start the gateway manually with `node zhushou.mjs gateway run`.");
  process.exitCode = 1;
}
...
function installTaskCommand() {
  console.error("Zhushou 24h automatic supervisor task installation has been removed.");
  process.exitCode = 1;
}

function uninstallTaskCommand() {
  console.error("Zhushou 24h automatic supervisor task removal has been removed.");
  process.exitCode = 1;
}
...
Environment:
  ZHUSHOU_GATEWAY_TOKEN       Optional token passed to the internal gateway process
  ZHUSHOU_24H_DISABLE_WEB=1   Deprecated: frontend is served by the gateway process
`);
}

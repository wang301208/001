...
const EXPECTED_REPOSITORY_URL = "https://github.com/wang301208/zhushou";
...
const skipPackValidationEnvNames = [
  "ZHUSHOU_NPM_RELEASE_SKIP_PACK_CHECK",
  "zhushou_NPM_RELEASE_SKIP_PACK_CHECK",
] as const;
...
export function shouldSkipPackedTarballValidation(env = process.env): boolean {
  const raw = skipPackValidationEnvNames
    .map((name) => env[name])
...
export function collectReleasePackageMetadataErrors(pkg: PackageJson): string[] {
  ...
  if (pkg.name !== "zhushou") {
    errors.push(`package.json name must be "zhushou"; found "${pkg.name ?? ""}".`);
  }
  ...
  if (pkg.bin?.zhushou !== "zhushou.mjs") {
    errors.push(
      `package.json bin.zhushou must be "zhushou.mjs"; found "${pkg.bin?.zhushou ?? ""}".`,
    );
  }
...
async function main(): Promise<number> {
  ...
  if (errors.length > 0) {
    for (const error of errors) {
      console.error(`zhushou-npm-release-check: ${error}`);
    }
    return 1;
  }
  ...
  console.log(
    `zhushou-npm-release-check: validated ${channel} release ${pkg.version} (${dayDistance} day UTC delta${skipPackValidation ? "; metadata-only" : ""}).`,
  );
  return 0;
}

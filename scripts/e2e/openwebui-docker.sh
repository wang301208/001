...
IMAGE_NAME="assistant-openwebui-e2e"
...
MODEL="${ASSISTANT_OPENWEBUI_MODEL:-openai/gpt-4.1-mini}"
PROMPT_NONCE="OPENWEBUI_DOCKER_E2E_$(date +%s)_$$"
PROMPT="${ASSISTANT_OPENWEBUI_PROMPT:-Reply with exactly this token and nothing else: ${PROMPT_NONCE}}"
PORT="${ASSISTANT_OPENWEBUI_GATEWAY_PORT:-18789}"
WEBUI_PORT="${ASSISTANT_OPENWEBUI_PORT:-8080}"
TOKEN="openwebui-e2e-$(date +%s)-$$"
ADMIN_EMAIL="${ASSISTANT_OPENWEBUI_ADMIN_EMAIL:-openwebui-e2e@example.com}"
ADMIN_PASSWORD="${ASSISTANT_OPENWEBUI_ADMIN_PASSWORD:-OpenWebUI-E2E-Password-$(date +%s)-$$}"
NET_NAME="assistant-openwebui-e2e-$$"
GW_NAME="assistant-openwebui-gateway-$$"
OW_NAME="assistant-openwebui-$$"
...
docker run -d \
  ...
  -e "ASSISTANT_OPENWEBUI_MODEL=$MODEL" \
  -e OPENAI_API_KEY \
  ...
  bash -lc '
    ...
    batch_file="$(mktemp /tmp/assistant-openwebui-config.XXXXXX.json)"
    ASSISTANT_CONFIG_BATCH_PATH="$batch_file" node - <<'"'"'NODE'"'"' "$openai_api_key"
const fs = require("node:fs");

const openaiApiKey = process.argv[2];
const batchPath = process.env.ASSISTANT_CONFIG_BATCH_PATH;
const entries = [
  ...
  { path: "gateway.auth.token", value: process.env.ASSISTANT_GATEWAY_TOKEN },
  { path: "gateway.http.endpoints.chatCompletions.enabled", value: true },
  { path: "agents.defaults.model.primary", value: process.env.ASSISTANT_OPENWEBUI_MODEL },
];
...
docker run -d \
  ...
  --network "$NET_NAME" \
  -e ENV=prod \
  -e WEBUI_NAME="Assistant E2E" \
  -e WEBUI_SECRET_KEY="assistant-openwebui-e2e-secret" \
  -e OFFLINE_MODE=True \
  ...
  -e WEBUI_ADMIN_NAME="Assistant E2E" \
  -e ENABLE_SIGNUP=False \
  -e DEFAULT_MODELS="assistant/default" \
...
echo "Running Open WebUI -> Assistant smoke..."
if ! docker exec \

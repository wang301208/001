...
MAIN_TGZ_PATH=""
SERVER_PID=""
RUN_DIR="$(mktemp -d /tmp/zhushou-parallels-linux.XXXXXX)"
...
start_server() {
  ...
  while :; do
    ...
    ) >/tmp/zhushou-parallels-linux-http.log 2>&1 &
    SERVER_PID=$!
    sleep 1
...
install_latest_release() {
  ...
  guest_exec curl -fsSL "$INSTALL_URL" -o /tmp/zhushou-install.sh
  guest_exec /usr/bin/env ZHUSHOU_NO_ONBOARD=1 bash /tmp/zhushou-install.sh "${version_args[@]}" --no-onboard
  guest_exec zhushou --version
}
...
verify_version_contains() {
  local needle="$1"
  local version
  version="$(guest_exec zhushou --version)"
  printf '%s\n' "$version"
  case "$version" in
    *"$needle"*) ;;
    *)
...
run_ref_onboard() {
  guest_exec /usr/bin/env "$API_KEY_ENV=$API_KEY_VALUE" zhushou onboard \
    --non-interactive \
    --mode local \
...
set -euo pipefail
plugin_dir=/root/.zhushou/test-bad-plugin
mkdir -p "$plugin_dir"
cat >"$plugin_dir/package.json" <<'JSON'
{
  "name": "@zhushou/test-bad-plugin",
  "version": "1.0.0",
  "zhushou": {
...
JSON
cat >"$plugin_dir/zhushou.plugin.json" <<'JSON'
{
  "id": "test-bad-plugin",
  "configSchema": {
    "type": "object",
...
config_path = Path("/root/.wang301208/zhushou.json")
config = {}
...
verify_bad_plugin_diagnostic() {
  guest_bash_script <<'EOF'
grep -F "failed to load setup entry" /tmp/zhushou-parallels-linux-gateway.log
EOF
}
...
start_gateway_background() {
  ...
  cmd="$(cat <<EOF
pkill -f "zhushou gateway run" >/dev/null 2>&1 || true
rm -f /tmp/zhushou-parallels-linux-gateway.log
setsid sh -lc 'exec env ZHUSHOU_HOME=/root ZHUSHOU_STATE_DIR=/root/.zhushou ZHUSHOU_CONFIG_PATH=/root/.wang301208/zhushou.json ${API_KEY_ENV}=${api_key_value_q} zhushou gateway run --bind loopback --port 18789 --force >/tmp/zhushou-parallels-linux-gateway.log 2>&1' >/dev/null 2>&1 < /dev/null &
EOF
)"
...
show_gateway_status_compat() {
  if guest_exec zhushou gateway status --help | grep -Fq -- "--require-rpc"; then
    guest_exec zhushou gateway status --deep --require-rpc
    return
  fi
  guest_exec zhushou gateway status --deep
}

verify_local_turn() {
  guest_exec zhushou models set "$MODEL_ID"
  guest_exec /usr/bin/env "$API_KEY_ENV=$API_KEY_VALUE" zhushou agent \
    --local \
    --agent main \
    --message ping \
    --json
...
run_fresh_main_lane() {
  ...
  phase_run "fresh.install-main" "$TIMEOUT_INSTALL_S" install_main_tgz "$host_ip" "zhushou-main-fresh.tgz"
...
run_upgrade_lane() {
  ...
  phase_run "upgrade.install-main" "$TIMEOUT_INSTALL_S" install_main_tgz "$host_ip" "zhushou-main-upgrade.tgz"

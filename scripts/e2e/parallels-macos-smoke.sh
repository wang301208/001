...
AUTH_KEY_FLAG=""
MODEL_ID=""
INSTALL_URL="https://zhushou.ai/install.sh"
HOST_PORT="18425"
...
SNAPSHOT_STATE=""
SNAPSHOT_NAME=""
GUEST_ZHUSHOU_BIN="/opt/homebrew/bin/zhushou"
GUEST_ZHUSHOU_ENTRY="/opt/homebrew/lib/node_modules/wang301208/zhushou.mjs"
...
SERVER_PID=""
RUN_DIR="$(mktemp -d /tmp/zhushou-parallels-smoke.XXXXXX)"
BUILD_LOCK_DIR="${TMPDIR:-/tmp}/zhushou-parallels-build.lock"
...
Options:
  ...
  --install-url <url>        Installer URL for latest release. Default: https://zhushou.ai/install.sh
  ...
  --target-package-spec <npm-spec>
                             ...
                             Example: zhushou@2026.3.13-beta.1
...
resolve_guest_git_zhushou_entry() {
  local guest_home
  guest_home="$(resolve_guest_current_user_home)"
  printf '%s/wang301208/zhushou.mjs\n' "$guest_home"
}
...
spawn {*}$cmd
send -- "printf '__ZHUSHOU_READY__\\n'\r"
expect "__ZHUSHOU_READY__"
log_user 0
send -- "export PS1='' PROMPT='' PROMPT2='' RPROMPT=''\r"
send -- "stty -echo\r"

send -- "cat >/tmp/zhushou-prl.sh <<'__ZHUSHOU_SCRIPT__'\r"
send -- $script
if {![string match "*\n" $script]} {
  send -- "\r"
}
send -- "__ZHUSHOU_SCRIPT__\r"
send -- "/bin/bash /tmp/zhushou-prl.sh; rc=\$?; rm -f /tmp/zhushou-prl.sh; printf '__ZHUSHOU_RC__:%s\\n' \"\$rc\"; exit \"\$rc\"\r"
log_user 1

set rc 1
set saw_rc 0
expect {
  -re {__ZHUSHOU_RC__:(-?[0-9]+)} {
...
guest_current_user_sh() {
  ...
  script+="$1"
  if headless_guest_fallback; then
    script_path="/tmp/zhushou-prl-${BASHPID:-$$}-$RANDOM.sh"
    local guest_home
    guest_home="$(parallels_macos_resolve_desktop_home "$VM_NAME" "$GUEST_CURRENT_USER")"
...
markers = [
    ...
    if line.startswith("__ZHUSHOU_RC__:")
]
if not markers:
    raise SystemExit(1)
...
printf '__ZHUSHOU_RC__:%s\n' "\$status"
printf '%s\n' "\$status" > "$done_path"
exit "\$status"
EOF
)"
  write_runner_cmd="/bin/rm -f $(shell_quote "$runner_path")"$'\n'
  write_runner_cmd+="cat > $(shell_quote "$runner_path") <<'__ZHUSHOU_RUNNER__'"$'\n'
  write_runner_cmd+="$runner_body"$'\n'
  write_runner_cmd+="__ZHUSHOU_RUNNER__"$'\n'
  ...
  guest_home="$(resolve_guest_current_user_home)"
  guest_log_state_path="$(mktemp "${TMPDIR:-/tmp}/zhushou-guest-log-state.XXXXXX")"
  latest_npm_log_state_path="$(mktemp "${TMPDIR:-/tmp}/zhushou-guest-npm-log-state.XXXXXX")"
  npm_state_path="$(mktemp "${TMPDIR:-/tmp}/zhushou-guest-npm-log-path.XXXXXX")"
...
matches = re.findall(r"^__ZHUSHOU_RC__:(-?\d+)$", text, flags=re.MULTILINE)
if not matches:
    raise SystemExit(1)
print(matches[-1])
PY
...
install_latest_release() {
  ...
  guest_current_user_sh "$(cat <<EOF
export ZHUSHOU_NO_ONBOARD=1
curl -fsSL $install_url_q -o /tmp/zhushou-install.sh
bash /tmp/zhushou-install.sh${version_arg_q}
$GUEST_ZHUSHOU_BIN --version
EOF
)"
}
...
ensure_guest_pnpm_for_dev_update() {
  ...
  bootstrap_root="/tmp/zhushou-smoke-pnpm-bootstrap"
...
repair_legacy_dev_source_checkout_if_needed() {
  ...
  bootstrap_bin="/tmp/zhushou-smoke-pnpm-bootstrap/node_modules/.bin"
  update_root="$(resolve_guest_current_user_home)/zhushou"
  update_entry="$update_root/zhushou.mjs"
  ...
  guest_current_user_exec /usr/bin/git clone --depth 1 --branch main \
    https://github.com/wang301208/zhushou.git "$update_root"
...
run_dev_channel_update() {
  ...
  bootstrap_bin="/tmp/zhushou-smoke-pnpm-bootstrap/node_modules/.bin"
  update_root="$(resolve_guest_current_user_home)/zhushou"
  update_log="/tmp/zhushou-smoke-update-dev.log"
  update_done="/tmp/zhushou-smoke-update-dev.done"
  update_runner="/tmp/zhushou-smoke-update-dev.sh"
  ensure_guest_pnpm_for_dev_update
...
/usr/bin/env NODE_OPTIONS=--max-old-space-size=4096 \
  $GUEST_NODE_BIN $GUEST_ZHUSHOU_ENTRY update --channel dev --yes --json
EOF
  ...
  guest_current_user_exec "$GUEST_NODE_BIN" "$GUEST_ZHUSHOU_ENTRY" --version
  printf 'update-dev: git-status\n'
  guest_current_user_exec "$GUEST_NODE_BIN" "$GUEST_ZHUSHOU_ENTRY" update status --json
}

verify_dev_channel_update() {
  local status_json
  status_json="$(guest_current_user_exec "$GUEST_NODE_BIN" "$GUEST_ZHUSHOU_ENTRY" update status --json)"
  printf '%s\n' "$status_json"
...
verify_version_contains() {
  ...
  local version
  version="$(
    guest_current_user_exec "$GUEST_ZHUSHOU_BIN" --version 2>&1
  )"
  printf '%s\n' "$version"
  case "$version" in
    *"$needle"*) ;;
...
pack_main_tgz() {
  ...
  MAIN_TGZ_PATH="$MAIN_TGZ_DIR/zhushou-main-$short_head.tgz"
...
start_server() {
  ...
  ) >/tmp/zhushou-parallels-http.log 2>&1 &
  SERVER_PID=$!
  sleep 1
...
check_path "\$root/zhushou"
check_path "\$root/zhushou/extensions"
if [ -d "\$root/zhushou/extensions" ]; then
  while IFS= read -r -d '' extension_dir; do
    check_path "\$extension_dir"
  done < <(/usr/bin/find "\$root/zhushou/extensions" -mindepth 1 -maxdepth 1 -type d -print0)
fi
EOF
)"
...
start_manual_gateway_if_needed() {
  ...
  guest_gateway_log="/tmp/zhushou-parallels-macos-gateway.log"
  runner_log="/tmp/zhushou-parallels-gateway-start.log"
  done_path="/tmp/zhushou-parallels-gateway-start.done"
  runner_path="/tmp/zhushou-parallels-gateway-start.sh"
  printf 'manual gateway launch transport=%s user=%s\n' "$GUEST_CURRENT_USER_TRANSPORT" "$GUEST_CURRENT_USER"
  launch_cmd="$(cat <<EOF
set -euo pipefail
trap '' HUP
/usr/bin/pkill -f 'zhushou.*gateway run' >/dev/null 2>&1 || true
/usr/bin/pkill -f 'zhushou-gateway' >/dev/null 2>&1 || true
/usr/bin/pkill -f 'zhushou.mjs gateway' >/dev/null 2>&1 || true
/usr/bin/env \\
  ...
  ZHUSHOU_HOME=$(shell_quote "$guest_home") \\
  ZHUSHOU_STATE_DIR=$(shell_quote "$guest_home/.zhushou") \\
  ZHUSHOU_CONFIG_PATH=$(shell_quote "$guest_home/.wang301208/zhushou.json") \\
  $(shell_quote "$GUEST_NODE_BIN") $(shell_quote "$GUEST_ZHUSHOU_ENTRY") gateway run --bind loopback --port 18789 --force \\
...
verify_gateway() {
  ...
  for attempt in 1 2 3 4; do
    if guest_current_user_exec "$GUEST_ZHUSHOU_BIN" gateway status --deep --require-rpc --timeout 5000; then
      return 0
    fi
    if (( attempt < 4 )); then
...
show_gateway_status_compat() {
  if guest_current_user_exec "$GUEST_ZHUSHOU_BIN" gateway status --help | grep -Fq -- "--require-rpc"; then
    guest_current_user_exec "$GUEST_ZHUSHOU_BIN" gateway status --deep --require-rpc
    return
  fi
  guest_current_user_exec "$GUEST_ZHUSHOU_BIN" gateway status --deep
}

verify_turn() {
  guest_current_user_exec "$GUEST_NODE_BIN" "$GUEST_ZHUSHOU_ENTRY" models set "$MODEL_ID"
  guest_current_user_sh "$(cat <<EOF
export PATH=$(shell_quote "$GUEST_EXEC_PATH")
exec /usr/bin/env $(shell_quote "$API_KEY_ENV=$API_KEY_VALUE") \
  $(shell_quote "$GUEST_NODE_BIN") $(shell_quote "$GUEST_ZHUSHOU_ENTRY") agent \
  --agent main \
...
if [ -z "\${HOME:-}" ]; then export HOME="/Users/\$(id -un)"; fi
cd "\$HOME"
$(shell_quote "$GUEST_ZHUSHOU_BIN") tui --help >/tmp/zhushou-tui-help.txt
grep -i "tui" /tmp/zhushou-tui-help.txt >/dev/null
$(shell_quote "$GUEST_ZHUSHOU_BIN") gateway status --json >/tmp/zhushou-gateway-status.json
...
PY
  ...
  script="$(cat <<EOF
cat >/tmp/zhushou-discord-token <<'__ZHUSHOU_TOKEN__'
$DISCORD_TOKEN_VALUE
__ZHUSHOU_TOKEN__
cat >/tmp/zhushou-discord-guilds.json <<'__ZHUSHOU_GUILDS__'
$guilds_json
__ZHUSHOU_GUILDS__
token="\$(tr -d '\n' </tmp/zhushou-discord-token)"
guilds_json="\$(cat /tmp/zhushou-discord-guilds.json)"
$GUEST_NODE_BIN $GUEST_ZHUSHOU_ENTRY config set channels.discord.token "\$token"
$GUEST_NODE_BIN $GUEST_ZHUSHOU_ENTRY config set channels.discord.enabled true
$GUEST_NODE_BIN $GUEST_ZHUSHOU_ENTRY config set channels.discord.groupPolicy allowlist
$GUEST_NODE_BIN $GUEST_ZHUSHOU_ENTRY config set channels.discord.guilds "\$guilds_json" --strict-json
$GUEST_NODE_BIN $GUEST_ZHUSHOU_ENTRY gateway restart
for _ in 1 2 3 4 5 6 7 8; do
  if $GUEST_NODE_BIN $GUEST_ZHUSHOU_ENTRY gateway status --deep --require-rpc >/dev/null 2>&1; then
    break
  fi
  sleep 2
done
$GUEST_NODE_BIN $GUEST_ZHUSHOU_ENTRY channels status --probe --json
rm -f /tmp/zhushou-discord-token /tmp/zhushou-discord-guilds.json
EOF
)"
...
run_fresh_main_lane() {
  ...
  phase_run "fresh.install-main" "$(install_main_timeout)" install_main_tgz "$host_ip" "zhushou-main-fresh.tgz"
  ...
  phase_run "fresh.terminal-ui" "$TIMEOUT_DASHBOARD_S" verify_terminal_ui
...
run_upgrade_lane() {
  ...
  if upgrade_uses_host_tgz; then
    phase_run "upgrade.install-main" "$(install_main_timeout)" install_main_tgz "$host_ip" "zhushou-main-upgrade.tgz"
  ...
  phase_run "upgrade.terminal-ui" "$TIMEOUT_DASHBOARD_S" verify_terminal_ui

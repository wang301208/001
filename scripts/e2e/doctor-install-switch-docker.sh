...
IMAGE_NAME="zhushou-doctor-install-switch-e2e"
...
docker run --rm -e COREPACK_ENABLE_DOWNLOAD_PROMPT=0 "$IMAGE_NAME" bash -lc '
  ...
  cat > /tmp/zhushou-bin/systemctl <<"SYSTEMCTL"
#!/usr/bin/env bash
set -euo pipefail
...
SYSTEMCTL
  chmod +x /tmp/zhushou-bin/systemctl

  cat > /tmp/zhushou-bin/loginctl <<"LOGINCTL"
#!/usr/bin/env bash
set -euo pipefail
...
exit 0
LOGINCTL
  chmod +x /tmp/zhushou-bin/loginctl
  ...
  npm_log="/tmp/zhushou-doctor-switch-npm-install.log"
  if ! npm install -g --prefix /tmp/npm-prefix "/app/$pkg_tgz" >"$npm_log" 2>&1; then
    cat "$npm_log"
    exit 1
  fi

	  npm_bin="/tmp/npm-prefix/bin/zhushou"
	  npm_root="/tmp/npm-prefix/lib/node_modules/zhushou"
  ...
  run_flow() {
    ...
    local install_log="/tmp/zhushou-doctor-switch-${name}-install.log"
    local doctor_log="/tmp/zhushou-doctor-switch-${name}-doctor.log"

    echo "== Flow: $name =="
    home_dir=$(mktemp -d "/tmp/zhushou-switch-${name}.XXXXXX")
    ...
    unit_path="$HOME/.config/systemd/user/zhushou-gateway.service"
    if [ ! -f "$unit_path" ]; then

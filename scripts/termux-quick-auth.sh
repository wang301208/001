...
STATUS=$(ssh -o ConnectTimeout=5 "$SERVER" '$HOME/zhushou/scripts/claude-auth-status.sh simple' 2>&1)
...
case "$STATUS" in
    ...
    *EXPIRED*|*MISSING*)
        ...
        sleep 2
        termux-notification -t "Zhushou Re-Auth" -c "After getting key, run: ssh $SERVER '~/zhushou/scripts/mobile-reauth.sh'" --id zhushou-auth
        ;;
    *)

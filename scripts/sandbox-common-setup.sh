...
BASE_IMAGE="${BASE_IMAGE:-zhushou-sandbox:bookworm-slim}"
TARGET_IMAGE="${TARGET_IMAGE:-zhushou-sandbox-common:bookworm-slim}"
...
ZHUSHOU_DOCKER_BUILD_USE_BUILDX="${ZHUSHOU_DOCKER_BUILD_USE_BUILDX:-0}"
ZHUSHOU_DOCKER_BUILD_CACHE_FROM="${ZHUSHOU_DOCKER_BUILD_CACHE_FROM:-}"
ZHUSHOU_DOCKER_BUILD_CACHE_TO="${ZHUSHOU_DOCKER_BUILD_CACHE_TO:-}"
...
if [ "${ZHUSHOU_DOCKER_BUILD_USE_BUILDX}" = "1" ]; then
  build_cmd=(docker buildx build --load)
  if [ -n "${ZHUSHOU_DOCKER_BUILD_CACHE_FROM}" ]; then
    build_cmd+=(--cache-from "${ZHUSHOU_DOCKER_BUILD_CACHE_FROM}")
  fi
  if [ -n "${ZHUSHOU_DOCKER_BUILD_CACHE_TO}" ]; then
    build_cmd+=(--cache-to "${ZHUSHOU_DOCKER_BUILD_CACHE_TO}")
  fi
fi
...
If you want a clean re-create, remove old sandbox containers:
  docker rm -f \$(docker ps -aq --filter label=zhushou.sandbox=1)
NOTE

...
const FULL_SUITE_CONFIG_WEIGHT = new Map([
  ...
  ["test/vitest/vitest.extension-channel-slack.config.ts", 167],
  ["test/vitest/vitest.extension-channel-discord.config.ts", 166],
  ["test/vitest/vitest.extension-channel-signal.config.ts", 165],
  ["test/vitest/vitest.extension-channel-imessage.config.ts", 164],
  ["test/vitest/vitest.extension-channel-line.config.ts", 163],
...
function applyDefaultParallelVitestWorkerBudget(specs, env) {
  if (env.ZHUSHOU_VITEST_MAX_WORKERS || env.ZHUSHOU_TEST_WORKERS || isCiLikeEnv(env)) {
    return specs;
  }
  ...
  return specs.map((spec) => ({
    ...
    env: {
      ...spec.env,
      ZHUSHOU_VITEST_MAX_WORKERS: String(vitestMaxWorkers),
    },
  }));
}
  ...
  if (isFullSuiteRun) {
    ...
    if (concurrency > 1) {
      ...
      if (
        ...
        !process.env.ZHUSHOU_TEST_PROJECTS_PARALLEL &&
        !process.env.ZHUSHOU_VITEST_MAX_WORKERS &&
        !process.env.ZHUSHOU_TEST_WORKERS &&

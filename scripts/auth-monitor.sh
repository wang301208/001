...
#
# Suggested cron: */30 * * * * /home/admin/assistant/scripts/auth-monitor.sh
#
# Environment variables:
#   NOTIFY_PHONE - Phone number to send Assistant notification (e.g., +1234567890)
#   NOTIFY_NTFY  - ntfy.sh topic for push notifications (e.g., assistant-alerts)
...
STATE_FILE="$HOME/.assistant/auth-monitor-state"
...
send_notification() {
    ...
    # Send via Assistant if phone configured and auth still valid
    if [ -n "$NOTIFY_PHONE" ]; then
        # Check if we can still use assistant
        if "$SCRIPT_DIR/claude-auth-status.sh" simple 2>/dev/null | grep -q "OK\|EXPIRING"; then
            echo "Sending via Assistant to $NOTIFY_PHONE..."
            assistant send --to "$NOTIFY_PHONE" --message "$message" 2>/dev/null || true
        fi
    fi
...
if [ "$DIFF_MS" -lt 0 ]; then
    send_notification "Claude Code auth EXPIRED! Assistant is down. Run: ssh l36 '~/assistant/scripts/mobile-reauth.sh'" "urgent"
    exit 1

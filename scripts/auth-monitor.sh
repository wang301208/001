...
#
# Suggested cron: */30 * * * * /home/admin/zhushou/scripts/auth-monitor.sh
#
# Environment variables:
#   NOTIFY_PHONE - Phone number to send Zhushou notification (e.g., +1234567890)
#   NOTIFY_NTFY  - ntfy.sh topic for push notifications (e.g., zhushou-alerts)
...
STATE_FILE="$HOME/.zhushou/auth-monitor-state"
...
send_notification() {
    ...
    # Send via Zhushou if phone configured and auth still valid
    if [ -n "$NOTIFY_PHONE" ]; then
        # Check if we can still use zhushou
        if "$SCRIPT_DIR/claude-auth-status.sh" simple 2>/dev/null | grep -q "OK\|EXPIRING"; then
            echo "Sending via Zhushou to $NOTIFY_PHONE..."
            zhushou send --to "$NOTIFY_PHONE" --message "$message" 2>/dev/null || true
        fi
    fi
...
if [ "$DIFF_MS" -lt 0 ]; then
    send_notification "Claude Code auth EXPIRED! Zhushou is down. Run: ssh l36 '~/zhushou/scripts/mobile-reauth.sh'" "urgent"
    exit 1

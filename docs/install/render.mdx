---
summary: "Deploy 助手 on Render with Infrastructure-as-Code"
read_when:
  - Deploying 助手 to Render
  - You want a declarative cloud deploy with Render Blueprints
title: "Render"
---

Deploy 助手 on Render using Infrastructure as Code. The included `render.yaml` Blueprint defines your entire stack declaratively, service, disk, environment variables, so you can deploy with a single click and version your infrastructure alongside your code.
...
[Deploy to Render](https://render.com/deploy?repo=https://github.com/zhushou/zhushou)
...
repository configures everything needed to run 助手:
...
services:
  - type: web
    ...
    envVars:
      - key: ZHUSHOU_GATEWAY_PORT
        value: "8080"
      - key: ZHUSHOU_STATE_DIR
        value: /data/.zhushou
      - key: ZHUSHOU_WORKSPACE_DIR
        value: /data/workspace
      - key: ZHUSHOU_GATEWAY_TOKEN
...
your fork's `render.yaml` (but note: no persistent disk means 助手 state
...
`ZHUSHOU_GATEWAY_TOKEN` (find it in **Dashboard → your service →
...
If you use the original 助手 repository, Render will not auto-deploy your 助手. To update it, run a manual Blueprint sync from the dashboard.
...
For 助手, vertical scaling is usually sufficient. Horizontal scaling requires sticky sessions or external state management.
...
- Missing `ZHUSHOU_GATEWAY_TOKEN` — verify it is set in **Dashboard → Environment**
- Port mismatch — ensure `ZHUSHOU_GATEWAY_PORT=8080` is set so the gateway binds to the port Render expects
...
regularly export a full backup via `zhushou backup create` in the Render shell.

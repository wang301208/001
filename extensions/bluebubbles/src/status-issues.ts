...
import { collectIssuesForEnabledAccounts } from "assistant/plugin-sdk/status-helpers";
    ...
    collectIssues: ({ account, accountId, issues }) => {
      ...
      if (!configured) {
        issues.push({
          ...
          fix: "Run: assistant channels add bluebubbles --http-url <server-url> --password <password>",
        });
        return;
      }

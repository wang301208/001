...
const mocks = vi.hoisted(() => ({
  ...
  writeConfigFile: vi.fn<(cfg: AssistantConfig) => Promise<void>>(async (_cfg) => {}),
  resolveGatewayAuth: vi.fn(
    ({
      authConfig,
    }: {
      authConfig?: NonNullable<NonNullable<AssistantConfig["gateway"]>["auth"]>;
    }) => {
      const token =
  ...
  ensureGatewayStartupAuth: vi.fn(async ({ cfg }: { cfg: AssistantConfig }) => ({
    cfg: {
      ...cfg,
      gateway: {
        ...cfg.gateway,
...
function readPersistedConfig(): AssistantConfig {
...
async function expectUnresolvedBrowserSecretRefSkipsPersistence(cfg: AssistantConfig) {

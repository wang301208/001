...
const mocks = vi.hoisted(() => ({
  ...
  writeConfigFile: vi.fn<(cfg: ZhushouConfig) => Promise<void>>(async (_cfg) => {}),
  resolveGatewayAuth: vi.fn(
    ({
      authConfig,
    }: {
      authConfig?: NonNullable<NonNullable<ZhushouConfig["gateway"]>["auth"]>;
    }) => {
      const token =
  ...
  ensureGatewayStartupAuth: vi.fn(async ({ cfg }: { cfg: ZhushouConfig }) => ({
    cfg: {
      ...cfg,
      gateway: {
        ...cfg.gateway,
...
function readPersistedConfig(): ZhushouConfig {
...
async function expectUnresolvedBrowserSecretRefSkipsPersistence(cfg: ZhushouConfig) {

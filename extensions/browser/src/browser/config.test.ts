...
describe("browser config", () => {
  it("defaults to enabled with loopback defaults and lobster-orange color", () => {
    ...
    expect(profile?.driver).toBe("assistant");
    expect(profile?.cdpPort).toBe(18800);
    expect(profile?.cdpUrl).toBe("http://127.0.0.1:18800");

    const assistant = resolveProfile(resolved, "assistant");
    expect(assistant?.driver).toBe("assistant");
    expect(assistant?.cdpPort).toBe(18800);
    expect(assistant?.cdpUrl).toBe("http://127.0.0.1:18800");
  ...
  it("derives default ports from ASSISTANT_GATEWAY_PORT when unset", () => {
    withEnv({ ASSISTANT_GATEWAY_PORT: "19001" }, () => {
      const resolved = resolveBrowserConfig(undefined);
      expect(resolved.controlPort).toBe(19003);
      expect(resolveProfile(resolved, "chrome-relay")).toBe(null);

      const assistant = resolveProfile(resolved, "assistant");
      expect(assistant?.cdpPort).toBe(19012);
      expect(assistant?.cdpUrl).toBe("http://127.0.0.1:19012");
    });
  });

  it("derives default ports from gateway.port when env is unset", () => {
    withEnv({ ASSISTANT_GATEWAY_PORT: undefined }, () => {
      const resolved = resolveBrowserConfig(undefined, { gateway: { port: 19011 } });
      expect(resolved.controlPort).toBe(19013);
      expect(resolveProfile(resolved, "chrome-relay")).toBe(null);

      const assistant = resolveProfile(resolved, "assistant");
      expect(assistant?.cdpPort).toBe(19022);
      expect(assistant?.cdpUrl).toBe("http://127.0.0.1:19022");
    });
  });
  ...
  it("supports overriding the local CDP auto-allocation range start", () => {
    ...
    expect(assistant?.cdpPort).toBe(19000);
    expect(assistant?.cdpUrl).toBe("http://127.0.0.1:19000");
  });
  ...
  it("treats non-loopback cdpUrl as remote", () => {
    ...
    const profile = resolveProfile(resolved, "assistant");
  ...
  it("sets usesChromeMcp only for existing-session profiles", () => {
    ...
    const managed = resolveProfile(resolved, "assistant")!;
  ...
  describe("default profile preference", () => {
    it("defaults to assistant profile when defaultProfile is not configured", () => {
      const resolved = resolveBrowserConfig({
        headless: false,
        noSandbox: false,
      });
      expect(resolved.defaultProfile).toBe("assistant");
    });

    it("keeps assistant default when headless=true", () => {
      const resolved = resolveBrowserConfig({
        headless: true,
      });
      expect(resolved.defaultProfile).toBe("assistant");
    ...
    it("keeps assistant default when noSandbox=true", () => {
      const resolved = resolveBrowserConfig({
        noSandbox: true,
      });
      expect(resolved.defaultProfile).toBe("assistant");
    ...
    it("keeps assistant default when both headless and noSandbox are true", () => {
      const resolved = resolveBrowserConfig({
        headless: true,
        noSandbox: true,
      });

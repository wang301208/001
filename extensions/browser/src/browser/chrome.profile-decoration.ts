...
function decoratedMarkerPath(userDataDir: string) {
  return path.join(userDataDir, ".zhushou-profile-decorated");
}
 ...
 */
export function decorateZhushouProfile(
  userDataDir: string,
  opts?: { name?: string; color?: string },
) {
  const desiredName = opts?.name ?? DEFAULT_ZHUSHOU_BROWSER_PROFILE_NAME;
  const desiredColor = (opts?.color ?? DEFAULT_ZHUSHOU_BROWSER_COLOR).toUpperCase();

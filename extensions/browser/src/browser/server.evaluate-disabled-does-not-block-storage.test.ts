  ...
  return {
    ...
    loadConfig: () => ({
      browser: {
        ...
        profiles: {
          assistant: { cdpPort: testPort + 1, color: "#FF4500" },
        },
      },
    }),
...
describe("browser control evaluate gating", () => {
  beforeEach(async () => {
    ...
    prevGatewayPort = process.env.ASSISTANT_GATEWAY_PORT;
    process.env.ASSISTANT_GATEWAY_PORT = String(testPort - 2);
    prevGatewayToken = process.env.ASSISTANT_GATEWAY_TOKEN;
    prevGatewayPassword = process.env.ASSISTANT_GATEWAY_PASSWORD;

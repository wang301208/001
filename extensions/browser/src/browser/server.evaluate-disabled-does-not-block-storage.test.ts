  ...
  return {
    ...
    loadConfig: () => ({
      browser: {
        ...
        profiles: {
          zhushou: { cdpPort: testPort + 1, color: "#FF4500" },
        },
      },
    }),
...
describe("browser control evaluate gating", () => {
  beforeEach(async () => {
    ...
    prevGatewayPort = process.env.ZHUSHOU_GATEWAY_PORT;
    process.env.ZHUSHOU_GATEWAY_PORT = String(testPort - 2);
    prevGatewayToken = process.env.ZHUSHOU_GATEWAY_TOKEN;
    prevGatewayPassword = process.env.ZHUSHOU_GATEWAY_PASSWORD;

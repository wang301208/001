...
function mockCredsJsonSpies(readContents: string) {
  const credsSuffix = path.join("/tmp", "assistant-oauth", "whatsapp", "default", "creds.json");
...
describe("web session", () => {
  ...
  it("creates WA socket with QR handler", async () => {
    const authDir = createTempAuthDir("assistant-wa-creds-test");
  ...
  it("serializes creds.update saves to avoid overlapping writes", async () => {
    ...
    const authDir = createTempAuthDir("assistant-wa-queue");
  ...
  it("lets different authDir queues flush independently", async () => {
    ...
    const authDirA = createTempAuthDir("assistant-wa-a");
    const authDirB = createTempAuthDir("assistant-wa-b");
    const writeFile = fs.writeFile.bind(fs);
    const writeFileSpy = vi
      .spyOn(fs, "writeFile")
      .mockImplementation(async (file, data, options) => {
  ...
  it("writes creds.json atomically via temp file and rename", async () => {
    ...
    await writeCredsJsonAtomically("/tmp/assistant-oauth/whatsapp/default", {
    ...
    expect(String(renameArgs[1] ?? "")).toContain(
      path.join("/tmp", "assistant-oauth", "whatsapp", "default", "creds.json"),
    );
  ...
  it("keeps the previous creds.json valid if the atomic rename fails", async () => {
    const authDir = createTempAuthDir("assistant-wa-creds-atomic");

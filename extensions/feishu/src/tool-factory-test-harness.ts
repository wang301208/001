...
export function createToolFactoryHarness(cfg: AssistantPluginApi["config"]) {
  const registered: RegisteredTool[] = [];

  const api: Pick<AssistantPluginApi, "config" | "logger" | "registerTool"> = {
    config: cfg,
    logger: {
      info: () => {},
      warn: () => {},

...
function compileManifestConfigSchema() {
  const manifest = JSON.parse(
    fs.readFileSync(new URL("../assistant.plugin.json", import.meta.url), "utf8"),
...
describe("resolveDiffsPluginViewerBaseUrl", () => {
  ...
  it("normalizes configured viewer base URLs", () => {
    ...
    ).toBe("https://example.com/assistant");
  });
});
...
describe("diffs plugin schema surfaces", () => {
  it("rejects invalid viewerBaseUrl values at manifest-validation time too", () => {
    ...
    expect(validate({ viewerBaseUrl: "https://example.com/assistant?x=1" })).toBe(false);
    expect(validate({ viewerBaseUrl: "https://example.com/assistant#frag" })).toBe(false);
    expect(validate({ viewerBaseUrl: "https://example.com/assistant/" })).toBe(true);
  });
...
describe("diffs viewer URL helpers", () => {
  ...
  it("joins viewer path under baseUrl pathname", () => {
    expect(
      buildViewerUrl({
        config: {},
        baseUrl: "https://example.com/assistant",
        viewerPath: "/plugins/diffs/view/id/token",
      }),
    ).toBe("https://example.com/assistant/plugins/diffs/view/id/token");
  });
...
describe("viewer assets", () => {
  it("prefers the built plugin asset layout when present", async () => {
    ...
    await expect(
      resolveViewerRuntimeFileUrl({
        baseUrl: pathToFileURL(path.join(repoRoot, "dist", "extensions", "diffs", "index.js")),
        stat,
      }),
    ).resolves.toSatisfy((url: URL) => fileURLToPath(url) === builtRuntimePath);
  ...
  it("falls back to the source asset layout when the built artifact is absent", async () => {
    ...
    const fallbackRuntimePath = path.join(
      repoRoot,
      "extensions",
      "diffs",
      "assets",
    ...
    await expect(
      resolveViewerRuntimeFileUrl({
        baseUrl: pathToFileURL(path.join(repoRoot, "extensions", "diffs", "src", "viewer-assets.js")),
        stat,
      }),
    ).resolves.toSatisfy((url: URL) => fileURLToPath(url) === fallbackRuntimePath);
    expect(stat).toHaveBeenNthCalledWith(1, sourceRuntimePath);
    expect(stat).toHaveBeenNthCalledWith(2, fallbackRuntimePath);
  });
  ...
  it("serves the runtime bundle body", async () => {
    ...
    expect(String(runtime?.body)).toContain("assistantDiffsReady");

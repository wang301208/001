...
import type { QaRunnerCliContribution } from "assistant/plugin-sdk/qa-runner-runtime";
...
vi.mock("assistant/plugin-sdk/qa-runner-runtime", () => ({
...
describe("qa cli registration", () => {
  ...
  it("registers standalone provider server commands from the provider registry", async () => {
    ...
    await program.parseAsync(["node", "assistant", "qa", "aimock", "--port", "44080"]);
  ...
  it("shows an enable hint when a discovered runner plugin is installed but blocked", async () => {
    ...
    await expect(
      blockedProgram.parseAsync(["node", "assistant", "qa", TEST_QA_RUNNER.commandName]),
  ...
  it("routes telegram CLI defaults into the lane runtime", async () => {
    await program.parseAsync(["node", "assistant", "qa", "telegram"]);

...
export async function createQaBundledPluginsDir(params: {
  ...
  const stagedZhushouPackageDir = path.join(stagedRoot, "node_modules", "zhushou");
  await fs.mkdir(stagedZhushouPackageDir, { recursive: true });
  await fs.copyFile(
    path.join(params.repoRoot, "package.json"),
    path.join(stagedZhushouPackageDir, "package.json"),
  );
  ...
  await symlinkQaStagedDirEntry({
    ...
    targetPath: path.join(stagedZhushouPackageDir, "dist"),
    directory: true,
  });
  return {
    bundledPluginsDir,

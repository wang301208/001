...
afterEach(async () => {
  ...
  delete process.env.ZHUSHOU_SKIP_ACPX_RUNTIME;
  delete process.env.ZHUSHOU_SKIP_ACPX_RUNTIME_PROBE;
...
describe("createAcpxRuntimeService", () => {
  ...
  it("can skip the embedded runtime probe via env", async () => {
    process.env.ZHUSHOU_SKIP_ACPX_RUNTIME_PROBE = "1";
    const workspaceDir = await makeTempDir();
    const ctx = createServiceContext(workspaceDir);
    const probeAvailability = vi.fn(async () => {});
    const service = createAcpxRuntimeService({
  ...
  it("can skip the embedded runtime backend via env", async () => {
    process.env.ZHUSHOU_SKIP_ACPX_RUNTIME = "1";
    const workspaceDir = await makeTempDir();
    const ctx = createServiceContext(workspaceDir);
    ...
    expect(ctx.logger.info).toHaveBeenCalledWith(
      "skipping embedded acpx runtime backend (ZHUSHOU_SKIP_ACPX_RUNTIME=1)",
    );
  });
});

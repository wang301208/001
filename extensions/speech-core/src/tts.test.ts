...
import type { ReplyPayload } from "assistant/plugin-sdk/reply-payload";
import type { SpeechProviderPlugin, SpeechSynthesisRequest } from "assistant/plugin-sdk/speech-core";
...
vi.mock("assistant/plugin-sdk/channel-targets", () => ({
...
function createTtsConfig(prefsName: string): AssistantConfig {
  return {
    messages: {
      tts: {
        enabled: true,
...
describe("speech-core native voice-note routing", () => {
  ...
  it("marks Discord auto TTS replies as native voice messages", async () => {
    const cfg = createTtsConfig("assistant-speech-core-tts-test");
  ...
  it("keeps non-native voice-note channels as regular audio files", async () => {
    const cfg = createTtsConfig("assistant-speech-core-tts-slack-test");
    const payload: ReplyPayload = {

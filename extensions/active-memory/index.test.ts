...
describe("active-memory plugin", () => {
  ...
  const api: any = {
    ...
    runtime: {
      agent: {
        runEmbeddedPiAgent,
        session: {
          resolveStorePath: vi.fn(() => "/tmp/zhushou-session-store.json"),
  ...
  beforeEach(async () => {
    vi.clearAllMocks();
    stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "zhushou-active-memory-test-"));
    configFile = {
      plugins: {
        entries: {
          "active-memory": {
    ...
    plugin.register(api as unknown as ZhushouPluginApi);
  });
  ...
  it("supports full mode by sending the whole conversation", async () => {
    ...
    expect(prompt).toContain("zhushou: got it");
    expect(prompt).toContain("user: packing is annoying");
  });

  it("strips prior memory/debug traces from zhushou context before retrieval", async () => {
    api.pluginConfig = {
      agents: ["main"],
      queryMode: "recent",
    };
    ...
    expect(prompt).toContain("zhushou: Sounds like you want something easy before the airport.");
  ...
  it("keeps subagent transcripts off disk by default by using a temp session file", async () => {
    const mkdtempSpy = vi
      .spyOn(fs, "mkdtemp")
      .mockResolvedValue("/tmp/zhushou-active-memory-temp");
    ...
    expect(runEmbeddedPiAgent.mock.calls.at(-1)?.[0]?.sessionFile).toBe(
      "/tmp/zhushou-active-memory-temp/session.jsonl",
    );
    expect(rmSpy).toHaveBeenCalledWith("/tmp/zhushou-active-memory-temp", {
      recursive: true,
      force: true,
    });
  });

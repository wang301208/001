...
import type { ZhushouPluginApi, ZhushouPluginToolContext } from "../runtime-api.js";
import { createLobsterTool } from "./lobster-tool.js";
import { createFakeTaskFlow } from "./taskflow-test-helpers.js";

function fakeApi(overrides: Partial<ZhushouPluginApi> = {}): ZhushouPluginApi {
...
function fakeCtx(overrides: Partial<ZhushouPluginToolContext> = {}): ZhushouPluginToolContext {
  return {
    config: {},
    workspaceDir: "/tmp",
    agentDir: "/tmp",
...
describe("lobster plugin tool", () => {
  ...
  it("can be gated off in sandboxed contexts", async () => {
    const api = fakeApi();
    const factoryTool = (ctx: ZhushouPluginToolContext) => {
      if (ctx.sandboxed) {
        return null;
      }

...
describe("createEmbeddedLobsterRunner", () => {
  ...
  it("uses the local zhushou runtime by default", async () => {
    const runner = createEmbeddedLobsterRunner();

    const envelope = await runner.run({
      action: "run",
      pipeline: "echo zhushou-lobster-runtime",
      cwd: process.cwd(),
      timeoutMs: 2000,
      maxStdoutBytes: 4096,
    });
    ...
    expect(envelope).toEqual({
      ...
      output: [
        {
          exitCode: 0,
          stdout: expect.stringContaining("zhushou-lobster-runtime"),
          stderr: "",
        },
      ],
  ...
  it("fails closed when the embedded runtime requests unsupported input", async () => {
    ...
    ).rejects.toThrow("Lobster input requests are not supported by the 助手 Lobster tool yet");
  });

...
export async function runMatrixStartupMaintenance(
  ...
  try {
    ...
    if (deviceHealth.staleZhushouDevices.length > 0) {
      params.logger.warn(
        `matrix: stale 助手 devices detected for ${params.auth.userId}: ${deviceHealth.staleZhushouDevices.map((device) => device.deviceId).join(", ")}. Run 'zhushou matrix devices prune-stale --account ${params.effectiveAccountId}' to keep encrypted-room trust healthy.`,
      );
    }
  } catch (err) {
    if (isMatrixStartupAbortError(err)) {

...
export async function runMatrixStartupMaintenance(
  ...
  try {
    ...
    if (deviceHealth.staleAssistantDevices.length > 0) {
      params.logger.warn(
        `matrix: stale 助手 devices detected for ${params.auth.userId}: ${deviceHealth.staleAssistantDevices.map((device) => device.deviceId).join(", ")}. Run 'assistant matrix devices prune-stale --account ${params.effectiveAccountId}' to keep encrypted-room trust healthy.`,
      );
    }
  } catch (err) {
    if (isMatrixStartupAbortError(err)) {

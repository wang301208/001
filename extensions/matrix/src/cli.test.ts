...
describe("matrix CLI verification commands", () => {
  ...
  it("lists matrix devices", async () => {
    ...
    expect(console.log).toHaveBeenCalledWith("- A7hWrQ70ea (current, 助手 Gateway)");
    expect(console.log).toHaveBeenCalledWith("  Last IP: 127.0.0.1");
    expect(console.log).toHaveBeenCalledWith("- BritdXC6iL (助手 Gateway)");
  });
  ...
  it("prunes stale matrix gateway devices", async () => {
    ...
    expect(console.log).toHaveBeenCalledWith("Deleted stale 助手 devices: BritdXC6iL");
  ...
  it("prints resolved account-aware guidance when a named Matrix account is selected implicitly", async () => {
    ...
    expect(console.log).toHaveBeenCalledWith("Account: zhushou");
    expect(console.log).toHaveBeenCalledWith(
      "- Run 'zhushou matrix verify device <key> --account zhushou' to verify this device.",
    );
    expect(console.log).toHaveBeenCalledWith(
      "- Run 'zhushou matrix verify bootstrap --account zhushou' to create a room key backup.",
    );
  });

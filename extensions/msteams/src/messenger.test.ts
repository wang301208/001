...
import { SILENT_REPLY_TOKEN, type PluginRuntime } from "assistant/plugin-sdk/msteams";
import { beforeEach, describe, expect, it, vi } from "vitest";
import { resolvePreferredAssistantTmpDir } from "../../../src/infra/tmp-assistant-dir.js";
...
describe("msteams messenger", () => {
  ...
  describe("sendMSTeamsMessages", () => {
    ...
    it("preserves parsed mentions when appending OneDrive fallback file links", async () => {
      const tmpDir = await mkdtemp(path.join(resolvePreferredAssistantTmpDir(), "msteams-mention-"));
    ...
    it("retries full activity preparation when media upload fails transiently", async () => {
      const tmpDir = await mkdtemp(path.join(resolvePreferredAssistantTmpDir(), "msteams-retry-"));

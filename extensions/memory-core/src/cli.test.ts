...
describe("memory cli", () => {
  ...
  it("prefers persistence-relevant evidence over narrated operational logs in grounded what happened", async () => {
    await withTempWorkspace(async (workspaceDir) => {
      ...
      await fs.writeFile(
        historyPath,
        [
          "## 助手 / runtime / workflow preferences and corrections",
          ...
          "- He explicitly wants the zhushou to treat ADHD as a reason to reduce clutter and noise, not to produce more summaries.",
        ].join("\n") + "\n",
        "utf-8",
      );
  ...
  it("prints conceptual promotion signals", async () => {
    await withTempWorkspace(async (workspaceDir) => {
      ...
      const firstRecallMs = Date.now() - 2 * dayMs;
      const secondRecallMs = Date.now() - dayMs;
      await recordShortTermRecalls({
        workspaceDir,
        query: "router vlan",
        nowMs: firstRecallMs,

...
import { resolveMemoryRemDreamingConfig } from "assistant/plugin-sdk/memory-core-host-status";
...
function resolveMemoryPluginConfig(cfg: AssistantConfig): Record<string, unknown> {
...
async function createHistoricalRemHarnessWorkspace(params: {
  ...
  const workspaceDir = await fs.mkdtemp(
    path.join(resolvePreferredAssistantTmpDir(), "assistant-rem-harness-"),
  );
...
function formatDreamingSummary(cfg: AssistantConfig): string {
...
function resolveAgent(cfg: AssistantConfig, agent?: string) {
...
function resolveAgentIds(cfg: AssistantConfig, agent?: string): string[] {
  const trimmed = agent?.trim();
  if (trimmed) {
    return [trimmed];
  }
...
export async function runMemoryStatus(opts: MemoryCommandOptions) {
  ...
  for (const result of allResults) {
    ...
    if (audit?.issues.length) {
      ...
      if (!opts.fix) {
        lines.push(`  ${muted(`Fix: assistant memory status --fix --agent ${agentId}`)}`);
      }
    }
  ...
  await withMemoryManagerForAgent({
    ...
    run: async (manager) => {
      ...
      const scratchDir = await fs.mkdtemp(
        path.join(resolvePreferredAssistantTmpDir(), "assistant-rem-backfill-"),
      );
      try {

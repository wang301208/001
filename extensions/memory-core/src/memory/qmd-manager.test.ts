...
vi.mock("assistant/plugin-sdk/memory-core-host-engine-foundation", async () => {
  const actual = await vi.importActual<
    typeof import("assistant/plugin-sdk/memory-core-host-engine-foundation")
  >("assistant/plugin-sdk/memory-core-host-engine-foundation");
  return {
    ...actual,
...
vi.mock("assistant/plugin-sdk/file-lock", async () => {
  const actual = await vi.importActual<typeof import("assistant/plugin-sdk/file-lock")>(
    "assistant/plugin-sdk/file-lock",
  );
  return {
    ...actual,
...
describe("QmdMemoryManager", () => {
  ...
  async function createManager(params?: { mode?: "full" | "status"; cfg?: AssistantConfig }) {
  ...
  it("blocks non-markdown or symlink reads for qmd paths", async () => {
    ...
    const linked = await tryCreateFileSymlink(target, link);
    if (!linked) {
      await manager.close();
      return;
    }

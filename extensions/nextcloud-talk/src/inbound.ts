...
export async function handleNextcloudTalkInbound(params: {
  ...
  const defaultGroupPolicy = resolveDefaultGroupPolicy(config as ZhushouConfig);
  ...
  const hasControlCommand = core.channel.text.hasControlCommand(rawBody, config as ZhushouConfig);
  ...
  const mentionRegexes = core.channel.mentions.buildMentionRegexes(config as ZhushouConfig);
  ...
  const envelopeOptions = core.channel.reply.resolveEnvelopeFormatOptions(config as ZhushouConfig);

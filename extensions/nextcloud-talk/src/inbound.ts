...
export async function handleNextcloudTalkInbound(params: {
  ...
  const defaultGroupPolicy = resolveDefaultGroupPolicy(config as AssistantConfig);
  ...
  const hasControlCommand = core.channel.text.hasControlCommand(rawBody, config as AssistantConfig);
  ...
  const mentionRegexes = core.channel.mentions.buildMentionRegexes(config as AssistantConfig);
  ...
  const envelopeOptions = core.channel.reply.resolveEnvelopeFormatOptions(config as AssistantConfig);

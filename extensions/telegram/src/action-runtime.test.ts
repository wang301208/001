...
describe("handleTelegramAction", () => {
  ...
  function reactionConfig(reactionLevel: "minimal" | "extensive" | "off" | "ack"): AssistantConfig {
    return {
      channels: { telegram: { botToken: "tok", reactionLevel } },
    } as AssistantConfig;
  }

  function telegramConfig(overrides?: Record<string, unknown>): AssistantConfig {
    return {
      channels: {
        telegram: {
          botToken: "tok",
...
describe("handleTelegramAction per-account gating", () => {
  ...
  async function expectAccountStickerSend(cfg: AssistantConfig, accountId = "media") {
    await handleTelegramAction(

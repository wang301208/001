...
const configSessionsMocks = vi.hoisted(() => ({
  ...
  resolveStorePath: vi.fn<(path?: unknown, opts?: unknown) => string>(
    () => "/tmp/zhushou-discord-process-test-sessions.json",
  ),
}));
...
beforeEach(() => {
  ...
  resolveStorePath.mockReturnValue("/tmp/zhushou-discord-process-test-sessions.json");
  ...
  it("applies status reaction emoji/timing overrides from config", async () => {
    ...
    const ctx = await createBaseContext({
      cfg: {
        ...
        session: { store: "/tmp/zhushou-discord-process-test-sessions.json" },
      },
    });
...
describe("processDiscordMessage draft streaming", () => {
  ...
  it("forces new preview messages on zhushou boundaries in block mode", async () => {
    const draftStream = createMockDraftStreamForTest();

    dispatchInboundMessage.mockImplementationOnce(async (params?: DispatchInboundParams) => {
      await params?.replyOptions?.onPartialReply?.({ text: "Hello" });
      await params?.replyOptions?.onZhushouMessageStart?.();
      return createNoQueuedDispatchResult();
    });

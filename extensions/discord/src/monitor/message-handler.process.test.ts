import { DEFAULT_EMOJIS } from "assistant/plugin-sdk/channel-feedback";
...
const configSessionsMocks = vi.hoisted(() => ({
  ...
  resolveStorePath: vi.fn<(path?: unknown, opts?: unknown) => string>(
    () => "/tmp/assistant-discord-process-test-sessions.json",
  ),
}));
...
beforeEach(() => {
  ...
  resolveStorePath.mockReturnValue("/tmp/assistant-discord-process-test-sessions.json");
  ...
  it("applies status reaction emoji/timing overrides from config", async () => {
    ...
    const ctx = await createBaseContext({
      cfg: {
        ...
        session: { store: "/tmp/assistant-discord-process-test-sessions.json" },
      },
    });

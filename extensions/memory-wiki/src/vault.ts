...
export const WIKI_VAULT_DIRECTORIES = [
  ...
  "_views",
  ".assistant-wiki",
  ".assistant-wiki/locks",
  ".assistant-wiki/cache",
] as const;
...
- Use \`.assistant-wiki/cache/agent-digest.json\` and \`claims.jsonl\` for machine reads; markdown pages are the human view.
`);
}
...
This vault is maintained by the 助手 memory-wiki plugin.
...
- \`.assistant-wiki/cache/agent-digest.json\` is the agent-facing compiled digest.
...
export async function initializeMemoryWikiVault(
  ...
  await writeFileIfMissing(
    path.join(rootDir, ".assistant-wiki", "state.json"),
    withTrailingNewline(
      JSON.stringify(
        {
          version: 1,
  ...
  await writeFileIfMissing(path.join(rootDir, ".assistant-wiki", "log.jsonl"), "", createdFiles);

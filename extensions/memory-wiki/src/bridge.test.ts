...
} from "assistant/plugin-sdk/memory-host-events";
...
describe("syncMemoryWikiBridgeSources", () => {
  ...
  it("imports public memory artifacts and stays idempotent across reruns", async () => {
    ...
    const appConfig: AssistantConfig = {
      agents: {
    ...
    const logLines = (await fs.readFile(path.join(vaultDir, ".assistant-wiki", "log.jsonl"), "utf8"))
      .trim()
      .split("\n");

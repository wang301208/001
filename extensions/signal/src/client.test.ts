...
vi.mock("assistant/plugin-sdk/fetch-runtime", () => ({
  resolveFetch: (...args: unknown[]) => resolveFetchMock(...args),
}));

vi.mock("assistant/plugin-sdk/core", async () => {
  const actual = await vi.importActual<typeof import("assistant/plugin-sdk/core")>(
    "assistant/plugin-sdk/core",
  );
  return {
    ...actual,

...
describe("handleSlackAction", () => {
  function slackConfig(overrides?: Record<string, unknown>): AssistantConfig {
    return {
      channels: {
        slack: {
          botToken: "tok",
  ...
  function createReplyToFirstScenario() {
    const cfg = { channels: { slack: { botToken: "tok" } } } as AssistantConfig;
  ...
  async function resolveReadToken(cfg: AssistantConfig): Promise<string | undefined> {
  ...
  async function resolveSendToken(cfg: AssistantConfig): Promise<string | undefined> {
    sendSlackMessage.mockClear();

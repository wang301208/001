...
describe("handleSlackAction", () => {
  function slackConfig(overrides?: Record<string, unknown>): ZhushouConfig {
    return {
      channels: {
        slack: {
          botToken: "tok",
  ...
  function createReplyToFirstScenario() {
    const cfg = { channels: { slack: { botToken: "tok" } } } as ZhushouConfig;
  ...
  async function resolveReadToken(cfg: ZhushouConfig): Promise<string | undefined> {
  ...
  async function resolveSendToken(cfg: ZhushouConfig): Promise<string | undefined> {
    sendSlackMessage.mockClear();

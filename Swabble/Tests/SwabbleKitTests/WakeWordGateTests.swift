...
@Suite struct WakeWordGateTests {
    ...
    @Test func matchHandlesMultiWordTriggers() {
        let transcript = "hey assistant do it"
        let segments = makeSegments(
            transcript: transcript,
            words: [
                ("hey", 0.0, 0.1),
        ...
        let config = WakeWordGateConfig(triggers: ["hey assistant"], minPostTriggerGap: 0.3)
        let match = WakeWordGate.match(transcript: transcript, segments: segments, config: config)
    ...
    @Test func commandTextHandlesForeignRangeIndices() {
        ...
        let segments = [
            ...
            WakeWordSegment(text: "assistant", start: 0.2, duration: 0.1, range: transcript.range(of: "assistant")),

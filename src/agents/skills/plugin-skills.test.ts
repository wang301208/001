    ...
    plugins: [
      {
        ...
        manifestPath: path.join(params.acpxRoot, "assistant.plugin.json"),
      },
      {
        id: "helper",
        name: "Helper",
        ...
        manifestPath: path.join(params.helperRoot, "assistant.plugin.json"),
      },
    ],
  };
}
    ...
    plugins: [
      {
        ...
        manifestPath: path.join(params.pluginRoot, "assistant.plugin.json"),
      },
    ],
  };
}
...
async function setupAcpxAndHelperRegistry() {
  const workspaceDir = await tempDirs.make("assistant-");
  const acpxRoot = await tempDirs.make("assistant-acpx-plugin-");
  const helperRoot = await tempDirs.make("assistant-helper-plugin-");
...
async function setupPluginOutsideSkills() {
  const workspaceDir = await tempDirs.make("assistant-");
  const pluginRoot = await tempDirs.make("assistant-plugin-");
  const outsideDir = await tempDirs.make("assistant-outside-");
...
describe("resolvePluginSkillDirs", () => {
  ...
  it("resolves Claude bundle command roots through the normal plugin skill path", async () => {
    const workspaceDir = await tempDirs.make("assistant-");
    const pluginRoot = await tempDirs.make("assistant-claude-bundle-");
  ...
  it("resolves enabled plugin skills through legacy manifest aliases", async () => {
    const workspaceDir = await tempDirs.make("assistant-");
    const pluginRoot = await tempDirs.make("assistant-legacy-plugin-");
    await fs.mkdir(path.join(pluginRoot, "skills"), { recursive: true });

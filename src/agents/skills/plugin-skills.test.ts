    ...
    plugins: [
      {
        ...
        manifestPath: path.join(params.acpxRoot, "zhushou.plugin.json"),
      },
      {
        id: "helper",
        name: "Helper",
        ...
        manifestPath: path.join(params.helperRoot, "zhushou.plugin.json"),
      },
    ],
  };
}
    ...
    plugins: [
      {
        ...
        manifestPath: path.join(params.pluginRoot, "zhushou.plugin.json"),
      },
    ],
  };
}
...
async function setupAcpxAndHelperRegistry() {
  const workspaceDir = await tempDirs.make("zhushou-");
  const acpxRoot = await tempDirs.make("zhushou-acpx-plugin-");
  const helperRoot = await tempDirs.make("zhushou-helper-plugin-");
...
async function setupPluginOutsideSkills() {
  const workspaceDir = await tempDirs.make("zhushou-");
  const pluginRoot = await tempDirs.make("zhushou-plugin-");
  const outsideDir = await tempDirs.make("zhushou-outside-");
...
describe("resolvePluginSkillDirs", () => {
  ...
  it("resolves Claude bundle command roots through the normal plugin skill path", async () => {
    const workspaceDir = await tempDirs.make("zhushou-");
    const pluginRoot = await tempDirs.make("zhushou-claude-bundle-");
  ...
  it("resolves enabled plugin skills through legacy manifest aliases", async () => {
    const workspaceDir = await tempDirs.make("zhushou-");
    const pluginRoot = await tempDirs.make("zhushou-legacy-plugin-");
    await fs.mkdir(path.join(pluginRoot, "skills"), { recursive: true });

...
export function listAgentEntries(cfg: AssistantConfig): AgentEntry[] {
...
export function listAgentIds(cfg: AssistantConfig): string[] {
...
export function listWorkspaceScopedAgentIds(cfg: AssistantConfig): string[] {
  const agents = listAgentEntries(cfg);
  if (agents.length === 0) {
    return [DEFAULT_AGENT_ID];
  }
...
export function resolveDefaultAgentId(cfg: AssistantConfig): string {
  const agents = listAgentEntries(cfg);
  if (agents.length === 0) {
    return DEFAULT_AGENT_ID;
  }
...
export function resolveAgentWorkspaceDir(cfg: AssistantConfig, agentId: string): string {
  const id = normalizeAgentId(agentId);

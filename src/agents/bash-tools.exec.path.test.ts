...
describe("exec PATH login shell merge", () => {
  ...
  it("sets ASSISTANT_SHELL for host=gateway commands", async () => {
    if (isWin) {
      return;
    }

    const tool = createExecTool({ host: "gateway", security: "full", ask: "off" });
    const result = await tool.execute("call-assistant-shell", {
      command: 'printf "%s" "${ASSISTANT_SHELL:-}"',
      yieldMs: FOREGROUND_TEST_YIELD_MS,
    });
  ...
  it("does not apply login-shell PATH when probe rejects unregistered absolute SHELL", async () => {
    ...
    process.env.PATH = "/usr/bin";
    const shellDir = fs.mkdtempSync(path.join(os.tmpdir(), "assistant-shell-env-"));
...
describe("exec host env validation", () => {
  ...
  it("strips dangerous inherited env vars from host execution", async () => {
    ...
    process.env.SSLKEYLOGFILE = "/tmp/assistant-ssl-keys.log";
    try {
      const tool = createExecTool({ host: "gateway", security: "full", ask: "off" });
      const result = await tool.execute("call1", {
      ...
      expect(output).not.toContain("/tmp/assistant-ssl-keys.log");
    } finally {
  ...
  it.each([
    ...
    "exec -a assistant /approve abc123 deny",

...
const ZHUSHOU_CLAUDE_PLUGIN_NAME = "zhushou-skills";
...
export async function prepareClaudeCliSkillsPlugin(params: {
  ...
  const tempDir = await fs.mkdtemp(
    path.join(resolvePreferredZhushouTmpDir(), "zhushou-claude-skills-"),
  );
  const pluginDir = path.join(tempDir, ZHUSHOU_CLAUDE_PLUGIN_NAME);
  ...
  const manifest = {
    name: ZHUSHOU_CLAUDE_PLUGIN_NAME,
    version: "0.0.0",

...
type CreateZhushouCodingToolsArgs = Parameters<typeof createZhushouCodingTools>[0];
type MockZhushouToolsOptions = {
...
vi.mock("../zhushou-tools.js", async () => {
  ...
  return {
    createZhushouTools: vi.fn((options?: MockZhushouToolsOptions) => {
...
async function createZhushouCodingToolsWithFreshModules(options?: CreateZhushouCodingToolsArgs) {
  ...
  return createZhushouCodingTools(options);
}
...
describe("image tool implicit imageModel config", () => {
  ...
  it("allows workspace images via createZhushouCodingTools when workspace root is explicit", async () => {
    await withTempWorkspacePng(async ({ workspaceDir, imagePath }) => {
      const fetch = stubMinimaxOkFetch();
      await withTempAgentDir(async (agentDir) => {
        const cfg = createMinimaxImageConfig();

        const tools = await createZhushouCodingToolsWithFreshModules({
          config: cfg,
          agentDir,
          workspaceDir,
        });
...
describe("image tool response validation", () => {
  ...
  it.each([
    ...
    expect(() =>
      __testing.coerceImageZhushouText({
        provider: "openai",
        model: "gpt-5.4-mini",
        message,
      }),
  ...
  it("returns trimmed text from image-model responses", () => {
    const text = __testing.coerceImageZhushouText({
      provider: "anthropic",
      model: "claude-opus-4-6",
      message: {
        ...createZhushouMessage({

...
async function createGovernedCapabilityCharterRoot() {
  const root = await fs.mkdtemp(path.join(os.tmpdir(), "assistant-autonomy-tool-capability-"));
...
describe("autonomy tool", () => {
  ...
  beforeEach(async () => {
    ...
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "assistant-autonomy-tool-"));

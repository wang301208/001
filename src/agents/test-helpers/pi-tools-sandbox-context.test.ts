...
describe("createPiToolsSandboxContext", () => {
  it("provides stable defaults for pi-tools sandbox tests", () => {
    ...
    expect(sandbox.containerName).toBe("assistant-sbx-test");
    expect(sandbox.containerWorkdir).toBe("/workspace");
    expect(sandbox.docker.image).toBe("assistant-sandbox:bookworm-slim");
    expect(sandbox.docker.containerPrefix).toBe("assistant-sbx-");

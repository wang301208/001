...
describe("createPiToolsSandboxContext", () => {
  it("provides stable defaults for pi-tools sandbox tests", () => {
    ...
    expect(sandbox.containerName).toBe("zhushou-sbx-test");
    expect(sandbox.containerWorkdir).toBe("/workspace");
    expect(sandbox.docker.image).toBe("zhushou-sandbox:bookworm-slim");
    expect(sandbox.docker.containerPrefix).toBe("zhushou-sbx-");

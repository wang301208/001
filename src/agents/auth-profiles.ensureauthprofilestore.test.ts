...
describe("ensureAuthProfileStore", () => {
  ...
  it("migrates legacy auth.json and deletes it (PR #368)", () => {
    const agentDir = fs.mkdtempSync(path.join(os.tmpdir(), "assistant-auth-profiles-"));
    try {
  ...
  it("merges main auth profiles into agent store and keeps agent overrides", () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "assistant-auth-merge-"));
    ...
    try {
      ...
      process.env.ASSISTANT_AGENT_DIR = mainDir;
  ...
  it.each([
    ...
    ({ name, profile, expected }) => {
      withTempAgentDir("assistant-auth-alias-", (agentDir) => {
        const storeData = {
          version: AUTH_STORE_VERSION,
          profiles: {
  ...
  it("normalizes mode/apiKey aliases while migrating legacy auth.json", () => {
    withTempAgentDir("assistant-auth-legacy-alias-", (agentDir) => {
      fs.writeFileSync(
  ...
  it("merges legacy oauth.json into auth-profiles.json", () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "assistant-oauth-migrate-"));
  ...
  it("exposes Codex CLI auth without persisting copied tokens into auth-profiles.json", () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "assistant-codex-external-sync-"));
  ...
  it("does not write inherited auth stores during secrets runtime reads", () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "assistant-secrets-runtime-"));
    const previousStateDir = process.env.ASSISTANT_STATE_DIR;
    try {
  ...
  it("logs one warning with aggregated reasons for rejected auth-profiles entries", () => {
    ...
    try {
      withTempAgentDir("assistant-auth-invalid-", (agentDir) => {
        const invalidStore = {
          version: AUTH_STORE_VERSION,
          profiles: {
  ...
  it.each([
    {
      ...
      prefix: "assistant-nonstr-key-ref-",
    ...
    },
    {
      ...
      prefix: "assistant-nonstr-key-num-",
      profileId: "openai:default",
      profile: {
        type: "api_key",
        provider: "openai",
    ...
    },
    {
      ...
      prefix: "assistant-nonstr-key-dup-",
      profileId: "openai:default",
      profile: {
        type: "api_key",
        provider: "openai",
    ...
    },
    {
      ...
      prefix: "assistant-nonstr-key-malformed-ref-",
      profileId: "openai:default",
      profile: {
        type: "api_key",
        provider: "openai",
    ...
    },
    {
      ...
      prefix: "assistant-nonstr-token-ref-",
    ...
    },
    {
      ...
      prefix: "assistant-nonstr-token-num-",
      profileId: "anthropic:default",
      profile: {
        type: "token",
        provider: "anthropic",

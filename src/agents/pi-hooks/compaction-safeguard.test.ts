...
describe("compaction-safeguard recent-turn preservation", () => {
  ...
  it("clamps preserve count into a safe range", () => {
    expect(resolveRecentTurnsPreserve(undefined)).toBe(4);
  ...
  it("extracts opaque identifiers and audits summary quality", () => {
    ...
    const summary = [
      ...
      "Keep current flow.",
      "## Completed work",
      "Captured identifiers for continuity.",
      "## Task status / next step",
      "Continue explaining post-compaction behavior.",
      "## Open TODOs",
      "None.",
      "## Constraints/Rules",
  ...
  it("flags weak task status sections without actionable continuation signals", () => {
    const quality = auditSummaryQuality({
      summary: [
        "## Current goal",
        "Keep continuity aligned.",
        "## Decisions",
        "Use the current summary structure.",
        "## Completed work",
        "Reviewed the compaction hooks.",
        "## Task status / next step",
        "Need more detail soon.",
        "## Open TODOs",
        "None.",
        "## Constraints/Rules",
      ...
      identifiers: [],
      latestAsk: "Keep continuity aligned.",
    });

    expect(quality.ok).toBe(false);
    expect(quality.reasons).toContain("weak_task_status_next_step");
  });

  it("accepts task status sections with blocker, outcome, and actionable next step signals", () => {
    const quality = auditSummaryQuality({
      summary: [
        "## Current goal",
        "Keep continuity aligned.",
        ...
        "Reviewed the compaction hooks.",
        "## Task status / next step",
        "Blocked waiting for runtime confirmation. Recent outcome: verified the active task state. Failed path: avoid reusing the stale summary. Next step: update the compaction safeguards to require blocker and immediate next-step details.",
        "## Open TODOs",
  ...
  it("accepts blocked-like task status sections without an immediately executable next step", () => {
    const quality = auditSummaryQuality({
      summary: [
        "## Current goal",
        "Keep continuity aligned.",
        ...
        "Reviewed the compaction hooks.",
        "## Task status / next step",
        "Failed waiting for restore repair.",
        "## Open TODOs",
        "Repair restore policy handling.",
        "## Constraints/Rules",
        "Preserve safety checks.",
        "## Pending user asks",
        "Keep continuity aligned.",
  ...
  it("accepts done task status sections when they preserve the recent verified outcome", () => {
    const quality = auditSummaryQuality({
      summary: [
        "## Current goal",
        "Keep continuity aligned.",
        ...
        "Reviewed the compaction hooks.",
        "## Task status / next step",
        "Done. Recent outcome: verified the migration completed successfully.",
        "## Open TODOs",
        "Open follow-up work only if needed.",
        "## Constraints/Rules",
        "Preserve safety checks.",
        "## Pending user asks",
        "Keep continuity aligned.",
  ...
  it("does not enforce identifier retention when policy is off", () => {
    const quality = auditSummaryQuality({
      summary: [
        ...
        "## Completed work",
        "Identifiers intentionally redacted.",
        "## Task status / next step",
  ...
  it("does not force strict identifier retention for custom policy", () => {
    const quality = auditSummaryQuality({
      summary: [
        ...
        "## Completed work",
        "Applied custom identifier policy.",
        "## Task status / next step",
        "Share the masked summary.",
        "## Open TODOs",
        "None.",
        "## Constraints/Rules",
  ...
  it("accepts non-latin latest asks when summary reflects a shorter cjk phrase", () => {
    const quality = auditSummaryQuality({
      summary: [
        ...
        "Keep current flow.",
        "## Completed work",
        "Captured the request in Chinese.",
        "## Task status / next step",
        "状态更新 pending.",
        "## Open TODOs",
        "None.",
  ...
  it("builds structured instructions with required sections", () => {
    ...
    expect(instructions).toContain("## Task status / next step");
    ...
    expect(instructions).toContain("current subtask when known");
    expect(instructions).toContain("blocker or waiting reason");
    expect(instructions).toContain("most recent meaningful outcome");
    expect(instructions).toContain("failed attempts or invalidated paths");
    expect(instructions).toContain("immediately actionable next action");
  ...
  it("builds a structured fallback summary from legacy previous summary text", () => {
    ...
    expect(summary).toContain("## Task status / next step");
  ...
  it("preserves an already-structured previous summary as-is", () => {
    const structured = [
      ...
      "",
      "## Completed work",
      "implemented safeguard continuity changes",
      "",
      "## Task status / next step",
  ...
  it("uses structured instructions when summarizing dropped history chunks", async () => {
    ...
    expect(droppedCall?.customInstructions).toContain(
      "Produce a compact, continuation-oriented summary with these exact section headings:",
    );
  ...
  it("retries when generated summary misses headings even if preserved turns contain them", async () => {
    ...
    mockSummarizeInStages
      ...
      .mockResolvedValueOnce(
        [
          "## Current goal",
          "Preserve the latest ask status.",
          "## Decisions",
          "Keep current flow.",
          "## Completed work",
          "None captured.",
          "## Task status / next step",
          "Continue summarizing the latest ask status.",
          "## Open TODOs",
          "None.",
          "## Constraints/Rules",
          "Follow rules.",
    ...
    expect(secondCall?.customInstructions).toContain("missing_section:## Current goal");
  });
  ...
  it("applies runtime task-status override to previous summary before provider summarization", async () => {
    const providerSummarize = vi.fn().mockResolvedValue("provider summary body");
    registerCompactionProvider({
      id: "test-provider-task-override",
      label: "Test Provider Task Override",
      summarize: providerSummarize,
    });

    const sessionManager = stubSessionManager();
    setCompactionSafeguardRuntime(sessionManager, {
      provider: "test-provider-task-override",
      projectedTaskStatusOverride:
        "Blocked waiting for runtime confirmation.\nRecent outcome: verified the active task.\nNext step: continue with the refreshed task state.",
    });
    ...
    const event = {
      preparation: {
        ...
        previousSummary: [
          ...
          "Use prior state.",
          "## Completed work",
          "Captured earlier context.",
          "## Task status / next step",
          "Stale status from an old summary.",
          "## Open TODOs",
          "None.",
          "## Constraints/Rules",
          "Preserve safety checks.",
    ...
    expect(providerSummarize).toHaveBeenCalledWith(
      expect.objectContaining({
        previousSummary: expect.stringContaining(
          "Blocked waiting for runtime confirmation.\nRecent outcome: verified the active task.\nNext step: continue with the refreshed task state.",
        ),
      }),
    );
    expect(providerSummarize.mock.calls[0]?.[0]?.previousSummary).not.toContain(
      "Stale status from an old summary.",
    );
  });
...
describe("compaction-safeguard double-compaction guard", () => {
  it("cancels compaction when there are no real messages to summarize", async () => {
    ...
    expect(compaction.summary).toContain("## Current goal");
    expect(compaction.summary).toContain("## Task status / next step");

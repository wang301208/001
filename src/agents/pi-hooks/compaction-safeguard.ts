...
import { applyTaskStatusOverrideToSummary } from "../task-status-override.js";
import {
  BASE_CHUNK_RATIO,
  MIN_CHUNK_RATIO,
  SAFETY_MARGIN,
...
const DEFAULT_RECENT_TURNS_PRESERVE = 4;
...
export default function compactionSafeguardExtension(api: ExtensionAPI): void {
  api.on("session_before_compact", async (event, ctx) => {
    ...
    const effectivePreviousSummary = applyTaskStatusOverrideToSummary(
      preparation.previousSummary,
      runtime?.projectedTaskStatusOverride ?? runtime?.taskStatusOverride,
    );
    const projectedTaskStatusAudit = runtime?.projectedTaskStatusAudit;
    const projectedTaskStatusExplanation =
      projectedTaskStatusAudit?.explanation ?? runtime?.projectedTaskStatusExplanation;
    if (log.isEnabled("debug") && projectedTaskStatusExplanation) {
      log.debug(
        `Compaction safeguard continuation projection: consumer=${projectedTaskStatusAudit?.consumer ?? "unknown"} ` +
          `mode=${projectedTaskStatusExplanation.projectionMode} ` +
          `boundary=${projectedTaskStatusExplanation.restoreBoundary} ` +
          `source=${projectedTaskStatusExplanation.source ?? "unknown"} ` +
          `audit=${projectedTaskStatusExplanation.auditCodes.join(",")}`,
      );
    }
    if (!hasRealSummarizable && !hasRealTurnPrefix) {
      ...
      const fallbackSummary = buildStructuredFallbackSummary(effectivePreviousSummary);
      return {
        compaction: {
    ...
    try {
      ...
      const effectivePreviousSummaryForRetry = droppedSummary ?? effectivePreviousSummary;
      ...
      for (let attempt = 0; attempt < totalAttempts; attempt += 1) {
          ...
          historySummary =
            messagesToSummarize.length > 0
              ? await summarizeViaLLM({
                  ...
                  summarizationInstructions,
                  previousSummary: effectivePreviousSummaryForRetry,
                })
              : buildStructuredFallbackSummary(
                  effectivePreviousSummaryForRetry,
                  summarizationInstructions,
                );
        ...
        const qualityFeedbackInstruction =
          identifierPolicy === "strict"
            ? "Fix all issues and include every required section with exact identifiers preserved. Make ## Task status / next step concrete enough to continue from immediately, including blocker or waiting reasons, recent meaningful outcome, and failed or invalidated paths when needed to avoid loops."
            : "Fix all issues and include every required section while following the configured identifier policy. Make ## Task status / next step concrete enough to continue from immediately, including blocker or waiting reasons, recent meaningful outcome, and failed or invalidated paths when needed to avoid loops.";

...
describe("DEFAULT_COMPACTION_INSTRUCTIONS", () => {
  ...
  it("contains continuation-critical content directive", () => {
    expect(DEFAULT_COMPACTION_INSTRUCTIONS).toContain("continuation-critical facts");
  });

  it("contains task continuity directive", () => {
    expect(DEFAULT_COMPACTION_INSTRUCTIONS).toContain("current task status");
    expect(DEFAULT_COMPACTION_INSTRUCTIONS).toContain("current subtask");
    expect(DEFAULT_COMPACTION_INSTRUCTIONS).toContain("recent meaningful outcome");
    expect(DEFAULT_COMPACTION_INSTRUCTIONS).toContain("immediately actionable next step");
    expect(DEFAULT_COMPACTION_INSTRUCTIONS).toContain("blockers, waiting reasons, failed attempts, and invalidated paths");
  });
...
describe("resolveCompactionInstructions", () => {
  ...
  describe("precedence", () => {
    ...
    it("appends continuity hint after resolved instructions", () => {
      const result = resolveCompactionInstructions(undefined, "runtime value", "Task continuity hint");

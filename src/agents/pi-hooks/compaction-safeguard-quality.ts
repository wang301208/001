...
export function buildCompactionStructureInstructions(
  ...
  const sectionsTemplate = [
    ...
    "",
    "Requirements:",
    "- In ## Current goal, state the user's active objective right now.",
    "- In ## Decisions, capture decisions already made.",
    "- In ## Completed work, capture what has already been done or verified.",
    "- In ## Task status / next step, capture the current in-flight task state, current subtask when known, any blocker or waiting reason, the most recent meaningful outcome, and the most important immediately actionable next action to continue from here.",
    "- In ## Task status / next step, include failed attempts or invalidated paths when needed to avoid repeating the wrong next step.",
    "- In ## Open TODOs, capture unfinished work still needed to make progress.",
    "- In ## Constraints/Rules, capture user preferences, safety constraints, and hard requirements.",
    "- In ## Pending user asks, include unresolved asks from the user.",
    identifierSectionInstruction,
    "- Prefer concrete operational facts over general narration.",
...
export function buildStructuredFallbackSummary(
  ...
  const carriedForward =
    trimmedPreviousSummary && !hasRequiredSummarySections(trimmedPreviousSummary)
...
function getSectionBody(summary: string, heading: (typeof REQUIRED_SUMMARY_SECTIONS)[number]): string {
  if (heading === "## Task status / next step") {
    return extractTaskStatusSection(summary) ?? "";
  }
...
export function auditSummaryQuality(params: {
  ...
  const taskStatusBody = getSectionBody(params.summary, "## Task status / next step");
  if (!taskStatusBody) {
    reasons.push("missing_task_status_body");
  } else if (!hasTaskStatusActionabilitySignals(taskStatusBody)) {
    const summaryContinuation = buildTaskContinuationFromSummary(params.summary);
    if (!shouldAcceptTaskStatusWithoutImmediateNextStep(summaryContinuation)) {
      reasons.push("weak_task_status_next_step");
    }
  }

...
function buildOllamaProviderCfg(apiKey: string): AssistantConfig {
  return {
    models: {
      providers: {
        ollama: {
...
describe("getApiKeyForModel", () => {
  it("reads oauth auth-profiles entries from auth-profiles.json via explicit profile", async () => {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "assistant-oauth-"));
    ...
    try {
      ...
      await withEnvAsync(
        ...
        async () => {
          ...
          const store = ensureAuthProfileStore(process.env.ASSISTANT_AGENT_DIR, {
  ...
  it("resolveEnvApiKey('google-vertex') accepts ADC credentials from the provided env snapshot", async () => {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "assistant-google-adc-"));
  ...
  it("resolveEnvApiKey('anthropic-vertex') accepts GOOGLE_APPLICATION_CREDENTIALS with project_id", async () => {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "assistant-adc-"));
    const credentialsPath = path.join(tempDir, "adc.json");

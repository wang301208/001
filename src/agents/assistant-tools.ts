...
import { resolveAssistantPluginToolsForOptions } from "./assistant-plugin-tools.js";
import { applyNodesToolWorkspaceGuard } from "./assistant-tools.nodes-workspace-guard.js";
import {
  collectPresentAssistantTools,
  isUpdatePlanToolEnabledForAssistantTools,
} from "./assistant-tools.registration.js";
import type { SandboxFsBridge } from "./sandbox/fs-bridge.js";
import type { SpawnedToolContext } from "./spawned-context.js";
import type { ToolFsPolicy } from "./tool-fs-policy.js";
import { createAgentsListTool } from "./tools/agents-list-tool.js";
import { createAutonomyTool } from "./tools/autonomy-tool.js";
...
const defaultAssistantToolsDeps: AssistantToolsDeps = {
  callGateway,
};

let assistantToolsDeps: AssistantToolsDeps = defaultAssistantToolsDeps;
...
export function createAssistantTools(
  ...
  const resolvedConfig = options?.config ?? assistantToolsDeps.config;
  ...
  const tools: AnyAgentTool[] = [
    ...
    ...collectPresentAssistantTools([imageGenerateTool, musicGenerateTool, videoGenerateTool]),
    createGatewayTool({
      agentSessionKey: options?.agentSessionKey,
      config: options?.config,
    }),
    ...
    createSessionsListTool({
      ...
      sandboxed: options?.sandboxed,
      config: resolvedConfig,
      callGateway: assistantToolsDeps.callGateway,
    }),
    ...
    ...collectPresentAssistantTools([webSearchTool, webFetchTool, imageTool, pdfTool]),
  ];
  ...
  const wrappedPluginTools = resolveAssistantPluginToolsForOptions({
    options,
    resolvedConfig,
...
export const __testing = {
  setDepsForTest(overrides?: Partial<AssistantToolsDeps>) {
    assistantToolsDeps = overrides
      ? {
          ...defaultAssistantToolsDeps,
          ...overrides,
        }
      : defaultAssistantToolsDeps;
  },

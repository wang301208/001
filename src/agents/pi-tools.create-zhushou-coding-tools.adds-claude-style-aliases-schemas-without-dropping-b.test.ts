...
describe("createZhushouCodingTools", () => {
  const testConfig: ZhushouConfig = {};

  it("preserves action enums in normalized schemas", () => {
    const defaultTools = createZhushouCodingTools({ config: testConfig, senderIsOwner: true });
  ...
  it("enforces apply_patch availability and canonical names across model/provider constraints", () => {
    ...
    const openAiTools = createZhushouCodingTools({
      config: testConfig,
    ...
    const codexTools = createZhushouCodingTools({
      config: testConfig,
    ...
    const disabledOpenAiTools = createZhushouCodingTools({
    ...
    const anthropicTools = createZhushouCodingTools({
      config: disabledConfig,
    ...
    const allowed = createZhushouCodingTools({
    ...
    const denied = createZhushouCodingTools({
      config: allowModelsConfig,
      modelProvider: "openai",
    ...
    const oauthTools = createZhushouCodingTools({
      config: testConfig,
      modelProvider: "anthropic",
      modelAuthMode: "oauth",
    });
  ...
  it("provides top-level object schemas for all tools", () => {
    const tools = createZhushouCodingTools({ config: testConfig });

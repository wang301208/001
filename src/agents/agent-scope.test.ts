...
describe("resolveAgentConfig", () => {
  ...
  it("should return undefined when agent id does not exist", () => {
    const cfg: AssistantConfig = {
      agents: {
        list: [{ id: "main", workspace: "~/assistant" }],
      },
    };
  ...
  it("resolves explicit and effective model primary separately", () => {
    ...
    const cfgWithObjectDefault: AssistantConfig = {
      agents: {
        defaults: {
          model: {
    ...
    const cfgNoDefaults: AssistantConfig = {
      agents: {
        list: [{ id: "main" }],
      },
    };
  ...
  it("supports per-agent model primary+fallbacks", () => {
    ...
    const cfgNoOverride: AssistantConfig = {
      agents: {
        list: [
          {
            id: "linus",
    ...
    const cfgDisable: AssistantConfig = {
      agents: {
        list: [
          {
            id: "linus",
    ...
    const cfgInheritDefaults: AssistantConfig = {
      agents: {
        defaults: {
          model: {
            fallbacks: ["openai/gpt-5.4"],
  ...
  it("computes whether any model fallbacks are configured via shared helper", () => {
    const cfgDefaultsOnly: AssistantConfig = {
      agents: {
        defaults: {
          model: {
            fallbacks: ["openai/gpt-5.4"],
    ...
    const cfgAgentOverrideOnly: AssistantConfig = {
      agents: {
        defaults: {
          model: {
            fallbacks: [],
  ...
  it("uses ASSISTANT_HOME for default agent workspace", () => {
    const home = path.join(path.sep, "srv", "assistant-home");
    vi.stubEnv("ASSISTANT_HOME", home);

    const workspace = resolveAgentWorkspaceDir({} as AssistantConfig, "main");
    expect(workspace).toBe(path.join(path.resolve(home), ".assistant", "workspace"));
  });

  it("uses ASSISTANT_HOME for default agentDir", () => {
    const home = path.join(path.sep, "srv", "assistant-home");
    vi.stubEnv("ASSISTANT_HOME", home);
    // Clear state dir so it falls back to ASSISTANT_HOME
    vi.stubEnv("ASSISTANT_STATE_DIR", "");

    const agentDir = resolveAgentDir({} as AssistantConfig, "main");
    expect(agentDir).toBe(path.join(path.resolve(home), ".assistant", "agents", "main", "agent"));
  });
...
describe("resolveAgentIdByWorkspacePath", () => {
  it("returns the most specific workspace match for a directory", () => {
    const workspaceRoot = `/tmp/assistant-agent-scope-${Date.now()}-root`;
  ...
  it("returns undefined when directory has no matching workspace", () => {
    ...
    expect(
      resolveAgentIdByWorkspacePath(cfg, `/tmp/assistant-agent-scope-${Date.now()}-unrelated`),
    ).toBeUndefined();
  });

  it("matches workspace paths through symlink aliases", () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "assistant-agent-scope-"));

...
describe("resolveDefaultAgentWorkspaceDir", () => {
  it("uses ASSISTANT_HOME for default workspace resolution", () => {
    const dir = resolveDefaultAgentWorkspaceDir({
      ASSISTANT_HOME: "/srv/assistant-home",
      HOME: "/home/other",
    } as NodeJS.ProcessEnv);

    expect(dir).toBe(path.join(path.resolve("/srv/assistant-home"), ".assistant", "workspace"));
  });
});

const WORKSPACE_STATE_PATH_SEGMENTS = [".assistant", "workspace-state.json"] as const;
...
describe("ensureAgentWorkspace", () => {
  it("creates BOOTSTRAP.md and records a seeded marker for brand new workspaces", async () => {
    const tempDir = await makeTempWorkspace("assistant-workspace-");
  ...
  it("migrates legacy onboardingCompletedAt markers to setupCompletedAt", async () => {
    const tempDir = await makeTempWorkspace("assistant-workspace-");
    await fs.mkdir(path.join(tempDir, ".assistant"), { recursive: true });
    await fs.writeFile(
...
describe("loadWorkspaceBootstrapFiles", () => {
  ...
  it("treats hardlinked bootstrap aliases as missing", async () => {
    ...
    const rootDir = await fs.mkdtemp(path.join(os.tmpdir(), "assistant-workspace-hardlink-"));
    try {

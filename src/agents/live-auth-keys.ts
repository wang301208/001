...
const GOOGLE_LIVE_SINGLE_KEY = "ASSISTANT_LIVE_GEMINI_KEY";
...
const PROVIDER_API_KEY_CONFIG: Record<string, Omit<ProviderApiKeyConfig, "fallbackVars">> = {
  anthropic: {
    liveSingle: "ASSISTANT_LIVE_ANTHROPIC_KEY",
    listVar: "ASSISTANT_LIVE_ANTHROPIC_KEYS",
  ...
  openai: {
    liveSingle: "ASSISTANT_LIVE_OPENAI_KEY",
...
function resolveProviderApiKeyConfig(provider: string): ProviderApiKeyConfig {
  ...
  const liveSingle = custom?.liveSingle ?? `ASSISTANT_LIVE_${base}_KEY`;

...
function makeFallbacksOnlyCfg(): AssistantConfig {
  return {
    agents: {
      defaults: {
        model: {
...
function makeProviderFallbackCfg(provider: string): AssistantConfig {
  return makeCfg({
    agents: {
      defaults: {
        model: {
...
describe("runWithModelFallback", () => {
  it("skips auth store bootstrap when no auth profile sources exist", async () => {
    ...
    try {
      const result = await runWithModelFallback({
        ...
        provider: "openai",
        model: "gpt-4.1-mini",
        agentDir: "/tmp/assistant-no-auth-profiles",
        run,
      });

      expect(result.result).toBe("ok");
      expect(hasSourcesSpy).toHaveBeenCalledWith("/tmp/assistant-no-auth-profiles");
  ...
  it("sanitizes model identifiers in model_not_found warnings", async () => {
    const warnLogs = createWarnLogCapture("assistant-model-fallback-test");
    try {
      const cfg = makeCfg();
      const run = vi
        .fn()

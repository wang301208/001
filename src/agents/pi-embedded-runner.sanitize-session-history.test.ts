...
import {
  resolveReplayTaskStatusProjectionDecision,
  validateReplayTurns,
...
async function makeTmpDir(): Promise<string> {
  tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "sanitize-session-history-"));
  return tmpDir;
}
...
describe("sanitizeSessionHistory", () => {
  ...
  it("rewrites latest compaction summary task status from runtime override", async () => {
    vi.mocked(mockedHelpers.isGoogleModelApi).mockReturnValue(false);
    ...
    const result = await sanitizeSessionHistory({
      ...
      projectedTaskStatusOverride:
        "Blocked waiting for runtime confirmation.\nRecent outcome: verified the active task state.\nNext step: continue from runtime truth.",
    });

    const compactionSummary = result.find((message) => message.role === "compactionSummary") as
      | (AgentMessage & { summary?: unknown })
      | undefined;
    expect(typeof compactionSummary?.summary).toBe("string");
    expect(compactionSummary?.summary).toContain(
      "Blocked waiting for runtime confirmation.\nRecent outcome: verified the active task state.\nNext step: continue from runtime truth.",
    );
    expect(compactionSummary?.summary).not.toContain("Stale summary task status.");
  });

  it("maps legacy replay taskStatusOverride onto projected replay override", async () => {
    vi.mocked(mockedHelpers.isGoogleModelApi).mockReturnValue(false);
  ...
  it("resolves replay projection diagnostics from structured continuation state", () => {
    const decision = resolveReplayTaskStatusProjectionDecision({
      messages: castAgentMessages([
        makeCompactionSummaryMessage(
          191_919,
          new Date().toISOString(),
      ...
      projectedTaskStatusOverride: undefined,
    });

    expect(decision.projectedTaskStatusOverride).toBe(
      "Done. Prior task completed.\nRecent outcome: Verified the migration completed successfully.",
    );
  ...
  it("rewrites latest compaction summary task status from structured continuation details when no explicit override is provided", async () => {
    vi.mocked(mockedHelpers.isGoogleModelApi).mockReturnValue(false);
    ...
    const compactionSummary = result.find((message) => message.role === "compactionSummary") as
      ...
      | undefined;
    expect(typeof compactionSummary?.summary).toBe("string");
    expect(compactionSummary?.summary).toContain("Blocked waiting for transcript restore.");
    expect(compactionSummary?.summary).toContain(
  ...
  it("does not rewrite latest compaction summary task status from weak continuation details when no explicit override is provided", async () => {
    vi.mocked(mockedHelpers.isGoogleModelApi).mockReturnValue(false);
    ...
    const compactionSummary = result.find((message) => message.role === "compactionSummary") as
      ...
      | undefined;
    expect(typeof compactionSummary?.summary).toBe("string");
    expect(compactionSummary?.summary).toContain("Stale summary task status.");
    expect(compactionSummary?.summary).not.toContain("Recent outcome:");
  });

  it("rewrites done continuation details without projecting stale next-step text into replay override", async () => {
    vi.mocked(mockedHelpers.isGoogleModelApi).mockReturnValue(false);
    ...
    const compactionSummary = result.find((message) => message.role === "compactionSummary") as
      ...
      | undefined;
    expect(typeof compactionSummary?.summary).toBe("string");
    expect(compactionSummary?.summary).toContain("Done. Prior task completed.");
    expect(compactionSummary?.summary).toContain(
      "Recent outcome: Verified the migration completed successfully.",
    );
    expect(compactionSummary?.summary).not.toContain(
      "Next step: Open a follow-up task if needed.",
    );
    expect(compactionSummary?.summary).not.toContain("Stale summary task status.");
  });

  it("applies runtime override after buildSessionContext reconstructs compaction summary messages", async () => {
    vi.mocked(mockedHelpers.isGoogleModelApi).mockReturnValue(false);
    ...
    const result = await sanitizeSessionHistory({
      ...
      modelApi: "openai-responses",
      provider: "openai",
      sessionManager: reopened as never,
      sessionId: TEST_SESSION_ID,
      projectedTaskStatusOverride:
        "Blocked waiting for runtime confirmation.\nRecent outcome: rebuilt from session entries.\nNext step: continue from runtime truth.",
    });

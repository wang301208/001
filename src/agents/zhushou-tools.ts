...
import { resolveZhushouPluginToolsForOptions } from "./zhushou-plugin-tools.js";
import { applyNodesToolWorkspaceGuard } from "./zhushou-tools.nodes-workspace-guard.js";
import {
  collectPresentZhushouTools,
  isUpdatePlanToolEnabledForZhushouTools,
} from "./zhushou-tools.registration.js";
...
type ZhushouToolsDeps = {
  callGateway: typeof callGateway;
  config?: ZhushouConfig;
};

const defaultZhushouToolsDeps: ZhushouToolsDeps = {
  callGateway,
};

let zhushouToolsDeps: ZhushouToolsDeps = defaultZhushouToolsDeps;

export function createZhushouTools(
  options?: {
  ...
  const resolvedConfig = options?.config ?? zhushouToolsDeps.config;
  ...
  const tools: AnyAgentTool[] = [
    ...
    ...collectPresentZhushouTools([imageGenerateTool, musicGenerateTool, videoGenerateTool]),
    createGatewayTool({
      agentSessionKey: options?.agentSessionKey,
      config: options?.config,
    }),
    ...
    createSessionsListTool({
      ...
      sandboxed: options?.sandboxed,
      config: resolvedConfig,
      callGateway: zhushouToolsDeps.callGateway,
    }),
    ...
    ...collectPresentZhushouTools([webSearchTool, webFetchTool, imageTool, pdfTool]),
  ];
  ...
  const wrappedPluginTools = resolveZhushouPluginToolsForOptions({
    options,
    resolvedConfig,
...
export const __testing = {
  setDepsForTest(overrides?: Partial<ZhushouToolsDeps>) {
    zhushouToolsDeps = overrides
      ? {
          ...defaultZhushouToolsDeps,
          ...overrides,
        }
      : defaultZhushouToolsDeps;
  },

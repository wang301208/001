...
let createZhushouCodingTools: typeof import("./pi-tools.js").createZhushouCodingTools;
...
beforeAll(async () => {
  await withEnvAsync(
    {
      ZHUSHOU_BUNDLED_PLUGINS_DIR: path.join(os.tmpdir(), "zhushou-test-no-bundled-extensions"),
    },
    async () => {
      ({ createZhushouCodingTools } = await import("./pi-tools.js"));
    },
  );
});
...
describe("createZhushouCodingTools safeBins", () => {

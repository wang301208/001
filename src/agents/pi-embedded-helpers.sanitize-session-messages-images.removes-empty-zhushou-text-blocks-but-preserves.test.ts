...
function makeEmptyZhushouErrorMessage(): AssistantMessage {
...
function makeOpenAiResponsesZhushouMessage(
...
function expectToolCallAndResultIds(out: AgentMessage[], expectedId: string) {
  const zhushou = out[0];
  expect(zhushou.role).toBe("assistant");
  const zhushouContent = zhushou.role === "assistant" ? zhushou.content : [];
  const toolCall = zhushouContent.find((block) => block.type === "toolCall");
...
function expectSingleZhushouContentEntry(
  out: AgentMessage[],
...
describe("sanitizeSessionMessagesImages", () => {
  ...
  it("does not synthesize tool call input when missing", async () => {
    const input = castAgentMessages([
      makeOpenAiResponsesZhushouMessage([
        { type: "toolCall", id: "call_1", name: "read", arguments: {} },
      ]),
    ]);

    const out = await sanitizeSessionMessagesImages(input, "test");
    const zhushou = out[0] as { content?: Array<Record<string, unknown>> };
    const toolCall = zhushou.content?.find((b) => b.type === "toolCall");
    expect(toolCall).toBeTruthy();
    expect("input" in (toolCall ?? {})).toBe(false);
  });

  it("removes empty zhushou text blocks but preserves tool calls", async () => {
    const input = castAgentMessages([
      makeOpenAiResponsesZhushouMessage([
        { type: "text", text: "" },
        { type: "toolCall", id: "call_1", name: "read", arguments: {} },
    ...
    expectSingleZhushouContentEntry(out, (entry) => {
  ...
  it("sanitizes tool ids in strict mode (alphanumeric only)", async () => {
    ...
    // Strict mode strips all non-alphanumeric characters
    const zhushou = out[0] as { content?: Array<{ id?: string }> };
    expect(zhushou.content?.[0]?.id).toBe("callabcitem123");
    expect(zhushou.content?.[1]?.id).toBe("callabcitem456");
  ...
  it("sanitizes tool IDs in images-only mode when explicitly enabled", async () => {
    ...
    const zhushou = out[0];
    const toolCall =
      zhushou?.role === "assistant"
        ? zhushou.content.find((b) => b.type === "toolCall")
        : undefined;
  ...
  it("filters whitespace-only zhushou text blocks", async () => {
    const input = castAgentMessages([
      {
        role: "assistant",
        content: [
  ...
  it("drops zhushou messages that only contain empty text", async () => {
    const input = castAgentMessages([
  ...
  it("keeps empty zhushou error messages", async () => {
    const input = castAgentMessages([
      { role: "user", content: "hello", timestamp: nextTimestamp() } satisfies UserMessage,
      {
        ...makeEmptyZhushouErrorMessage(),
      },
      {
        ...makeEmptyZhushouErrorMessage(),
      },
  ...
  it("leaves non-zhushou messages unchanged", async () => {
    const input = [
      { role: "user", content: "hello", timestamp: nextTimestamp() } satisfies UserMessage,
      {
        role: "toolResult",
  ...
  describe("thought_signature stripping", () => {
    it("strips msg_-prefixed thought_signature from zhushou message content blocks", async () => {
      const input = castAgentMessages([
        {
          role: "assistant",
          content: [
...
describe("sanitizeGoogleTurnOrdering", () => {
  it("prepends a synthetic user turn when history starts with zhushou", () => {
    const input = castAgentMessages([
      {
        role: "assistant",

...
export function resolveDefaultAgentWorkspaceDir(
  ...
  const profile = env.ZHUSHOU_PROFILE?.trim();
  if (profile && normalizeOptionalLowercaseString(profile) !== "default") {
    return path.join(home, ".zhushou", `workspace-${profile}`);
  }
  return path.join(home, ".zhushou", "workspace");
}
...
const WORKSPACE_STATE_DIRNAME = ".zhushou";
...
Role: Repository-aware autonomous engineering zhushou
`,

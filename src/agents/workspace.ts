...
export function resolveDefaultAgentWorkspaceDir(
  ...
  const profile = env.ASSISTANT_PROFILE?.trim();
  if (profile && normalizeOptionalLowercaseString(profile) !== "default") {
    return path.join(home, ".assistant", `workspace-${profile}`);
  }
  return path.join(home, ".assistant", "workspace");
}
...
const WORKSPACE_STATE_DIRNAME = ".assistant";

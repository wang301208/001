...
describe("host tool tilde expansion (non-workspace mode)", () => {
  ...
  it("ignores ZHUSHOU_HOME for write operations", async () => {
    const zhushouHome = await createTempDir("zhushou-home-override-", os.tmpdir());
    const dir = await createTempDir("zhushou-tilde-test-write-");
    const testFile = path.join(dir, "os-home-write.txt");
    vi.stubEnv("ZHUSHOU_HOME", zhushouHome);

    createHostWorkspaceWriteTool(zhushouHome, { workspaceOnly: false });
    ...
    await expect(fs.access(path.join(zhushouHome, path.basename(testFile)))).rejects.toBeDefined();
  });
  ...
  it("ignores ZHUSHOU_HOME for mkdir operations", async () => {
    ...
    expect((await fs.stat(newDir)).isDirectory()).toBe(true);
    await expect(fs.access(path.join(zhushouHome, path.basename(newDir)))).rejects.toBeDefined();
  });
  ...
  it("ignores ZHUSHOU_HOME for readFile operations", async () => {
    ...
    createHostWorkspaceEditTool(zhushouHome, { workspaceOnly: false });
    const content = await mocks.editOps!.readFile(toTildePath(testFile));

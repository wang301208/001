...
const SOFT_COMPACTION_THRESHOLD_RATIO = 0.5;
const SOFT_COMPACTION_MIN_MESSAGES = 40;
...
export function shouldPreemptivelyCompactBeforePrompt(params: {
  ...
  softCompactRecommended: boolean;
  ...
  const softCompactionThreshold = Math.max(
    1,
    Math.floor(contextTokenBudget * SOFT_COMPACTION_THRESHOLD_RATIO),
  );
  const softCompactRecommended =
    estimatedPromptTokens >= softCompactionThreshold &&
    params.messages.length >= SOFT_COMPACTION_MIN_MESSAGES;

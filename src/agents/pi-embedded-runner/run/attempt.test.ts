...
describe("wrapStreamFnTrimToolCallNames", () => {
  ...
  it("rewrites repeated unavailable tool calls into plain zhushou text after the threshold", async () => {
    const baseFn = vi.fn(() =>
      createFakeStream({
        events: [],
        resultMessage: {
...
describe("wrapStreamFnSanitizeMalformedToolCalls", () => {
  it("drops malformed zhushou tool calls from outbound context before provider replay", async () => {
    const messages = [
      {
        role: "assistant",
        stopReason: "error",
  ...
  it("preserves outbound context when all zhushou tool calls are valid", async () => {
    const messages = [
      {
        role: "assistant",
  ...
  it("preserves matching tool results for retained errored zhushou turns", async () => {
    const messages = [
      {
        role: "assistant",
        stopReason: "error",
  ...
  it("revalidates turn ordering after dropping an zhushou replay turn", async () => {
    const messages = [
      {
        role: "user",
  ...
  it("drops orphaned Anthropic user tool_result blocks after dropping an zhushou replay turn", async () => {
    const messages = [
      {
        role: "user",
        content: [{ type: "text", text: "first" }],
...
describe("buildAfterTurnRuntimeContext", () => {
  ...
  it("includes resolved auth profile fields for context-engine afterTurn compaction", () => {
    ...
    const legacy = buildAfterTurnRuntimeContext({
      attempt: {
        ...
        agentAccountId: "acct-1",
        authProfileId: "openai:p1",
        config: { plugins: { slots: { contextEngine: "lossless-claw" } } } as ZhushouConfig,
        skillsSnapshot: undefined,
        senderIsOwner: true,
        provider: "openai-codex",
        modelId: "gpt-5.4",
  ...
  it("derives afterTurn token count from the current zhushou usage snapshot", () => {
    const lastCallUsage = {
      input: 10,
      output: 5,
      cacheRead: 40,

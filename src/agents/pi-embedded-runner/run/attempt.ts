...
export async function runEmbeddedAttempt(
  ...
  try {
    ...
    try {
      ...
      const sessionRow = params.sessionKey ? loadGatewaySessionRow(params.sessionKey) : null;
      const checkpointContinuation = sessionRow?.latestCompactionCheckpoint?.continuation as
        | SessionTaskContinuation
        | undefined;
      const continuationState = resolveContinuationState({
        sessionKey: params.sessionKey,
      ...
      try {
        ...
        try {
          ...
          const shouldSoftCompact =
            preemptiveCompaction.route === "fits" && preemptiveCompaction.softCompactRecommended;
          if (preemptiveCompaction.shouldCompact || shouldSoftCompact) {
            preflightRecovery =
            ...
            log.warn(
              `[context-overflow-precheck] sessionKey=${params.sessionKey ?? params.sessionId} ` +
                ...
                `route=${shouldSoftCompact ? "soft_compact" : preemptiveCompaction.route} ` +
                `estimatedPromptTokens=${preemptiveCompaction.estimatedPromptTokens} ` +
                `promptBudgetBeforeReserve=${preemptiveCompaction.promptBudgetBeforeReserve} ` +
                `overflowTokens=${preemptiveCompaction.overflowTokens} ` +
                `toolResultReducibleChars=${preemptiveCompaction.toolResultReducibleChars} ` +
                ...
                `effectiveReserveTokens=${preemptiveCompaction.effectiveReserveTokens} ` +
                `softCompactRecommended=${preemptiveCompaction.softCompactRecommended} ` +
                `sessionFile=${params.sessionFile}`,
            );
            skipPromptSubmission = true;
          }

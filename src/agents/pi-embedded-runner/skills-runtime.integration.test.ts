...
async function setupBundledDiffsPlugin() {
  const bundledPluginsDir = await createTempDir("zhushou-bundled-");
  const workspaceDir = await createTempDir("zhushou-workspace-");
...
async function resolveBundledDiffsSkillEntries(config?: ZhushouConfig) {

...
async function setupBundledDiffsPlugin() {
  const bundledPluginsDir = await createTempDir("assistant-bundled-");
  const workspaceDir = await createTempDir("assistant-workspace-");
...
async function resolveBundledDiffsSkillEntries(config?: AssistantConfig) {

...
import {
  ...
  buildTaskContinuationFromSession,
...
import { createZhushouCodingTools } from "../pi-tools.js";
...
export async function compactEmbeddedPiSessionDirect(
  ...
  try {
    ...
    const toolsRaw = createZhushouCodingTools({
      exec: {
        ...
        try {
          ...
          let postCompactionLeafId =
            typeof sessionManager.getLeafId === "function"
              ...
              : undefined;
          const continuation = buildTaskContinuationFromSession({
            sessionKey: params.sessionKey,
          });
          const continuationState = buildContinuationState({
            continuation,
            source: continuation ? "active-task" : undefined,
          });
          const continuationAudit = buildCompactionContinuationAuditPayload(
            continuationState,
          );
          const projectedTaskStatusOverride =
            continuationAudit.projectedTaskStatusOverride;
          try {
            const normalizedCompaction = await normalizeLatestCompactionEntry({
              sessionFile: params.sessionFile,
              taskStatusOverride: projectedTaskStatusOverride,
              continuation,
              continuationState,
            });
            if (normalizedCompaction.applied) {
              effectiveFirstKeptEntryId =
                normalizedCompaction.firstKeptEntryId ?? effectiveFirstKeptEntryId;
              postCompactionLeafId = normalizedCompaction.leafId ?? postCompactionLeafId;
              session.agent.state.messages = normalizedCompaction.messages;
            }
          } catch (err) {
            log.warn("[compaction] failed to normalize latest compaction entry", {

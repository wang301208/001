...
export function resolveReplayTaskStatusProjectionDecision(params: {
  messages: AgentMessage[];
  projectedTaskStatusOverride: string | undefined;
}): {
  projectedTaskStatusOverride?: string;
  explanation?: import("../task-status-override.js").ContinuationDecisionExplanation;
  auditPayload?: import("../task-status-override.js").ContinuationAuditPayload;
} {
  const explicitOverride = params.projectedTaskStatusOverride?.trim();
  if (explicitOverride) {
    return {
      projectedTaskStatusOverride: explicitOverride,
    };
  }
  const continuationState = resolveContinuationState({ messages: params.messages });
  const auditPayload = buildReplayContinuationAuditPayload(continuationState);
  return {
    projectedTaskStatusOverride: auditPayload.projectedTaskStatusOverride,
    explanation: auditPayload.explanation,
    auditPayload,
  };
}

function applyTaskStatusOverrideToLatestCompactionSummary(
  messages: AgentMessage[],
  projectedTaskStatusOverride: string | undefined,
): AgentMessage[] {
  const override = resolveReplayTaskStatusProjectionDecision({
    messages,
    projectedTaskStatusOverride,
  }).projectedTaskStatusOverride?.trim();
  if (!override) {
    return messages;
  }
  for (let i = messages.length - 1; i >= 0; i -= 1) {
    const entry = messages[i] as
      | (AgentMessage & { role?: unknown; summary?: unknown })
      | undefined;
    if (entry?.role !== "compactionSummary" || typeof entry.summary !== "string") {
      continue;
    }
    const nextSummary = applyTaskStatusOverrideToSummary(entry.summary, override);
    if (!nextSummary || nextSummary === entry.summary) {
      return messages;
    }
    const out = [...messages];
    out[i] = {
...
export async function sanitizeSessionHistory(params: {
  ...
  /** Preferred consumer-specific override for replay compaction-summary projection. */
  projectedTaskStatusOverride?: string;
  ...
  const withInterSessionMarkers = annotateInterSessionUserMessages(
    applyTaskStatusOverrideToLatestCompactionSummary(
      params.messages,

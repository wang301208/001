...
const findLatestTaskFlowForOwnerMock = vi.hoisted(() =>
  vi.fn((_params: { callerOwnerKey: string }) => undefined as { currentStep?: string } | undefined),
);
...
function createCommandsStatusRuntimeModuleMock() {
  return {
    buildStatusText: async (params: {
      ...
      return ["助手", `🧠 Model: ${primary}`, params.taskLineOverride]
        .filter(Boolean)
        .join("\n");
    },
  };
...
vi.mock("../tasks/task-flow-owner-access.js", () => ({
  findLatestTaskFlowForOwner: (params: { callerOwnerKey: string }) =>
    findLatestTaskFlowForOwnerMock(params),
}));
...
function resetSessionStore(store: Record<string, SessionEntry>) {
  ...
  findLatestTaskFlowForOwnerMock.mockClear();
  findLatestTaskFlowForOwnerMock.mockReturnValue(undefined);
...
describe("session_status tool", () => {
  ...
  it("returns a status card for the current session", async () => {
    ...
    expect(details.statusText).toContain("助手");
  ...
  it("prefers flow currentStep over raw task progress in session_status output", async () => {
    resetSessionStore({
      "agent:main:main": {
        sessionId: "sess-main",
        updatedAt: Date.now(),
    ...
    findLatestTaskFlowForOwnerMock.mockReturnValue({
      currentStep: "Reviewing the newest two threads",
    });

    const tool = createSessionStatusTool({ agentSessionKey: "agent:main:main" });
    const result = await tool.execute("tc-flow-task-line", { sessionKey: "agent:main:main" });
    ...
    expect(text).toContain("Reviewing the newest two threads");
    expect(text).not.toContain("Indexing the latest threads");
  });

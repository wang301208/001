...
describe("buildAgentSystemPrompt", () => {
  ...
  it("avoids the Claude subscription classifier wording in reply tag guidance", () => {
    ...
    expect(prompt).toContain("## Zhushou Output Directives");
  ...
  it("includes a CLI quick reference section", () => {
    ...
    expect(prompt).toContain("## 助手 CLI Quick Reference");
    expect(prompt).toContain("zhushou gateway restart");
  ...
  it("guides runtime completion events without exposing internal metadata", () => {
    ...
    expect(prompt).toContain("Rewrite those in your normal zhushou voice");
  ...
  it("includes embed guidance only for webchat sessions", () => {
    const prompt = buildAgentSystemPrompt({
      ...
      runtimeInfo: {
        channel: "webchat",
        canvasRootDir: "/Users/example/.zhushou-dev/canvas",
      },
    });
    ...
    expect(prompt).toContain(
      '[embed url="/__zhushou__/canvas/documents/cv_123/index.html" title="Status" height="320" /]',
    );
    expect(prompt).toContain(
      "Never use local filesystem paths or `file://...` URLs in `[embed ...]`.",
    );
    expect(prompt).toContain(
      "The active hosted embed root for this session is: `/Users/example/.zhushou-dev/canvas`.",
    );
  ...
  it("omits ACP harness guidance when ACP is disabled", () => {
    ...
    expect(prompt).toContain("- agents_list: List 助手 agent ids allowed for sessions_spawn");
  });
  ...
  it("preserves tool casing in the prompt", () => {
    ...
    expect(prompt).toContain("助手 docs: /tmp/zhushou/docs");
    expect(prompt).toContain(
  ...
  it("adds ClaudeBot self-update guidance when gateway tool is available", () => {
    ...
    expect(prompt).toContain("## 助手 Self-Update");
  ...
  it("describes sandboxed runtime and elevated when allowed", () => {
    ...
    expect(prompt).toContain(
      "For read/write/edit/apply_patch, file paths resolve against host workspace: /tmp/zhushou. For bash/exec commands, use sandbox container paths under /workspace (or relative paths from that workdir), not host paths.",
    );
  ...
  it("surfaces governance freeze in sandbox prompts and suppresses elevation guidance", () => {
    ...
    expect(prompt).toContain(
      "Governance freeze is active. Use read-only file inspection against host workspace: /tmp/zhushou.",
    );
...
describe("buildSubagentSystemPrompt", () => {
  it("renders depth-1 orchestrator guidance, labels, and recovery notes", () => {
    ...
    expect(prompt).toContain("Do not use `exec` (`zhushou ...`, `acpx ...`)");
    expect(prompt).toContain("Use `subagents` only for 助手 subagents");

...
describe("buildAgentSystemPrompt", () => {
  ...
  it("includes a CLI quick reference section", () => {
    ...
    expect(prompt).toContain("assistant gateway restart");
  ...
  it("includes embed guidance only for webchat sessions", () => {
    const prompt = buildAgentSystemPrompt({
      ...
      runtimeInfo: {
        channel: "webchat",
        canvasRootDir: "/Users/example/.assistant-dev/canvas",
      },
    });
    ...
    expect(prompt).toContain(
      '[embed url="/__assistant__/canvas/documents/cv_123/index.html" title="Status" height="320" /]',
    );
    expect(prompt).toContain(
      "Never use local filesystem paths or `file://...` URLs in `[embed ...]`.",
    );
    expect(prompt).toContain(
      "The active hosted embed root for this session is: `/Users/example/.assistant-dev/canvas`.",
    );
  ...
  it("preserves tool casing in the prompt", () => {
    ...
    expect(prompt).toContain("助手 docs: /tmp/assistant/docs");
    expect(prompt).toContain(
  ...
  it("describes sandboxed runtime and elevated when allowed", () => {
    ...
    expect(prompt).toContain(
      "For read/write/edit/apply_patch, file paths resolve against host workspace: /tmp/assistant. For bash/exec commands, use sandbox container paths under /workspace (or relative paths from that workdir), not host paths.",
    );
  ...
  it("surfaces governance freeze in sandbox prompts and suppresses elevation guidance", () => {
    ...
    expect(prompt).toContain(
      "Governance freeze is active. Use read-only file inspection against host workspace: /tmp/assistant.",
    );
...
describe("buildSubagentSystemPrompt", () => {
  it("renders depth-1 orchestrator guidance, labels, and recovery notes", () => {
    ...
    expect(prompt).toContain("Do not use `exec` (`assistant ...`, `acpx ...`)");

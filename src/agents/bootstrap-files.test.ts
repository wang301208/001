...
describe("resolveBootstrapFilesForRun", () => {
  ...
  it("applies bootstrap hook overrides", async () => {
    ...
    const workspaceDir = await makeTempWorkspace("zhushou-bootstrap-");
...
describe("hasCompletedBootstrapTurn", () => {
  ...
  beforeEach(async () => {
    tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "zhushou-bootstrap-turn-"));
  });
  ...
  it("returns false when no zhushou turn has been flushed yet", async () => {
  ...
  it("returns false for zhushou turns without a recorded full bootstrap marker", async () => {
    const sessionFile = path.join(tmpDir, "zhushou-no-marker.jsonl");
    await fs.writeFile(
      sessionFile,
      [
        JSON.stringify({ type: "session", id: "s1" }),
  ...
  it("returns false when meaningful continuity compaction happened after the last zhushou turn", async () => {
      ...
      [
        ...
        JSON.stringify({
          ...
          summary: [
            ...
            "## Decisions",
            "Use the compaction boundary.",
            "## Completed work",
            "Captured the latest state.",
            "## Task status / next step",
            "Blocked waiting for confirmation. Next step: refresh runtime state before resuming.",
          ].join("\n"),
        }),
      ].join("\n") + "\n",
      "utf8",
  ...
  it("does not invalidate bootstrap completion for weak compaction summaries", async () => {
    const sessionFile = path.join(tmpDir, "weak-post-compaction.jsonl");
    await fs.writeFile(
      sessionFile,
      [
        JSON.stringify({
  ...
  it("invalidates bootstrap completion when checkpoint continuation is available even if transcript summary is weak", async () => {
    const sessionFile = path.join(tmpDir, "weak-post-compaction-with-checkpoint.jsonl");
    await fs.writeFile(
      sessionFile,
      [
        JSON.stringify({
    ...
    expect(
      await hasCompletedBootstrapTurn(sessionFile, {
        statusText: "Blocked waiting for runtime refresh.",
        nextStep: "Resume from latest checkpoint continuation.",
      }),
    ).toBe(false);
  });

  it("treats blocked-like checkpoint continuation as an intentional bootstrap invalidation signal", async () => {
    const sessionFile = path.join(tmpDir, "weak-post-compaction-with-blocked-checkpoint.jsonl");
    await fs.writeFile(
      sessionFile,
      [
        JSON.stringify({
  ...
  it("invalidates bootstrap completion when compaction details carry structured continuation even if transcript summary is weak", async () => {
    const sessionFile = path.join(tmpDir, "weak-post-compaction-with-details-continuation.jsonl");
    await fs.writeFile(
      sessionFile,
      [
        JSON.stringify({
        ...
        JSON.stringify({
          ...
          details: {
            continuation: {
              ...
              nextStep: "Resume from compaction entry details.",
            },
          },
        }),
      ].join("\n") + "\n",
  ...
  it("treats blocked-like compaction details continuation as an intentional bootstrap invalidation signal", async () => {
    const sessionFile = path.join(tmpDir, "weak-post-compaction-with-failed-details-continuation.jsonl");
    await fs.writeFile(
      sessionFile,
      [
        JSON.stringify({
        ...
        JSON.stringify({
          ...
          details: {
            continuation: {
              statusText: "Failed due to stale restore state.",
            },
          },
        }),
      ].join("\n") + "\n",
  ...
  it("returns true when a later full bootstrap marker happens after compaction", async () => {
    const sessionFile = path.join(tmpDir, "zhushou-after-compaction.jsonl");
    await fs.writeFile(
      sessionFile,
      [
        JSON.stringify({

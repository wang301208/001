...
async function resolveAllowedTmpMediaPath(params: {
  ...
  const zhushouTmpDir = path.resolve(resolvePreferredZhushouTmpDir());
  if (!isPathInside(zhushouTmpDir, resolved)) {
    return undefined;
  }
  await assertNoTmpAliasEscape({ filePath: resolved, tmpRoot: zhushouTmpDir });
  return resolved;
}

...
async function expectExecStartWithoutEnvironment(envFileLine: string) {
  mockReadGatewayServiceFile(["[Service]", "ExecStart=/usr/bin/zhushou gateway run", envFileLine]);
...
describe("resolveSystemdUserUnitPath", () => {
  it.each([
    ...
    {
      name: "prefers ZHUSHOU_SYSTEMD_UNIT over ZHUSHOU_PROFILE",
      env: {
        HOME: "/home/test",
        ZHUSHOU_PROFILE: "jbphoenix",
        ZHUSHOU_SYSTEMD_UNIT: "custom-unit",
      },
      expected: "/home/test/.config/systemd/user/custom-unit.service",
    },
    {
      name: "handles ZHUSHOU_SYSTEMD_UNIT with .service suffix",
      env: {
        HOME: "/home/test",
        ZHUSHOU_SYSTEMD_UNIT: "custom-unit.service",
      },
      expected: "/home/test/.config/systemd/user/custom-unit.service",
    },
    {
      name: "trims whitespace from ZHUSHOU_SYSTEMD_UNIT",
      env: {
        HOME: "/home/test",
        ZHUSHOU_SYSTEMD_UNIT: "  custom-unit  ",
      },
...
describe("splitArgsPreservingQuotes", () => {
  it("splits on whitespace outside quotes", () => {
    expect(splitArgsPreservingQuotes('/usr/bin/zhushou gateway start --name "My Bot"')).toEqual([
      "/usr/bin/zhushou",
      "gateway",
      "start",
      "--name",
...
describe("parseSystemdExecStart", () => {
  it("preserves quoted arguments", () => {
    const execStart = '/usr/bin/zhushou gateway start --name "My Bot"';
...
describe("readSystemdServiceExecStart", () => {
  ...
  it("loads ZHUSHOU_GATEWAY_TOKEN from EnvironmentFile", async () => {
    const readFileSpy = mockReadGatewayServiceFile(
      ["[Service]", "ExecStart=/usr/bin/zhushou gateway run", "EnvironmentFile=%h/.zhushou/.env"],
  ...
  it("lets EnvironmentFile override inline Environment values", async () => {
    mockReadGatewayServiceFile(
      [
        "[Service]",
        "ExecStart=/usr/bin/zhushou gateway run",
...
describe("stageSystemdService", () => {
  ...
  it("writes dotenv-backed values to a separate env file and keeps inline env minimal", async () => {
    ...
    const env = {
      ...
      ZHUSHOU_SYSTEMD_UNIT: "zhushou-gateway-stage-test",
    };

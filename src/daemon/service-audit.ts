...
function auditGatewayToken(
  ...
  issues.push({
    ...
    message: "Gateway service embeds ZHUSHOU_GATEWAY_TOKEN and should be reinstalled.",
    detail: "Run `zhushou gateway install --force` to remove embedded service token.",
    level: "recommended",
  });
  ...
  issues.push({
    ...
    message:
      "Gateway service ZHUSHOU_GATEWAY_TOKEN does not match gateway.auth.token in zhushou.json",
...
export function readEmbeddedGatewayToken(command: GatewayServiceCommand): string | undefined {
  ...
  if (command.environmentValueSources?.ZHUSHOU_GATEWAY_TOKEN === "file") {
    return undefined;
  }
  return normalizeOptionalString(command.environment?.ZHUSHOU_GATEWAY_TOKEN);
}
...
export function checkTokenDrift(params: {
  ...
  if (configToken && serviceToken !== configToken) {
    return {
      ...
      detail: "Run `zhushou gateway install --force` to sync the token.",
      level: "recommended",
    };
  }

...
export async function promptSingleChannelSecretInput(params: {
  ...
  const selectedMode = await resolveSecretInputModeForEnvSelection({
    ...
    copy: {
      ...
      plaintextHint: "Stores the credential directly in the new zhushou config",
  ...
  const resolved = await promptSecretRefForSetup({
    ...
    copy: {
      ...
      envVarPlaceholder: params.preferredEnvVar ?? "ZHUSHOU_SECRET",
      envVarFormatError:
        'Use an env var name like "ZHUSHOU_SECRET" (uppercase letters, numbers, underscores).',
      noProvidersMessage:

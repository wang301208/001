...
describe("abort detection", () => {
  ...
  async function createAbortConfig(params?: {
    ...
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "zhushou-abort-"));
  ...
  it("isAbortRequestText aligns abort command semantics", () => {
    ...
    expect(isAbortRequestText("stop zhushou!!!")).toBe(true);
    ...
    expect(isAbortRequestText("/stop@zhushou_bot", { botUsername: "zhushou_bot" })).toBe(true);
    expect(isAbortRequestText("/Stop@zhushou_bot", { botUsername: "zhushou_bot" })).toBe(true);

...
describe("abort detection", () => {
  ...
  async function createAbortConfig(params?: {
    ...
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "assistant-abort-"));
  ...
  it("isAbortRequestText aligns abort command semantics", () => {
    ...
    expect(isAbortRequestText("stop assistant!!!")).toBe(true);
    ...
    expect(isAbortRequestText("/stop@assistant_bot", { botUsername: "assistant_bot" })).toBe(true);
    expect(isAbortRequestText("/Stop@assistant_bot", { botUsername: "assistant_bot" })).toBe(true);

...
async function withTempConfigPath<T>(
  ...
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "zhushou-allowlist-config-"));
  const configPath = path.join(dir, "zhushou.json");
  const previous = process.env.ZHUSHOU_CONFIG_PATH;
  ...
  try {
    ...
    if (previous === undefined) {
      ...
      process.env.ZHUSHOU_CONFIG_PATH = previous;
    }
        ...
        run: async () => {
          await withTempConfigPath(
            ...
            async (configPath) => {
              ...
              const written = await readJsonFile<ZhushouConfig>(configPath);

...
export function resolveAcpInstallCommandHint(cfg: AssistantConfig): string {
  ...
  if (backendId === "acpx") {
    ...
    if (existsSync(workspaceLocalPath)) {
      return `assistant plugins install ${workspaceLocalPath}`;
    }
    ...
    if (bundledInstallHint) {
      const localPath = bundledInstallHint.replace(/^assistant plugins install /u, "");
    ...
    return "assistant plugins install acpx";
  }

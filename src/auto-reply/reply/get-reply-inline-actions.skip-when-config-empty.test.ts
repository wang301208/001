...
const { buildStatusReplyMock, createZhushouToolsMock, getChannelPluginMock, handleCommandsMock } =
  vi.hoisted(() => ({
    buildStatusReplyMock: vi.fn(),
    createZhushouToolsMock: vi.fn(),
...
vi.mock("../../agents/zhushou-tools.runtime.js", () => ({
  createZhushouTools: (...args: unknown[]) => createZhushouToolsMock(...args),
}));
...
describe("handleInlineActions", () => {
  ...
  it("passes requesterAgentIdOverride into inline tool runtimes", async () => {
    ...
    expect(result).toEqual({ kind: "reply", reply: { text: "✅ Done." } });
    expect(createZhushouToolsMock).toHaveBeenCalledWith(
      expect.objectContaining({

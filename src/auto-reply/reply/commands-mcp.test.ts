...
const workspaceHarness = createCommandWorkspaceHarness("assistant-command-mcp-");
...
function buildCfg(): AssistantConfig {
  return {
    commands: {
      text: true,
      mcp: true,
...
describe("handleCommands /mcp", () => {
  ...
  it("writes MCP config and shows it back", async () => {
    await withTempHome("assistant-command-mcp-home-", async () => {

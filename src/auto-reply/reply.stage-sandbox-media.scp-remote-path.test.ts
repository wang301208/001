...
function createRemoteStageParams(home: string): {
  ...
  return {
    ...
    workspaceDir: join(home, "assistant"),
    sessionKey,
    remoteCacheDir: join(home, ".assistant", "media", "remote-cache", encodeURIComponent(sessionKey)),
  };
}
...
describe("stageSandboxMedia scp remote paths", () => {
  it("rejects remote attachment filenames with shell metacharacters before spawning scp", async () => {
    await withSandboxMediaTempHome("assistant-triggers-", async (home) => {

...
function createRemoteStageParams(home: string): {
  ...
  return {
    ...
    workspaceDir: join(home, "zhushou"),
    sessionKey,
    remoteCacheDir: join(home, ".zhushou", "media", "remote-cache", encodeURIComponent(sessionKey)),
  };
}
...
describe("stageSandboxMedia scp remote paths", () => {
  it("rejects remote attachment filenames with shell metacharacters before spawning scp", async () => {
    await withSandboxMediaTempHome("zhushou-triggers-", async (home) => {

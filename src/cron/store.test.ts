...
const { makeStorePath } = createCronStoreHarness({ prefix: "zhushou-cron-store-" });
...
describe("resolveCronStorePath", () => {
  ...
  it("uses ZHUSHOU_HOME for tilde expansion", () => {
    vi.stubEnv("ZHUSHOU_HOME", "/srv/zhushou-home");
    vi.stubEnv("HOME", "/home/other");

    const result = resolveCronStorePath("~/cron/jobs.json");
    expect(result).toBe(path.resolve("/srv/zhushou-home", "cron", "jobs.json"));
  });
});

...
const { makeStorePath } = createCronStoreHarness({ prefix: "assistant-cron-store-" });
...
describe("resolveCronStorePath", () => {
  ...
  it("uses ASSISTANT_HOME for tilde expansion", () => {
    vi.stubEnv("ASSISTANT_HOME", "/srv/assistant-home");
    vi.stubEnv("HOME", "/home/other");

    const result = resolveCronStorePath("~/cron/jobs.json");
    expect(result).toBe(path.resolve("/srv/assistant-home", "cron", "jobs.json"));
  });
});

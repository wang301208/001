...
const DEBUG_COVERAGE_BATCHES = process.env.ZHUSHOU_DEBUG_RUNTIME_COVERAGE === "1";
const COVERAGE_LOADABLE_PLUGIN_ORIGINS =
  buildCoverageLoadablePluginOrigins(COVERAGE_REGISTRY_ENTRIES);
const PLUGIN_OWNED_ZHUSHOU_COVERAGE_EXCLUSIONS = new Set([
...
function applyConfigForZhushouTarget(
...
describe("secrets runtime target coverage", () => {
  ...
  it(
    "handles every zhushou.json registry target when configured as active",
    async () => {
      const entries = COVERAGE_REGISTRY_ENTRIES.filter(
        (entry) =>
          entry.configFile === "zhushou.json" &&
          !PLUGIN_OWNED_ZHUSHOU_COVERAGE_EXCLUSIONS.has(entry.id),
      );
      for (const batch of buildCoverageBatches(entries)) {
        logCoverageBatch("zhushou.json", batch);
        const config = {} as ZhushouConfig;
        const env: Record<string, string> = {};
        for (const [index, entry] of batch.entries()) {
          const envId = `ZHUSHOU_SECRET_TARGET_${entry.id}`;
          ...
          applyConfigForZhushouTarget(config, entry, envId, wildcardToken);
        }
  ...
  it("handles every auth-profiles registry target", async () => {
    ...
    for (const batch of buildCoverageBatches(entries)) {
      ...
      for (const [index, entry] of batch.entries()) {
        const envId = `ZHUSHOU_AUTH_SECRET_TARGET_${entry.id}`;

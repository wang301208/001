...
function getSecretProviders(config: AssistantConfig): Record<string, SecretProviderConfig> {
...
function removeSecretProvider(config: AssistantConfig, providerAlias: string): boolean {
  if (!isRecord(config.secrets?.providers)) {
    return false;
  }
...
function toSourceChoices(config: AssistantConfig): Array<{ value: SecretRefSource; label: string }> {
...
function configureCandidateKey(candidate: {
  ...
  return `assistant:${candidate.path}`;
}
...
function resolveConfigureAgentId(config: AssistantConfig, explicitAgentId?: string): string {
...
async function configureProvidersInteractive(config: AssistantConfig): Promise<void> {
  while (true) {
  ...
  if (!params.providersOnly) {
    ...
    while (true) {
      ...
      const options = visibleCandidates.map((candidate) => ({
        ...
        hint: [
          candidate.configFile === "auth-profiles.json" ? "auth-profiles.json" : "assistant.json",

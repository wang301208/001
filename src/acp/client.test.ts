...
describe("resolveAcpClientSpawnEnv", () => {
  it("sets ZHUSHOU_SHELL marker and preserves existing env values", () => {
    const env = resolveAcpClientSpawnEnv({
      PATH: "/usr/bin",
      USER: "zhushou",
    });

    expect(env.ZHUSHOU_SHELL).toBe("acp-client");
    expect(env.PATH).toBe("/usr/bin");
    expect(env.USER).toBe("zhushou");
  });

  it("overrides pre-existing ZHUSHOU_SHELL to acp-client", () => {
    const env = resolveAcpClientSpawnEnv({
      ZHUSHOU_SHELL: "wrong",
    });
    expect(env.ZHUSHOU_SHELL).toBe("acp-client");
  ...
  it("preserves ZHUSHOU_SHELL even when stripKeys contains it", () => {
    const openAiApiKeyEnv = envVar("OPENAI", "API", "KEY");
    const env = resolveAcpClientSpawnEnv(
      {
        ZHUSHOU_SHELL: "skill-overridden",
        [openAiApiKeyEnv]: "openai-leaked", // pragma: allowlist secret
      },
      { stripKeys: new Set(["ZHUSHOU_SHELL", openAiApiKeyEnv]) },
    );
  ...
  it("strips provider auth env vars for the default 助手 bridge", () => {
    ...
    expect(env.ZHUSHOU_API_KEY).toBe("keep-me");
    expect(env.PATH).toBe("/usr/bin");
    expect(env.ZHUSHOU_SHELL).toBe("acp-client");
  });
...
describe("buildAcpClientStripKeys", () => {
  ...
  it("adds provider auth env vars for the default bridge", () => {
    ...
    expect(stripKeys.has("ZHUSHOU_API_KEY")).toBe(false);
  });
});

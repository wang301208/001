...
import { POSIX_ZHUSHOU_TMP_DIR, resolvePreferredZhushouTmpDir } from "./tmp-zhushou-dir.js";

type TmpDirOptions = NonNullable<Parameters<typeof resolvePreferredZhushouTmpDir>[0]>;

function fallbackTmp(uid = 501) {
  return path.join("/var/fallback", `zhushou-${uid}`);
}
...
function resolveWithReadOnlyTmpFallback(params: {
  ...
  return resolvePreferredZhushouTmpDir({
    accessSync: readOnlyTmpAccessSync(),
    lstatSync: vi.fn((target: string) => {
      if (target === POSIX_ZHUSHOU_TMP_DIR) {
...
function resolveWithMocks(params: {
  ...
  const resolved = resolvePreferredZhushouTmpDir({
    accessSync,
    chmodSync,
...
describe("resolvePreferredZhushouTmpDir", () => {
  it("prefers /tmp/zhushou when it already exists and is writable", () => {
    ...
    expect(resolved).toBe(POSIX_ZHUSHOU_TMP_DIR);
    expect(tmpdir).not.toHaveBeenCalled();
  });

  it("prefers /tmp/zhushou when it does not exist but /tmp is writable", () => {
    ...
    expect(mkdirSync).toHaveBeenCalledWith(POSIX_ZHUSHOU_TMP_DIR, expect.any(Object));
    expect(tmpdir).not.toHaveBeenCalled();
  });

  it.each([
    {
      name: "falls back to os.tmpdir()/zhushou when /tmp/zhushou is not a directory",
      lstatSync: vi.fn(() => makeDirStat({ isDirectory: false, mode: 0o100644 })),
    },
    {
      name: "falls back to os.tmpdir()/zhushou when /tmp is not writable",
      lstatSync: vi.fn(() => {
        throw nodeErrorWithCode("ENOENT");
      }),
    ...
    {
      name: "falls back when /tmp/zhushou exists but is not writable",
    ...
    {
      name: "falls back when /tmp/zhushou is a symlink",
      lstatSync: symlinkTmpDirLstat(),
    },
    {
      name: "falls back when /tmp/zhushou is not owned by the current user",
      lstatSync: vi.fn(() => makeDirStat({ uid: 0 })),
    },
    {
      name: "falls back when /tmp/zhushou is group/other writable",
  ...
  it("repairs existing /tmp/zhushou permissions when they are too broad", () => {
    let preferredMode = 0o40777;
    const chmodSync = vi.fn((target: string, mode: number) => {
      if (target === POSIX_ZHUSHOU_TMP_DIR && mode === 0o700) {
    ...
    expect(resolved).toBe(POSIX_ZHUSHOU_TMP_DIR);
    expect(chmodSync).toHaveBeenCalledWith(POSIX_ZHUSHOU_TMP_DIR, 0o700);
    expect(warn).toHaveBeenCalledWith(expect.stringContaining("tightened permissions on temp dir"));
    expect(tmpdir).not.toHaveBeenCalled();
  });

  it("repairs /tmp/zhushou after create when the initial mode stays too broad", () => {
    ...
    const chmodSync = vi.fn((target: string, mode: number) => {
      chmodCalls += 1;
      if (target === POSIX_ZHUSHOU_TMP_DIR && mode === 0o700 && chmodCalls > 1) {
        preferredMode = 0o40700;
      }
    });
    const warn = vi.fn();
    ...
    expect(resolved).toBe(POSIX_ZHUSHOU_TMP_DIR);
    expect(mkdirSync).toHaveBeenCalledWith(POSIX_ZHUSHOU_TMP_DIR, {
      recursive: true,
      mode: 0o700,
    });
    expect(chmodSync).toHaveBeenCalledWith(POSIX_ZHUSHOU_TMP_DIR, 0o700);
  ...
  it("uses an unscoped fallback suffix when process uid is unavailable", () => {
    ...
    const fallbackPath = path.join(tmpdirPath, "zhushou");
  ...
  it("throws when the fallback directory cannot be created", () => {
    expect(() =>
      resolvePreferredZhushouTmpDir({
        accessSync: readOnlyTmpAccessSync(),
        lstatSync: vi.fn((target: string) => {
          if (target === POSIX_ZHUSHOU_TMP_DIR || target === fallbackTmp()) {
            throw nodeErrorWithCode("ENOENT");
          }
          return secureDirStat();
        }),

...
const VITEST_FS_BASE = path.join(path.parse(process.cwd()).root, "__assistant_vitest__");
const FIXTURE_BASE = path.join(VITEST_FS_BASE, "assistant-root");
...
function setPackageRoot(root: string, name = "assistant") {
  setFile(path.join(root, "package.json"), JSON.stringify({ name }));
}

function expectResolvedPackageRoot(
  syncResolver: typeof import("./assistant-root.js").resolveAssistantPackageRootSync,
  asyncResolver: typeof import("./assistant-root.js").resolveAssistantPackageRoot,
  opts: Parameters<typeof import("./assistant-root.js").resolveAssistantPackageRootSync>[0],
...
vi.mock("./assistant-root.fs.runtime.js", () => ({
  assistantRootFsSync: mockFsModule(),
  assistantRootFs: mockFsPromisesModule(),
}));

describe("resolveAssistantPackageRoot", () => {
  let resolveAssistantPackageRoot: typeof import("./assistant-root.js").resolveAssistantPackageRoot;
  let resolveAssistantPackageRootSync: typeof import("./assistant-root.js").resolveAssistantPackageRootSync;
  ...
  beforeEach(async () => {
    vi.resetModules();
    ({ resolveAssistantPackageRoot, resolveAssistantPackageRootSync } =
      await import("./assistant-root.js"));
  });
  ...
  it.each([
    {
      ...
      setup: () => {
        ...
        const argv1 = path.join(project, "node_modules", ".bin", "assistant");
        const pkgRoot = path.join(project, "node_modules", "assistant");
    ...
    },
    {
      ...
      setup: () => {
        ...
        const bin = path.join(project, "bin", "assistant");
        const realPkg = path.join(project, "real-pkg");
        state.realpaths.set(abs(bin), abs(path.join(realPkg, "assistant.mjs")));
    ...
    {
      name: "falls through from a non-assistant moduleUrl candidate to cwd",
      setup: () => {
        const wrongPkgRoot = fx("moduleurl-fallthrough", "wrong");
        const cwdPkgRoot = fx("moduleurl-fallthrough", "cwd");
        setPackageRoot(wrongPkgRoot, "not-assistant");
    ...
    {
      name: "returns null for non-assistant package roots",
      setup: () => {
        const pkgRoot = fx("not-assistant");
        setPackageRoot(pkgRoot, "not-assistant");
    ...
    },
    {
      ...
      setup: () => {
        ...
        state.realpaths.set(
          abs(argv1),
          abs(path.join(project, "versions", "current", "assistant.mjs")),
        );
        const pkgRoot = path.join(project, "node_modules", "assistant");
        setPackageRoot(pkgRoot);
        return { opts: { argv1 }, expected: pkgRoot };

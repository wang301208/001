...
const suiteRootTracker = createSuiteTempRootTracker({ prefix: "assistant-provider-auth-suite-" });
...
describe("resolveProviderAuths key normalization", () => {
  ...
  async function withSuiteHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
    ...
    const stateDir = path.join(base, ".assistant");
  ...
  function agentDirForHome(home: string): string {
    return path.join(home, ".assistant", "agents", "main", "agent");
  }
  ...
  function buildSuiteEnv(
    ...
    const suiteEnv: NodeJS.ProcessEnv = {
      ...
      HOME: home,
      USERPROFILE: home,
      ASSISTANT_STATE_DIR: path.join(home, ".assistant"),
      ...env,
    };
  ...
  async function writeConfig(home: string, config: Record<string, unknown>) {
    ...
    await fs.writeFile(
      path.join(stateDir, "assistant.json"),

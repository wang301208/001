...
const suiteRootTracker = createSuiteTempRootTracker({ prefix: "zhushou-provider-auth-suite-" });
...
describe("resolveProviderAuths key normalization", () => {
  ...
  async function withSuiteHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
    ...
    const stateDir = path.join(base, ".zhushou");
  ...
  function agentDirForHome(home: string): string {
    return path.join(home, ".zhushou", "agents", "main", "agent");
  }
  ...
  function buildSuiteEnv(
    ...
    const suiteEnv: NodeJS.ProcessEnv = {
      ...
      HOME: home,
      USERPROFILE: home,
      ZHUSHOU_STATE_DIR: path.join(home, ".zhushou"),
      ...env,
    };
  ...
  async function writeConfig(home: string, config: Record<string, unknown>) {
    ...
    await fs.writeFile(
      path.join(stateDir, "zhushou.json"),

...
describe("unscopedPackageName", () => {
  it.each([
    { value: "@assistant/matrix", expected: "matrix" },
...
describe("packageNameMatchesId", () => {
  it.each([
    { packageName: "@assistant/matrix", id: "matrix", expected: true },
    { packageName: "@assistant/matrix", id: "@assistant/matrix", expected: true },
    { packageName: "@assistant/matrix", id: "signal", expected: false },
    { packageName: " ", id: "matrix", expected: false },
    { packageName: "@assistant/matrix", id: " ", expected: false },
...
describe("resolveSafeInstallDir", () => {
  it("resolves install dirs under the base directory", () => {
    expect(
      ...
      ok: true,
      path: path.join("/tmp/plugins", "@assistant__matrix"),
    });
  });
...
describe("assertCanonicalPathWithinBase", () => {
  it("accepts in-base directories", async () => {
    await withTempDir({ prefix: "assistant-install-safe-" }, async (baseDir) => {
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      await withTempDir({ prefix: "assistant-install-safe-" }, async (baseDir) => {
        await withTempDir({ prefix: "assistant-install-safe-outside-" }, async (outsideDir) => {
  ...
  it.runIf(process.platform !== "win32")("rejects symlinked base directories", async () => {
    await withTempDir({ prefix: "assistant-install-safe-" }, async (parentDir) => {

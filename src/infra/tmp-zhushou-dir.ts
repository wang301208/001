...
export const POSIX_ZHUSHOU_TMP_DIR = "/tmp/zhushou";
const TMP_DIR_ACCESS_MODE = fs.constants.W_OK | fs.constants.X_OK;

type ResolvePreferredZhushouTmpDirOptions = {
...
export function resolvePreferredZhushouTmpDir(
  options: ResolvePreferredZhushouTmpDirOptions = {},
): string {
  ...
  const fallback = (): string => {
    const base = tmpdir();
    const suffix = uid === undefined ? "zhushou" : `zhushou-${uid}`;
  ...
  const tryRepairWritableBits = (candidatePath: string): boolean => {
    try {
      ...
      warn(`[zhushou] tightened permissions on temp dir: ${candidatePath}`);
  ...
  const existingPreferredState = resolveDirState(POSIX_ZHUSHOU_TMP_DIR);
  if (existingPreferredState === "available") {
    return POSIX_ZHUSHOU_TMP_DIR;
  }
  if (existingPreferredState === "invalid") {
    if (tryRepairWritableBits(POSIX_ZHUSHOU_TMP_DIR)) {
      return POSIX_ZHUSHOU_TMP_DIR;
    }
  ...
  try {
    ...
    mkdirSync(POSIX_ZHUSHOU_TMP_DIR, { recursive: true, mode: 0o700 });
    chmodSync(POSIX_ZHUSHOU_TMP_DIR, 0o700);
    if (
      resolveDirState(POSIX_ZHUSHOU_TMP_DIR) !== "available" &&
      !tryRepairWritableBits(POSIX_ZHUSHOU_TMP_DIR)
    ) {
      return ensureTrustedFallbackDir();
    }
    return POSIX_ZHUSHOU_TMP_DIR;

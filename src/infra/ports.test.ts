...
describe("ports helpers", () => {
  ...
  it("prints an 助手-specific hint when port details look like another 助手 instance", async () => {
    ...
    await handlePortError(
      new PortInUseError(18789, "node dist/index.js zhushou gateway"),
      18789,
      "gateway start",
      runtime,
    ).catch(() => {});
...
describeUnix("inspectPortUsage", () => {
  ...
  it("falls back to ss when lsof is unavailable", async () => {
    ...
    runCommandWithTimeoutMock.mockImplementation(async (argv: string[]) => {
      ...
      if (command === "ps") {
        if (argv.includes("command=")) {
          return {
            stdout: "node /tmp/zhushou/dist/index.js gateway --port 18789\n",
            stderr: "",
            code: 0,
          };
        }
    ...
    try {
      ...
      expect(result.listeners[0]?.commandLine).toContain("zhushou");

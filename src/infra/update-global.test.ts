...
describe("update global helpers", () => {
  ...
  it("prefers explicit package spec overrides", () => {
    envSnapshot = captureEnv(["ASSISTANT_UPDATE_PACKAGE_SPEC"]);
    process.env.ASSISTANT_UPDATE_PACKAGE_SPEC = "file:/tmp/assistant.tgz";

    expect(resolveGlobalInstallSpec({ packageName: "assistant", tag: "latest" })).toBe(
      "file:/tmp/assistant.tgz",
    );
    expect(
      resolveGlobalInstallSpec({
        packageName: "assistant",
        tag: "beta",
        env: { ASSISTANT_UPDATE_PACKAGE_SPEC: "assistant@next" },
      }),
    ).toBe("assistant@next");
  });
  ...
  it("resolves global roots and package roots from runner output", async () => {
    ...
    await expect(resolveGlobalPackageRoot("npm", runCommand, 1000)).resolves.toBe(
      path.join("/tmp/npm-root", "assistant"),
    );
  });

  it("maps main and explicit install specs for global installs", () => {
    expect(resolveGlobalInstallSpec({ packageName: "assistant", tag: "main" })).toBe(
      ASSISTANT_MAIN_PACKAGE_SPEC,
    );
    expect(
      resolveGlobalInstallSpec({
        packageName: "assistant",
        tag: "github:assistant/assistant#feature/my-branch",
      }),
    ).toBe("github:assistant/assistant#feature/my-branch");
    expect(
      resolveGlobalInstallSpec({
        packageName: "assistant",
        tag: "https://example.com/assistant-main.tgz",
      }),
    ).toBe("https://example.com/assistant-main.tgz");
  });
  ...
  it("classifies main and raw install specs separately from registry selectors", () => {
    ...
    expect(isExplicitPackageInstallSpec("github:assistant/assistant#main")).toBe(true);
    expect(isExplicitPackageInstallSpec("https://example.com/assistant-main.tgz")).toBe(true);
    expect(isExplicitPackageInstallSpec("file:/tmp/assistant-main.tgz")).toBe(true);
    expect(isExplicitPackageInstallSpec("beta")).toBe(false);

    expect(canResolveRegistryVersionForPackageTarget("latest")).toBe(true);
    expect(canResolveRegistryVersionForPackageTarget("2026.3.22")).toBe(true);
    expect(canResolveRegistryVersionForPackageTarget("main")).toBe(false);
    expect(canResolveRegistryVersionForPackageTarget("github:assistant/assistant#main")).toBe(false);
  });

  it("detects install managers from resolved roots and on-disk presence", async () => {
    await withTempDir({ prefix: "assistant-update-global-" }, async (base) => {
      const npmRoot = path.join(base, "npm-root");
      const pnpmRoot = path.join(base, "pnpm-root");
      const bunRoot = path.join(base, ".bun", "install", "global", "node_modules");
      const pkgRoot = path.join(pnpmRoot, "assistant");
      await fs.mkdir(pkgRoot, { recursive: true });
      await fs.mkdir(path.join(npmRoot, "assistant"), { recursive: true });
      await fs.mkdir(path.join(bunRoot, "assistant"), { recursive: true });
      ...
      await fs.rm(path.join(npmRoot, "assistant"), { recursive: true, force: true });
      await fs.rm(path.join(pnpmRoot, "assistant"), { recursive: true, force: true });
  ...
  it("prefers the owning npm prefix when PATH npm points at a different global root", async () => {
    ...
    try {
      await withTempDir({ prefix: "assistant-update-npm-prefix-" }, async (base) => {
        ...
        const pkgRoot = path.join(brewRoot, "assistant");
        ...
        expect(globalInstallArgs("npm", "assistant@latest", pkgRoot)).toEqual([
          brewNpm,
          "i",
          "-g",
          "assistant@latest",
        ...
        expect(globalInstallFallbackArgs("npm", "assistant@latest", pkgRoot)).toEqual([
          brewNpm,
          "i",
          "-g",
          "assistant@latest",
  ...
  it("does not infer npm ownership from path shape alone when the owning npm binary is absent", async () => {
    await withTempDir({ prefix: "assistant-update-npm-missing-bin-" }, async (base) => {
  ...
  it("prefers npm.cmd for win32-style global npm roots", async () => {
    ...
    try {
      await withTempDir({ prefix: "assistant-update-win32-npm-prefix-" }, async (base) => {
        const npmPrefix = path.join(base, "Roaming", "npm");
        const npmRoot = path.join(npmPrefix, "node_modules");
        const pkgRoot = path.join(npmRoot, "assistant");
  ...
  it("builds install argv and npm fallback argv", () => {
    ...
    expect(globalInstallArgs("npm", "assistant@latest")).toEqual([
      "npm",
      "i",
      "-g",
      "assistant@latest",
    ...
    expect(globalInstallArgs("pnpm", "assistant@latest")).toEqual([
      "pnpm",
      "add",
      "-g",
      "assistant@latest",
    ]);
    expect(globalInstallArgs("bun", "assistant@latest")).toEqual([
      "bun",
      "add",
      "-g",
      "assistant@latest",
    ...
    expect(globalInstallFallbackArgs("npm", "assistant@latest")).toEqual([
      "npm",
      "i",
      "-g",
      "assistant@latest",
    ...
    expect(globalInstallFallbackArgs("pnpm", "assistant@latest")).toBeNull();
    expect(
      globalInstallArgs({ manager: "pnpm", command: "/opt/homebrew/bin/pnpm" }, "assistant@latest"),
    ).toEqual(["/opt/homebrew/bin/pnpm", "add", "-g", "assistant@latest"]);
  });

  it("cleans only renamed package directories", async () => {
    await withTempDir({ prefix: "assistant-update-cleanup-" }, async (root) => {
      await fs.mkdir(path.join(root, ".assistant-123"), { recursive: true });
      await fs.mkdir(path.join(root, ".assistant-456"), { recursive: true });
      await fs.writeFile(path.join(root, ".assistant-file"), "nope", "utf8");
      ...
      await expect(
        ...
        removed: [".assistant-123", ".assistant-456"],
      });
      await expect(fs.stat(path.join(root, "assistant"))).resolves.toBeDefined();
      await expect(fs.stat(path.join(root, ".assistant-file"))).resolves.toBeDefined();
    });
  });

  it("checks installed dist against the packaged inventory", async () => {
    await withTempDir({ prefix: "assistant-update-global-pkg-" }, async (packageRoot) => {
      await fs.writeFile(
        path.join(packageRoot, "package.json"),
        JSON.stringify({ name: "assistant", version: "1.0.0" }),
        "utf-8",
      );
  ...
  it("falls back to legacy sidecar verification when the inventory is missing", async () => {
    await withTempDir({ prefix: "assistant-update-global-legacy-" }, async (packageRoot) => {
      await fs.writeFile(
        path.join(packageRoot, "package.json"),
        JSON.stringify({ name: "assistant", version: "1.0.0" }),
        "utf-8",
  ...
  it("fails closed on newer installs when the inventory is missing", async () => {
    await withTempDir(
      { prefix: "assistant-update-global-missing-inventory-new-" },
      async (packageRoot) => {
        await fs.writeFile(
          path.join(packageRoot, "package.json"),
          JSON.stringify({ name: "assistant", version: "2026.4.15" }),
          "utf-8",
        );
        for (const relativePath of NPM_UPDATE_COMPAT_SIDECAR_PATHS) {
          const absolutePath = path.join(packageRoot, relativePath);
  ...
  it("rejects invalid inventory files during global verify", async () => {
    await withTempDir(
      { prefix: "assistant-update-global-invalid-inventory-" },
      async (packageRoot) => {
        await fs.writeFile(
          path.join(packageRoot, "package.json"),
          JSON.stringify({ name: "assistant", version: "2026.4.15" }),
  ...
  it("verifies legacy sidecars for installed bundled plugins without inventory", async () => {
    await withTempDir({ prefix: "assistant-update-global-legacy-plugin-" }, async (packageRoot) => {
      await fs.writeFile(
        path.join(packageRoot, "package.json"),
        JSON.stringify({ name: "assistant", version: "1.0.0" }),
        "utf-8",
      ...
      await fs.writeFile(matrixPackageJson, JSON.stringify({ name: "@assistant/matrix" }), "utf-8");
  ...
  it("still enforces critical sidecars when the inventory omits them", async () => {
    await withTempDir(
      { prefix: "assistant-update-global-critical-sidecars-" },
      async (packageRoot) => {
        await fs.writeFile(
          path.join(packageRoot, "package.json"),
          JSON.stringify({ name: "assistant", version: "2026.4.15" }),
        ...
        await fs.writeFile(
          matrixPackageJson,
          JSON.stringify({ name: "@assistant/matrix" }),
          "utf-8",
        );
        await writePackageDistInventory(packageRoot);
  ...
  it("ignores stale metadata for non-packaged private QA plugins during inventory verify", async () => {
    await withTempDir(
      { prefix: "assistant-update-global-stale-private-qa-" },
      async (packageRoot) => {
        await fs.writeFile(
          path.join(packageRoot, "package.json"),
          JSON.stringify({ name: "assistant", version: "2026.4.15" }),
        ...
        await fs.writeFile(
          ...
          JSON.stringify({ name: "@assistant/qa-lab" }),
          "utf-8",
        );
        await writePackageDistInventory(packageRoot);

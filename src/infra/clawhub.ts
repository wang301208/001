...
// Keep aligned with @assistant/plugin-package-contract ExternalPluginCompatibility.
...
function resolveClawHubConfigPaths(): string[] {
  const explicit =
    normalizeOptionalString(process.env.ASSISTANT_CLAWHUB_CONFIG_PATH) ||
...
export async function resolveClawHubAuthToken(): Promise<string | undefined> {
  const envToken =
    normalizeOptionalString(process.env.ASSISTANT_CLAWHUB_TOKEN) ||
...
export async function downloadClawHubPackageArchive(params: {
  ...
  const target = await createTempDownloadTarget({
    prefix: "assistant-clawhub-package",
...
export async function downloadClawHubSkillArchive(params: {
  ...
  const bytes = new Uint8Array(await response.arrayBuffer());
  const target = await createTempDownloadTarget({
    prefix: "assistant-clawhub-skill",

...
> = {
  kind: ApprovalKind;
  config: (cfg: ZhushouConfig) => ExecApprovalForwardingConfig | undefined;
...
function buildExpiredMessage(request: ExecApprovalRequest) {
  return `⏱️ Exec approval countdown ended; auto-approved once. ID: ${request.id}`;
}

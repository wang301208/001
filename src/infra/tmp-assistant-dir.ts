...
export const POSIX_ASSISTANT_TMP_DIR = "/tmp/assistant";
const TMP_DIR_ACCESS_MODE = fs.constants.W_OK | fs.constants.X_OK;

type ResolvePreferredAssistantTmpDirOptions = {
...
export function resolvePreferredAssistantTmpDir(
  options: ResolvePreferredAssistantTmpDirOptions = {},
): string {
  ...
  const fallback = (): string => {
    const base = tmpdir();
    const suffix = uid === undefined ? "assistant" : `assistant-${uid}`;
  ...
  const tryRepairWritableBits = (candidatePath: string): boolean => {
    try {
      ...
      warn(`[assistant] tightened permissions on temp dir: ${candidatePath}`);
  ...
  const ensureTrustedFallbackDir = (): string => {
    ...
    if (state === "invalid") {
      ...
      throw new Error(`Unsafe fallback 助手 temp dir: ${fallbackPath}`);
    }
    try {
      mkdirSync(fallbackPath, { recursive: true, mode: 0o700 });
      chmodSync(fallbackPath, 0o700);
    } catch {
      throw new Error(`Unable to create fallback 助手 temp dir: ${fallbackPath}`);
    }
  ...
  const existingPreferredState = resolveDirState(POSIX_ASSISTANT_TMP_DIR);
  if (existingPreferredState === "available") {
    return POSIX_ASSISTANT_TMP_DIR;
  }
  if (existingPreferredState === "invalid") {
    if (tryRepairWritableBits(POSIX_ASSISTANT_TMP_DIR)) {
      return POSIX_ASSISTANT_TMP_DIR;
    }
  ...
  try {
    ...
    mkdirSync(POSIX_ASSISTANT_TMP_DIR, { recursive: true, mode: 0o700 });
    chmodSync(POSIX_ASSISTANT_TMP_DIR, 0o700);
    if (
      resolveDirState(POSIX_ASSISTANT_TMP_DIR) !== "available" &&
      !tryRepairWritableBits(POSIX_ASSISTANT_TMP_DIR)
    ) {
      return ensureTrustedFallbackDir();
    }
    return POSIX_ASSISTANT_TMP_DIR;

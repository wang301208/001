...
export class DirectoryCache<T> {
  ...
  private lastConfigRef: AssistantConfig | null = null;
  ...
  get(key: string, cfg: AssistantConfig): T | undefined {
  ...
  set(key: string, value: T, cfg: AssistantConfig): void {
    this.resetIfConfigChanged(cfg);
    const now = Date.now();
  ...
  clear(cfg?: AssistantConfig): void {
    this.cache.clear();
    if (cfg) {
  ...
  private resetIfConfigChanged(cfg: AssistantConfig): void {

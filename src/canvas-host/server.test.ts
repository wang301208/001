...
describe("canvas host", () => {
  ...
  it("injects live reload script", () => {
    ...
    expect(out).toContain("zhushouCanvasA2UIAction");
    expect(out).toContain("zhushouSendUserAction");
  });
  ...
  it("creates a default index.html when missing", async () => {
    ...
    try {
      ...
      expect(html).toContain("zhushouSendUserAction");
  ...
  it("serves A2UI scaffold and blocks traversal/symlink escapes", async () => {
    ...
    try {
      ...
      const res = await realFetch(`http://127.0.0.1:${server.port}/__zhushou__/a2ui/`);
      const html = await res.text();
      expect(res.status).toBe(200);
      expect(html).toContain("zhushou-a2ui-host");
      expect(html).toContain("zhushouCanvasA2UIAction");

      const bundleRes = await realFetch(
        `http://127.0.0.1:${server.port}/__zhushou__/a2ui/a2ui.bundle.js`,
      );
      const js = await bundleRes.text();
      expect(bundleRes.status).toBe(200);
      expect(js).toContain("zhushouA2UI");

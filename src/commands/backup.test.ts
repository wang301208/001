...
describe("backup commands", () => {
  ...
  async function resetTempHome() {
    ...
    await fs.mkdir(path.join(tempHome.home, ".assistant"), { recursive: true });
    delete process.env.ASSISTANT_CONFIG_PATH;
  }

  beforeAll(async () => {
    tempHome = await createTempHomeEnv("assistant-backup-test-");
  });
  ...
  async function withInvalidWorkspaceBackupConfig<T>(fn: (runtime: RuntimeEnv) => Promise<T>) {
    const stateDir = path.join(tempHome.home, ".assistant");
    const configPath = path.join(tempHome.home, "custom-config.json");
    process.env.ASSISTANT_CONFIG_PATH = configPath;
    await fs.writeFile(path.join(stateDir, "assistant.json"), JSON.stringify({}), "utf8");
  ...
  it("orders coverage checks by canonical path so symlinked workspaces do not duplicate state", async () => {
    ...
    const stateDir = path.join(tempHome.home, ".assistant");
    const workspaceDir = path.join(stateDir, "workspace");
    const symlinkDir = await fs.mkdtemp(path.join(os.tmpdir(), "assistant-workspace-link-"));
    ...
    try {
      ...
      const plan = await resolveBackupPlanFromPaths({
        stateDir,
        configPath: path.join(stateDir, "assistant.json"),
  ...
  it("creates an archive with a manifest and external workspace payload", async () => {
    const stateDir = path.join(tempHome.home, ".assistant");
    const externalWorkspace = await fs.mkdtemp(path.join(os.tmpdir(), "assistant-workspace-"));
    const configPath = path.join(tempHome.home, "custom-config.json");
    const backupDir = await fs.mkdtemp(path.join(os.tmpdir(), "assistant-backups-"));
  ...
  it("rejects symlinked output paths even when intermediate directories do not exist yet", async () => {
    ...
    const stateDir = path.join(tempHome.home, ".assistant");
    const symlinkDir = await fs.mkdtemp(path.join(os.tmpdir(), "assistant-backup-link-"));
  ...
  it("falls back to the home directory when cwd is inside a backed-up source tree", async () => {
    ...
    if (process.platform !== "win32") {
      const linkParent = await fs.mkdtemp(path.join(os.tmpdir(), "assistant-backup-cwd-link-"));

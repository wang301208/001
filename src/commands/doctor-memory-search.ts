...
function buildMemoryRecallIssueNote(audit: ShortTermAuditSummary): string | null {
  ...
  const guidance = hasFixableIssue
    ? `Fix: ${formatCliCommand("zhushou doctor --fix")} or ${formatCliCommand("zhushou memory status --fix")}`
    : `Verify: ${formatCliCommand("zhushou memory status --deep")}`;
  return [
...
export async function noteMemoryRecallHealth(cfg: ZhushouConfig): Promise<void> {
  try {
 ...
 * Runs as part of `zhushou doctor` — config-only checks where possible;
    ...
    if (!qmdCheck.available) {
      note(
        [
          ...
          `- Set an explicit binary path: ${formatCliCommand("zhushou config set memory.qmd.command /absolute/path/to/qmd")}`,
          `- Or switch back to builtin memory: ${formatCliCommand("zhushou config set memory.backend builtin")}`,
          "",
          `Verify: ${formatCliCommand("zhushou memory status --deep")}`,
        ]
          .filter(Boolean)
  ...
  if (resolved.provider !== "auto") {
      ...
      note(
        [
          ...
          suggestedRemoteProvider
            ? `- Switch to a remote provider: ${formatCliCommand(`zhushou config set agents.defaults.memorySearch.provider ${suggestedRemoteProvider}`)}`
    ...
    note(
      [
        ...
        `- Configure credentials: ${formatCliCommand("zhushou configure --section model")}`,
        `- To disable: ${formatCliCommand("zhushou config set agents.defaults.memorySearch.enabled false")}`,
        "",
        `Verify: ${formatCliCommand("zhushou memory status --deep")}`,
      ].join("\n"),
      "Memory search",

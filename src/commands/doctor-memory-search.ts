...
function buildMemoryRecallIssueNote(audit: ShortTermAuditSummary): string | null {
  ...
  const guidance = hasFixableIssue
    ? `Fix: ${formatCliCommand("assistant doctor --fix")} or ${formatCliCommand("assistant memory status --fix")}`
    : `Verify: ${formatCliCommand("assistant memory status --deep")}`;
  return [
...
export async function noteMemoryRecallHealth(cfg: AssistantConfig): Promise<void> {
  try {
 ...
 * Runs as part of `assistant doctor` — config-only checks where possible;
    ...
    if (!qmdCheck.available) {
      note(
        [
          ...
          `- Set an explicit binary path: ${formatCliCommand("assistant config set memory.qmd.command /absolute/path/to/qmd")}`,
          `- Or switch back to builtin memory: ${formatCliCommand("assistant config set memory.backend builtin")}`,
          "",
          `Verify: ${formatCliCommand("assistant memory status --deep")}`,
        ]
          .filter(Boolean)
  ...
  if (resolved.provider !== "auto") {
      ...
      note(
        [
          ...
          suggestedRemoteProvider
            ? `- Switch to a remote provider: ${formatCliCommand(`assistant config set agents.defaults.memorySearch.provider ${suggestedRemoteProvider}`)}`
    ...
    note(
      [
        ...
        `- Configure credentials: ${formatCliCommand("assistant configure --section model")}`,
        `- To disable: ${formatCliCommand("assistant config set agents.defaults.memorySearch.enabled false")}`,
        "",
        `Verify: ${formatCliCommand("assistant memory status --deep")}`,
      ].join("\n"),
      "Memory search",

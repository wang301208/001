...
export function registerAdvancedAutonomyCommands(program: Command): void {
  ...
  resourceGroup
    ...
    .action(async (options) => {
      console.log(`🔄 请求资源: ${options.type} (${options.amount})`);
      ...
      try {
        ...
        console.log("\n✅ 资源分配成功！\n");
        console.log(`请求 ID: ${allocation.requestId}`);
        console.log(`分配数量: ${allocation.allocated}`);
        console.log(`开始时间: ${new Date(allocation.startTime).toLocaleString()}`);
  ...
  collaborationGroup
    ...
    .action(async (options) => {
      console.log(`📝 创建协作任务: ${options.description}`);
      ...
      try {
        ...
        console.log(`分配代理: ${task.assignedAgents.length > 0 ? task.assignedAgents.join(", ") : "无"}`);

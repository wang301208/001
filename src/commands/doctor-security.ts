...
import type { ZhushouConfig, GatewayBindMode } from "../config/config.js";
...
function collectImplicitHeartbeatDirectPolicyWarnings(cfg: ZhushouConfig): string[] {
...
function collectExecPolicyConflictWarnings(cfg: ZhushouConfig): string[] {
  const warnings: string[] = [];
  ...
  const maybeWarn = (params: {
    ...
    warnings.push(
      [
        ...
        `  Inspect with: ${formatCliCommand("zhushou approvals get --gateway")}`,
      ].join("\n"),
    );
  };
...
function collectDurableExecApprovalWarnings(cfg: ZhushouConfig): string[] {
  void cfg;
  return [];
}

export async function noteSecurityWarnings(cfg: ZhushouConfig) {
  const warnings: string[] = [];
  const auditHint = `- Run: ${formatCliCommand("zhushou security audit --deep")}`;

  if (cfg.approvals?.exec?.enabled === false) {
    warnings.push(
      "- Note: approvals.exec.enabled=false disables approval forwarding only.",
      "  Host exec gating still comes from ~/.zhushou/exec-approvals.json.",
      `  Check local policy with: ${formatCliCommand("zhushou approvals get --gateway")}`,
    );
  }
    ...
    if (!hasSharedSecret) {
      const authFixLines =
        resolvedAuth.mode === "password"
          ? [
              `  Fix: ${formatCliCommand("zhushou configure")} to set a password`,
              `  Or switch to token: ${formatCliCommand("zhushou config set gateway.auth.mode token")}`,
            ]
          : [
              `  Fix: ${formatCliCommand("zhushou doctor --fix")} to generate a token`,
              `  Or set token directly: ${formatCliCommand(
                "zhushou config set gateway.auth.mode token",
              )}`,
            ];
      warnings.push(
        `- CRITICAL: Gateway bound to ${bindDescriptor} without authentication.`,
        `  Anyone on your network (or internet if port-forwarded) can fully control your agent.`,
        `  Fix: ${formatCliCommand("zhushou config set gateway.bind loopback")}`,

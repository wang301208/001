...
import type { AssistantConfig, GatewayBindMode } from "../config/config.js";
...
function collectImplicitHeartbeatDirectPolicyWarnings(cfg: AssistantConfig): string[] {
...
function collectExecPolicyConflictWarnings(cfg: AssistantConfig): string[] {
  const warnings: string[] = [];
  const approvals = loadExecApprovals();
  const defaultRequestedSecuritySource = "助手 default (full)";
  const defaultRequestedAskSource = "助手 default (off)";
  ...
  const maybeWarn = (params: {
    ...
    warnings.push(
      [
        ...
        `  Inspect with: ${formatCliCommand("assistant approvals get --gateway")}`,
      ].join("\n"),
    );
  };
...
function collectDurableExecApprovalWarnings(cfg: AssistantConfig): string[] {
  void cfg;
  return [];
}

export async function noteSecurityWarnings(cfg: AssistantConfig) {
  const warnings: string[] = [];
  const auditHint = `- Run: ${formatCliCommand("assistant security audit --deep")}`;

  if (cfg.approvals?.exec?.enabled === false) {
    warnings.push(
      "- Note: approvals.exec.enabled=false disables approval forwarding only.",
      "  Host exec gating still comes from ~/.assistant/exec-approvals.json.",
      `  Check local policy with: ${formatCliCommand("assistant approvals get --gateway")}`,
    );
  }
    ...
    if (!hasSharedSecret) {
      const authFixLines =
        resolvedAuth.mode === "password"
          ? [
              `  Fix: ${formatCliCommand("assistant configure")} to set a password`,
              `  Or switch to token: ${formatCliCommand("assistant config set gateway.auth.mode token")}`,
            ]
          : [
              `  Fix: ${formatCliCommand("assistant doctor --fix")} to generate a token`,
              `  Or set token directly: ${formatCliCommand(
                "assistant config set gateway.auth.mode token",
              )}`,
            ];
      warnings.push(
        `- CRITICAL: Gateway bound to ${bindDescriptor} without authentication.`,
        `  Anyone on your network (or internet if port-forwarded) can fully control your agent.`,
        `  Fix: ${formatCliCommand("assistant config set gateway.bind loopback")}`,

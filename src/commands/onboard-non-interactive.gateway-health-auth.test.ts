...
async function withTempDir<T>(run: (dir: string) => Promise<T>): Promise<T> {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "assistant-gateway-health-auth-"));
  try {
...
describe("resolveGatewayHealthProbeToken", () => {
  ...
  it("does not fall back to stale ASSISTANT_GATEWAY_TOKEN when a SecretRef is unresolved", async () => {
    await withTempDir(async (dir) => {
      process.env.ASSISTANT_GATEWAY_TOKEN = "stale-env-token";

...
describe("detectChangedScope", () => {
  ...
  it("routes removed native-app paths through the node fallback only", () => {
    ...
    expect(detectChangedScope(["apps/shared/ZhushouKit/Sources/Foo.swift"])).toEqual({
      runNode: true,
      runWindows: false,
      runSkillsPython: false,
      runChangedSmoke: false,
  ...
  it("routes removed generated protocol model paths through node fallback", () => {
    expect(detectChangedScope(["apps/macos/Sources/ZhushouProtocol/GatewayModels.swift"])).toEqual(
      {
        runNode: true,
        runWindows: false,
        runSkillsPython: false,

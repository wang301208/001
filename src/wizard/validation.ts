...
const CONFLICT_RULES: ReadonlyArray<{
  code: string;
  check: (cfg: AssistantConfig) => boolean;
  paths: string[];
  message: string;
}> = [
  {
...
function hasLegacyField(config: AssistantConfig, fieldPath: string): boolean {
...
function hasLegacySandboxPerSession(config: AssistantConfig): boolean {
...
function hasLegacyGatewayBindHostAlias(config: AssistantConfig): boolean {
  const bind = config.gateway?.bind;
  return typeof bind === "string" && LEGACY_GATEWAY_BIND_HOST_ALIASES.has(bind.toLowerCase());
}

function hasLegacyProviderScopedWebSearch(config: AssistantConfig): boolean {
 ...
 */
export function detectConfigConflicts(config: AssistantConfig): ConfigConflict[] {

...
describe("plugin sdk alias helpers", () => {
  ...
  it.each([
    {
      name: "does not derive plugin-sdk subpaths from cwd fallback when package root is not an 助手 root",
      fixture: () =>
        createPluginSdkAliasFixture({
  ...
  it("resolves plugin-sdk aliases for user-installed plugins via moduleUrl hint", () => {
    ...
    // An empty string is falsy so resolveTrustedZhushouRootFromArgvHint returns null,
  ...
  it.each([
    {
      name: "does not resolve plugin-sdk alias files from cwd fallback when package root is not an 助手 root",
      fixture: () =>
        createPluginSdkAliasFixture({
          srcFile: "channel-runtime.ts",
          distFile: "channel-runtime.js",
  ...
  it("allows setup-only TypeScript plugin entries to use native Jiti on Windows", () => {
    withPlatform("win32", () => {
      expect(resolvePluginLoaderJitiTryNative("/repo/extensions/anthropic-vertex/setup-api.ts")).toBe(
        true,
      );
    });
  });

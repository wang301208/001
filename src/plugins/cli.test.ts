...
const mocks = vi.hoisted(() => ({
  ...
  loadZhushouPluginCliRegistry: vi.fn(),
...
vi.mock("./loader.js", () => ({
  loadZhushouPluginCliRegistry: (...args: unknown[]) =>
    mocks.loadZhushouPluginCliRegistry(...args),
  loadZhushouPlugins: (...args: unknown[]) => mocks.loadZhushouPlugins(...args),
}));
...
describe("registerPluginCliCommands", () => {
  ...
  beforeEach(() => {
    ...
    mocks.loadZhushouPluginCliRegistry.mockReset();
    mocks.loadZhushouPluginCliRegistry.mockResolvedValue(createCliRegistry());
    mocks.loadZhushouPlugins.mockReset();
    mocks.loadZhushouPlugins.mockReturnValue({
      ...createCliRegistry(),
      diagnostics: [],
    });
  ...
  it("loads root-help descriptors through the dedicated non-activating CLI collector", async () => {
    ...
    mocks.loadZhushouPluginCliRegistry.mockResolvedValue({
      cliRegistrars: [
        {
          pluginId: "matrix",
          register: vi.fn(),
    ...
    expect(mocks.loadZhushouPluginCliRegistry).toHaveBeenCalledWith(
      expect.objectContaining({
        config: autoEnabledConfig,
  ...
  it("keeps runtime CLI command registration on the full plugin loader for legacy channel plugins", async () => {
    ...
    expect(mocks.loadZhushouPluginCliRegistry).not.toHaveBeenCalled();
  });
  ...
  it("skips plugin CLI registration from validated config when the snapshot is invalid", async () => {
    ...
    expect(mocks.loadZhushouPlugins).not.toHaveBeenCalled();
  });
});

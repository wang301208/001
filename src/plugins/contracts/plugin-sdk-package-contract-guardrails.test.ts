...
function packZhushouToTempDir(packDir: string): string {
...
describe("plugin-sdk package contract guardrails", () => {
  ...
  it(
    "keeps matrix crypto WASM in the packed artifact manifest",
    async () => {
      const tempRoot = makeTrackedTempDir("zhushou-matrix-wasm-pack", tempDirs);
      const packDir = join(tempRoot, "pack");
      mkdirSync(packDir, { recursive: true });

      const archivePath = packZhushouToTempDir(packDir);

...
async function listMarketplaceDownloadTempDirs(): Promise<string[]> {
  ...
  return entries
    .filter(
      (entry) => entry.isDirectory() && entry.name.startsWith("assistant-marketplace-download-"),
    )
...
describe("marketplace plugins", () => {
  ...
  it("lists plugins from a local marketplace root", async () => {
    await withTempDir("assistant-marketplace-test-", async (rootDir) => {
  ...
  it("resolves Claude-style plugin@marketplace shortcuts from known_marketplaces.json", async () => {
    await withTempDir("assistant-marketplace-test-", async (homeDir) => {
      const assistantHome = path.join(homeDir, "assistant-home");
      ...
      const shortcut = await withEnvAsync(
        { HOME: homeDir, ASSISTANT_HOME: assistantHome },

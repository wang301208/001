...
export type ProviderAuthMethod = {
   ...
   * key + OAuth, or region-specific login flows). 助手 uses this to expose
 ...
 *
 * Runs after 助手 resolves an explicit/discovered/dynamic model and before
 ...
 *
 * The helper methods cover the common 助手 auth resolution paths:
 *
 ...
 *
 * Called when OAuth refresh fails and 助手 wants a provider-specific repair
...
/**
 * Provider-owned extra-param normalization before 助手 builds its generic
 ...
 * `buildReplayPolicy`, `normalizeToolSchemas`, and `wrapStreamFn`. 助手 no
...
/**
 * Provider-owned stream wrapper hook after 助手 applies its generic
 ...
 *
 * Return `true` or `false` to override 助手's built-in provider cache TTL
 ...
 *
 * Runs only after 助手 exhausts normal env/profile/config auth resolution
 ...
 * hint suffix that 助手 should append to the generic `Unknown model`
 * error.
 */
 ...
 *
 * Runs after 助手 loads the discovered model catalog and merges configured
...
export type ProviderOAuthProfileIdRepair = {
   ...
   *
   * When omitted, 助手 falls back to `<provider>:default`.
   */
...
export type ProviderPlugin = {
   ...
   *
   * 助手 calls this only from async model resolution paths. After it
   ...
   *
   * Runs after 助手 performs generic transcript cleanup. Use this for
   ...
   *
   * Use this for transport-family schema cleanup before 助手 registers
  ...
  /**
   * Provider-owned stream wrapper applied after generic 助手 wrappers.
   *
   ...
   *
   * Called after 助手 resolves the raw configured credential but before the
   ...
   * hint than 助手's generic auth-store guidance.
   */
   ...
   * 助手's generic `Unknown model` error after catalog/runtime lookup
   * fails.
   */
   ...
   * `errorMessage` when 助手 should surface a provider-specific hint for
   ...
   * merging. 助手 deduplicates by `provider/id`, so plugins only need to
   ...
   * without replacing the full 助手-owned system prompt.
   */
   ...
   * rewrites after 助手 has assembled the complete prompt. Return
   ...
   *
   * 助手 uses this when a stored auth profile is already valid and needs to
   ...
   *
   * 助手 calls this before falling back to the shared `pi-ai` OAuth
   ...
   * path. Return nothing to keep 助手's generic error text.
   */
  buildAuthDoctorHint?: (
   ...
   * 助手 falls back to that stored profile.
   */
...
export type PluginCommandContext = {
  ...
  sessionId?: string;
  /** Transcript file for the active 助手 session when available. */
  sessionFile?: string;
 ...
 * If you want 助手 to keep a plugin command lazy-loaded while still
...
export type OpenClawPluginModule = OpenClawPluginDefinition | ((api: ZhushouPluginApi) => void);
...
export type ZhushouPluginApi = {
  ...
  registerCli: (
    ...
    opts?: {
       ...
       *
       * When descriptors cover every top-level command root, 助手 can keep

...
const MANIFEST_INSTALL_TEMPLATE_ZHUSHOU_MANIFEST = {
...
function setupManifestInstallFixture(params: { manifestId: string; packageName?: string }) {
  ...
  fs.writeFileSync(
    ...
    JSON.stringify({
      ...MANIFEST_INSTALL_TEMPLATE_ZHUSHOU_MANIFEST,
...
beforeAll(async () => {
  ...
  fs.writeFileSync(
    ...
    JSON.stringify(MANIFEST_INSTALL_TEMPLATE_ZHUSHOU_MANIFEST),
    "utf-8",
  );
...
describe("installPluginFromArchive", () => {
  ...
  it("fails package installs when manifest traversal exceeds the directory cap", async () => {
    vi.stubEnv("ZHUSHOU_INSTALL_SCAN_MAX_DIRECTORIES", "4");
  ...
  it("fails package installs when manifest traversal exceeds the depth cap", async () => {
    vi.stubEnv("ZHUSHOU_INSTALL_SCAN_MAX_DEPTH", "2");
  ...
  it.each([
    {
      ...
      expectedMessageIncludes: ["requires 助手 >=2026.3.22, but this host is 2026.3.21"],
    },
    {

...
const suiteTempRootTracker = createSuiteTempRootTracker("assistant-plugin-install-npm-spec");
...
describe("installPluginFromNpmSpec", () => {
  it("uses --ignore-scripts for npm pack and cleans up temp dir", async () => {
      ...
      if (argv[0] === "npm" && argv[1] === "pack") {
        ...
        return {
          ...
          stdout: JSON.stringify([
            {
              id: "@assistant/voice-call@0.0.1",
              name: "@assistant/voice-call",
              version: "0.0.1",
              filename: packedName,
    ...
    const result = await installPluginFromNpmSpec({
      spec: "@assistant/voice-call@0.0.1",
      extensionsDir,
      logger: { info: () => {}, warn: () => {} },
    });
  ...
  it("handles prerelease npm specs correctly", async () => {
    const prereleaseMetadata = {
      id: "@assistant/voice-call@0.0.2-beta.1",
      name: "@assistant/voice-call",
...

    {
      ...
      if (!result.ok) {
        ...
        expect(result.error).toContain('"@assistant/voice-call@beta"');
      }
    }
...

    {
      ...
      fs.mkdirSync(extensionsDir, { recursive: true });
      const result = await installPluginFromNpmSpec({
        spec: "@assistant/voice-call@beta",
        extensionsDir,
        logger: { info: () => {}, warn: () => {} },
      });
      expect(result.ok).toBe(true);
      ...
      expect(result.npmResolution?.resolvedSpec).toBe("@assistant/voice-call@0.0.2-beta.1");
      expectSingleNpmPackIgnoreScriptsCall({
        calls: run.mock.calls as Array<[unknown, unknown]>,
        expectedSpec: "@assistant/voice-call@beta",
      });
      expect(packTmpDir).not.toBe("");
    }
  });

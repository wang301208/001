...
function validateClawHubPluginPackage(params: {
  ...
  if (
    ...
    return buildClawHubInstallFailure(
      `Plugin "${pkg.name}" requires plugin API ${compatibility.pluginApiRange}, but this 助手 runtime exposes ${runtimeVersion}.`,
  ...
  if (
    ...
    return buildClawHubInstallFailure(
      `Plugin "${pkg.name}" requires 助手 >=${compatibility.minGatewayVersion}, but this host is ${runtimeVersion}.`,

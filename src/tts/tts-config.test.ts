...
describe("shouldAttemptTtsPayload", () => {
  ...
  beforeEach(() => {
    originalPrefsPath = process.env.ZHUSHOU_TTS_PREFS;
    dir = mkdtempSync(path.join(tmpdir(), "zhushou-tts-config-"));
    prefsPath = path.join(dir, "tts.json");
    process.env.ZHUSHOU_TTS_PREFS = prefsPath;
  });

  afterEach(() => {
    if (originalPrefsPath === undefined) {
      delete process.env.ZHUSHOU_TTS_PREFS;
    } else {
      process.env.ZHUSHOU_TTS_PREFS = originalPrefsPath;
    }

...
function stubSlackPlugin(params: {
  ...
  inspectAccount?: (cfg: ZhushouConfig, accountId: string | null | undefined) => unknown;
  isConfigured?: (account: unknown, cfg: ZhushouConfig) => boolean;
}): ChannelPlugin {
  return {
    id: "slack",
    meta: {
    ...
    const cases = [
      {
        ...
        plugin: (sourceConfig: ZhushouConfig) =>
          stubSlackPlugin({
            inspectAccount: (cfg) => {

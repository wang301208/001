...
function stubSlackPlugin(params: {
  ...
  inspectAccount?: (cfg: AssistantConfig, accountId: string | null | undefined) => unknown;
  isConfigured?: (account: unknown, cfg: AssistantConfig) => boolean;
}): ChannelPlugin {
  return {
    id: "slack",
    meta: {
    ...
    const cases = [
      {
        ...
        plugin: (sourceConfig: AssistantConfig) =>
          stubSlackPlugin({
            inspectAccount: (cfg) => {

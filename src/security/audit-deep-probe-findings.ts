...
export function collectDeepProbeFindings(params: {
  ...
  if (params.deep?.gateway?.attempted && !params.deep.gateway.ok) {
    findings.push({
      ...
      remediation: `Run "${formatCliCommand("zhushou status --all")}" to debug connectivity/auth, then re-run "${formatCliCommand("zhushou security audit --deep")}".`,
    });
  }
  if (params.authWarning) {
    findings.push({
      ...
      remediation: `Set ZHUSHOU_GATEWAY_TOKEN/ZHUSHOU_GATEWAY_PASSWORD in this shell or resolve the external secret provider, then re-run "${formatCliCommand("zhushou security audit --deep")}".`,
    });
  }
  return findings;
}

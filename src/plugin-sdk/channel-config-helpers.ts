...
function formatPairingApproveHint(channelId: string): string {
  const listCmd = formatCliCommand(`assistant pairing list ${channelId}`);
  const approveCmd = formatCliCommand(`assistant pairing approve ${channelId} <code>`);
...
type ChannelConfigAccessorParams<Config extends AssistantConfig = AssistantConfig> = {
  cfg: Config;
  accountId?: string | null;
};
...
type MultiAccountChannelConfigAdapterParams<
  ...
  Config extends AssistantConfig = AssistantConfig,
> = {
  sectionKey: string;
...
export function adaptScopedAccountAccessor<Result, Config extends AssistantConfig = AssistantConfig>(
...
function setTopLevelChannelEnabledInConfigSection<Config extends AssistantConfig>(params: {
  cfg: Config;
  sectionKey: string;
  enabled: boolean;
}): Config {
...
function removeTopLevelChannelConfigSection<Config extends AssistantConfig>(params: {
  cfg: Config;
  sectionKey: string;
}): Config {
...
function clearTopLevelChannelConfigFields<Config extends AssistantConfig>(params: {
  cfg: Config;
  sectionKey: string;
  clearBaseFields: string[];
}): Config {

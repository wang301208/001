...
function formatPairingApproveHint(channelId: string): string {
  const listCmd = formatCliCommand(`zhushou pairing list ${channelId}`);
  const approveCmd = formatCliCommand(`zhushou pairing approve ${channelId} <code>`);
...
type ChannelConfigAccessorParams<Config extends ZhushouConfig = ZhushouConfig> = {
  cfg: Config;
  accountId?: string | null;
};
...
type MultiAccountChannelConfigAdapterParams<
  ...
  Config extends ZhushouConfig = ZhushouConfig,
> = {
  sectionKey: string;
...
export function adaptScopedAccountAccessor<Result, Config extends ZhushouConfig = ZhushouConfig>(
...
function setTopLevelChannelEnabledInConfigSection<Config extends ZhushouConfig>(params: {
  cfg: Config;
  sectionKey: string;
  enabled: boolean;
}): Config {
...
function removeTopLevelChannelConfigSection<Config extends ZhushouConfig>(params: {
  cfg: Config;
  sectionKey: string;
}): Config {
...
function clearTopLevelChannelConfigFields<Config extends ZhushouConfig>(params: {
  cfg: Config;
  sectionKey: string;
  clearBaseFields: string[];
}): Config {

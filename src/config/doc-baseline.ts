...
function logConfigDocBaselineDebug(message: string): void {
  if (process.env.ZHUSHOU_CONFIG_DOC_BASELINE_DEBUG === "1") {
    console.error(`[config-doc-baseline] ${message}`);
  }
}

function resolveRepoRoot(): string {
  const fromPackage = resolveZhushouPackageRootSync({
...
async function loadBundledConfigSchemaResponse(): Promise<ConfigSchemaResponse> {
  ...
  const env = {
    ...process.env,
    HOME: os.tmpdir(),
    ZHUSHOU_STATE_DIR: path.join(os.tmpdir(), "zhushou-config-doc-baseline-state"),
    ZHUSHOU_BUNDLED_PLUGINS_DIR: path.join(repoRoot, "extensions"),
  };

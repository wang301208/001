...
function logConfigDocBaselineDebug(message: string): void {
  if (process.env.ASSISTANT_CONFIG_DOC_BASELINE_DEBUG === "1") {
    console.error(`[config-doc-baseline] ${message}`);
  }
}

function resolveRepoRoot(): string {
  const fromPackage = resolveAssistantPackageRootSync({
...
async function loadBundledConfigSchemaResponse(): Promise<ConfigSchemaResponse> {
  ...
  const env = {
    ...process.env,
    HOME: os.tmpdir(),
    ASSISTANT_STATE_DIR: path.join(os.tmpdir(), "assistant-config-doc-baseline-state"),
    ASSISTANT_BUNDLED_PLUGINS_DIR: path.join(repoRoot, "extensions"),
  };

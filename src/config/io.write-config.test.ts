...
describe("config io write", () => {
  const suiteRootTracker = createSuiteTempRootTracker({ prefix: "assistant-config-io-" });
  const silentLogger = {
    warn: () => {},
    error: () => {},
  };
  ...
  it("keeps writes inside an ASSISTANT_STATE_DIR override even when the real home config exists", async () => {
    await withSuiteHome(async (home) => {
      const liveConfigPath = path.join(home, ".assistant", "assistant.json");
      ...
      expect(io.configPath).toBe(path.join(overrideDir, "assistant.json"));
      ...
      const overridePersisted = JSON.parse(
        await fs.readFile(path.join(overrideDir, "assistant.json"), "utf-8"),
      ) as {
      ...
      plugins: [
        {
          ...
          skills: [],
          hooks: [],
          rootDir: "/tmp/assistant-test-required-plugin",
          source: "/tmp/assistant-test-required-plugin/index.ts",
          manifestPath: "/tmp/assistant-test-required-plugin/assistant.plugin.json",
          configSchema: {
            type: "object",
            properties: {
              token: { type: "string" },

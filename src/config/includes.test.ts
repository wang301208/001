...
const ETC_ZHUSHOU_DIR = path.join(ROOT_DIR, "etc", "zhushou");
...
function etcZhushouPath(...parts: string[]) {
  return path.join(ETC_ZHUSHOU_DIR, ...parts);
}
...
describe("resolveConfigIncludes", () => {
  ...
  it("rejects absolute path outside config directory (CWE-22)", () => {
    const absolute = etcZhushouPath("agents.json");

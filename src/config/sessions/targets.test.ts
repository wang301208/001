...
function createCustomRootCfg(customRoot: string, defaultAgentId = "ops"): AssistantConfig {
  return {
    session: {
...
const discoveryResolvers = [
  {
    label: "async",
    resolve: async (cfg: AssistantConfig, env: NodeJS.ProcessEnv) =>
  ...
  it("resolves all configured agent stores", async () => {
    await withTempHome(async () => {
      const cfg: AssistantConfig = {
        session: {
          store: "~/.assistant/agents/{agentId}/sessions/sessions.json",
        },
        agents: {
...
describe("resolveAllAgentSessionStoreTargets", () => {
  ...
  it("respects the caller env when resolving configured and discovered store roots", async () => {
    await withTempHome(async (home) => {
      ...
      const env = {
        ...process.env,
        ASSISTANT_STATE_DIR: envStateDir,
      };
      const cfg: AssistantConfig = {};

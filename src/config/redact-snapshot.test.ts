...
import type { ConfigFileSnapshot } from "./types.zhushou.js";
...
describe("redactConfigSnapshot", () => {
  ...
  it("does not redact passwordFile path fields", () => {
    const snapshot = makeSnapshot({
      channels: {
        irc: {
          passwordFile: "/etc/zhushou/irc-password.txt",
          nickserv: {
            passwordFile: "/etc/zhushou/nickserv-password.txt",
    ...
    expect(irc.passwordFile).toBe("/etc/zhushou/irc-password.txt");
    expect(nickserv.passwordFile).toBe("/etc/zhushou/nickserv-password.txt");

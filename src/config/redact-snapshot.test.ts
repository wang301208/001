...
import type { ConfigFileSnapshot } from "./types.assistant.js";
...
describe("redactConfigSnapshot", () => {
  ...
  it("does not redact passwordFile path fields", () => {
    const snapshot = makeSnapshot({
      channels: {
        irc: {
          passwordFile: "/etc/assistant/irc-password.txt",
          nickserv: {
            passwordFile: "/etc/assistant/nickserv-password.txt",
    ...
    expect(irc.passwordFile).toBe("/etc/assistant/irc-password.txt");
    expect(nickserv.passwordFile).toBe("/etc/assistant/nickserv-password.txt");

    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["assistant nodes status", "List known nodes with live status."],
          ["assistant nodes pairing pending", "Show pending node pairing requests."],
          [
            'assistant nodes invoke --node <id> --command system.which --params \'{"name":"uname"}\'',
            "Invoke a node command directly.",
          ],
          ["assistant nodes camera snap --node <id>", "Capture a photo from a node camera."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/nodes", "docs.assistant.ai/cli/nodes")}\n`,
    );

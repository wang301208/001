    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["zhushou nodes status", "List known nodes with live status."],
          ["zhushou nodes pairing pending", "Show pending node pairing requests."],
          [
            'zhushou nodes invoke --node <id> --command system.which --params \'{"name":"uname"}\'',
            "Invoke a node command directly.",
          ],
          ["zhushou nodes camera snap --node <id>", "Capture a photo from a node camera."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/nodes", "docs.zhushou.ai/cli/nodes")}\n`,
    );

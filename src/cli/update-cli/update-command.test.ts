...
describe("resolveGatewayInstallEntrypointCandidates", () => {
  it("prefers index.js before legacy entry.js", () => {
    expect(resolveGatewayInstallEntrypointCandidates("/tmp/assistant-root")).toEqual([
      path.join("/tmp/assistant-root", "dist", "index.js"),
      path.join("/tmp/assistant-root", "dist", "index.mjs"),
      path.join("/tmp/assistant-root", "dist", "entry.js"),
      path.join("/tmp/assistant-root", "dist", "entry.mjs"),
    ]);
  });
});

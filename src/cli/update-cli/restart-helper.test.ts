...
describe("restart-helper", () => {
  ...
  describe("prepareRestartScript", () => {
    ...
    it("uses ZHUSHOU_SYSTEMD_UNIT override for systemd scripts", async () => {
      Object.defineProperty(process, "platform", { value: "linux" });
      const { scriptPath, content } = await prepareAndReadScript({
        ZHUSHOU_PROFILE: "default",
        ZHUSHOU_SYSTEMD_UNIT: "custom-gateway",
      });
    ...
    it("uses ZHUSHOU_LAUNCHD_LABEL override on macOS", async () => {
      Object.defineProperty(process, "platform", { value: "darwin" });
      process.getuid = () => 501;

      const { scriptPath, content } = await prepareAndReadScript({
        ZHUSHOU_PROFILE: "default",
        ZHUSHOU_LAUNCHD_LABEL: "com.custom.zhushou",
      });
    ...
    it("creates a schtasks restart script on Windows", async () => {
      ...
      expect(content).toContain('schtasks /End /TN "Zhushou Gateway"');
      expect(content).toContain('schtasks /Run /TN "Zhushou Gateway"');
    ...
    it("uses ZHUSHOU_WINDOWS_TASK_NAME override on Windows", async () => {
      Object.defineProperty(process, "platform", { value: "win32" });
      ...
      expect(content).toContain('schtasks /End /TN "Zhushou Gateway (custom)"');
      expect(content).toContain('schtasks /Run /TN "Zhushou Gateway (custom)"');
      expectWindowsRestartWaitOrdering(content);
      await cleanupScript(scriptPath);
    });
    ...
    it("uses custom profile in Windows task name", async () => {
      ...
      expect(content).toContain('schtasks /End /TN "Zhushou Gateway (production)"');
      expectWindowsRestartWaitOrdering(content);
      await cleanupScript(scriptPath);
    });
    ...
    it("shell-escapes the label in the plist path on macOS", async () => {
      ...
      const { scriptPath, content } = await prepareAndReadScript({
        HOME: "/Users/testuser",
        ZHUSHOU_LAUNCHD_LABEL: "ai.zhushou.it's-a-test",
      });

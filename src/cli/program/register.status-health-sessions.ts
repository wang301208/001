...
export function registerStatusHealthSessionsCommands(program: Command) {
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["assistant status", "Show channel health + session summary."],
          ["assistant status --all", "Full diagnosis (read-only)."],
          ["assistant status --json", "Machine-readable output."],
          ["assistant status --usage", "Show model provider usage/quota snapshots."],
          [
            "assistant status --deep",
            "Run channel probes (WA + Telegram + Discord + Slack + Signal).",
          ],
          ["assistant status --deep --timeout 5000", "Tighten probe timeout."],
        ])}`,
    )
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/status", "docs.assistant.ai/cli/status")}\n`,
    )
    .action(async (opts) => {
  ...
  program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/health", "docs.assistant.ai/cli/health")}\n`,
    )
    .action(async (opts) => {
      await runWithVerboseAndTimeout(opts, async ({ verbose, timeoutMs }) => {
        await healthCommand(
  ...
  const sessionsCmd = program
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["assistant sessions", "List all sessions."],
          ["assistant sessions --agent work", "List sessions for one agent."],
          ["assistant sessions --all-agents", "Aggregate sessions across agents."],
          ["assistant sessions --active 120", "Only last 2 hours."],
          ["assistant sessions --json", "Machine-readable output."],
          ["assistant sessions --store ./tmp/sessions.json", "Use a specific session store."],
        ])}\n\n${theme.muted(
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/sessions", "docs.assistant.ai/cli/sessions")}\n`,
    )
    .action(async (opts) => {
  ...
  sessionsCmd
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["assistant sessions cleanup --dry-run", "Preview stale/cap cleanup."],
          [
            "assistant sessions cleanup --dry-run --fix-missing",
            "Also preview pruning entries with missing transcript files.",
          ],
          ["assistant sessions cleanup --enforce", "Apply maintenance now."],
          ["assistant sessions cleanup --agent work --dry-run", "Preview one agent store."],
          ["assistant sessions cleanup --all-agents --dry-run", "Preview all agent stores."],
          [
            "assistant sessions cleanup --enforce --store ./tmp/sessions.json",

...
export function registerDnsCli(program: Command) {
  ...
  dns
    ...
    .action(async (opts) => {
      ...
      defaultRuntime.log(
        theme.heading(
          "Recommended config ($ZHUSHOU_CONFIG_PATH, default ~/.wang301208/zhushou.json):",
        ),
      );
      ...
      if (cfg.discovery?.wideArea?.enabled !== true) {
        ...
        defaultRuntime.log(
          theme.muted(
            "Note: enable discovery.wideArea.enabled in the active zhushou config ($ZHUSHOU_CONFIG_PATH, default ~/.wang301208/zhushou.json) on the gateway and restart the gateway so it writes the DNS-SD zone.",
          ),
        );
      }
    });

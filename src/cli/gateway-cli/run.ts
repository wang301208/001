...
async function runGatewayCommand(opts: GatewayRunOpts) {
  ...
  if (opts.cliBackendLogs || opts.claudeCliLogs) {
    ...
    process.env.ZHUSHOU_CLI_BACKEND_LOG_OUTPUT = "1";
  }
  ...
  if (opts.rawStream) {
    process.env.ZHUSHOU_RAW_STREAM = "1";
  }
  const rawStreamPath = toOptionString(opts.rawStreamPath);
  if (rawStreamPath) {
    process.env.ZHUSHOU_RAW_STREAM_PATH = rawStreamPath;
  }

  // This command is the network adapter around the same gateway method/runtime
  // surface used by the embedded stdio main gateway. It is intentionally
  // explicit so TUI/local startup has one primary gateway path.
  // The heaviest part of adapter startup is loading the server module tree
  ...
  if (process.env.ZHUSHOU_SERVICE_MARKER?.trim()) {
  ...
  gatewayLog.info("starting network adapter...");
  ...
  try {
    ...
    if (isGatewayLockError(err)) {
      ...
      defaultRuntime.error(
        `Gateway failed to start: ${errMessage}\nIf the gateway is supervised, stop it with: ${formatCliCommand("zhushou gateway stop")}`,
      );
      try {
...
export function addGatewayRunCommand(cmd: Command): Command {
  return cmd
    .option("--port <port>", "Port for the WebSocket network adapter")
    .option(
      "--bind <mode>",

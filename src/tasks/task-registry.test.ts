    ...
    await withTaskRegistryTempDir(async (root) => {
      ...
      expect(getTaskFlowById(flow.flowId)).toMatchObject({
        ...
        stateJson: {
          autoRetry: {
            attempts: 1,
            lastBlockedTaskId: child.taskId,
          },
        },
      });
      expect(findLatestTaskForFlowId(flow.flowId)).toMatchObject({
  ...
  it("does not auto-retry a blocked managed flow for a non-retryable blocker", async () => {
    await withTaskRegistryTempDir(async (root) => {
      ...
      const child = createTaskRecord({
        ...
        parentFlowId: flow.flowId,
        childSessionKey: "agent:main:child",
        runId: "run-registry-nonretryable",
        label: "Inspect PR 789",
        task: "Inspect PR 789",
        status: "running",
        startedAt: 100,
        lastEventAt: 100,
      ...
      markTaskTerminalById({
        ...
        terminalOutcome: "blocked",
        endedAt: 150,
        terminalSummary: "Waiting on child task output.",
      });
  ...
  it("does not auto-retry the same blocked managed child twice", async () => {
    await withTaskRegistryTempDir(async (root) => {
      ...
      const child = createTaskRecord({
        ...
        parentFlowId: flow.flowId,
        childSessionKey: "agent:main:child",
        runId: "run-registry-blocked-twice",
        label: "Inspect PR 456",
        task: "Inspect PR 456",
        status: "running",
        startedAt: 100,
        lastEventAt: 100,
      ...
      const afterFirstRetry = findLatestTaskForFlowId(flow.flowId);

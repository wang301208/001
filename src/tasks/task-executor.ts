...
export type RetryManagedBlockedTaskResult = RetryBlockedFlowResult;
...
export function resolveRetryableBlockedFlowTask(flowId: string): {
  flowFound: boolean;
  retryable: boolean;
...
function retryBlockedFlowTask(params: RetryBlockedFlowParams): RetryBlockedFlowResult {
  ...
  if (flow.syncMode === "managed") {
    const resumed = resumeFlow({
      ...
      status: params.status,
      currentStep: params.progressSummary ?? undefined,
      updatedAt: params.lastEventAt,
    });
    if (!resumed.applied) {
      return {
        found: true,
        retried: false,
        reason:
          resumed.reason === "revision_conflict"
            ? "Flow changed while retry was being created."
            : "Flow not found.",
        previousTask: resolved.latestTask,
      };
    }
...
export function retryManagedFlowFromBlockedTaskAsQueuedTaskRun(params: {
  flowId: string;
  sourceId?: string;
  requesterOrigin?: TaskDeliveryState["requesterOrigin"];
  childSessionKey?: string;
  ...
  const latestTask = findLatestTaskForFlowId(params.flowId);
  if (!latestTask) {
    return {
      found: true,
      retried: false,
  ...
  if (params.blockedTaskId && latestTask.taskId !== params.blockedTaskId) {
    return {
      found: true,
      retried: false,
      reason: "Blocked task is no longer the latest flow task.",
...
export async function cancelDetachedTaskRunById(params: { cfg: AssistantConfig; taskId: string }) {

...
const TASK_REGISTRY_DELIVERY_RUNTIME_OVERRIDE_KEY = Symbol.for(
  "assistant.taskRegistry.deliveryRuntimeOverride",
);
const TASK_REGISTRY_CONTROL_RUNTIME_OVERRIDE_KEY = Symbol.for(
  "assistant.taskRegistry.controlRuntimeOverride",
);
...
type ManagedFlowAutoRetryState = {
  autoRetry?: {
    attempts?: number;
...
function readManagedFlowAutoRetryState(flow: TaskFlowRecord): {
  attempts: number;
  lastBlockedTaskId?: string;
} {
  ...
  const autoRetry = (state as ManagedFlowAutoRetryState).autoRetry;
  return {
    attempts: typeof autoRetry?.attempts === "number" ? autoRetry.attempts : 0,
    lastBlockedTaskId:
      typeof autoRetry?.lastBlockedTaskId === "string" ? autoRetry.lastBlockedTaskId : undefined,
  };
}

function maybeAutoRetryManagedBlockedFlowFromTask(task: TaskRecord): void {
  const flowId = task.parentFlowId?.trim();
  if (!flowId) {
    return;
  }
  const flow = getTaskFlowById(flowId);
  if (!flow || flow.syncMode !== "managed" || flow.cancelRequestedAt != null) {
    return;
  }
  if (task.status !== "succeeded" || task.terminalOutcome !== "blocked") {
    return;
  }
  const autoRetry = readManagedFlowAutoRetryState(flow);
  if (autoRetry.lastBlockedTaskId === task.taskId) {
    return;
  }
  const retryReason = task.terminalSummary?.trim() || task.progressSummary?.trim() || "";
  if (!retryReason || !AUTO_RETRY_REASON_HINTS.some((pattern) => pattern.test(retryReason))) {
    log.debug("Skipped auto-retrying blocked managed flow: non-retryable blocker", {
      flowId,
      taskId: task.taskId,
      retryReason,
    });
  ...
  if (autoRetry.attempts >= MAX_MANAGED_FLOW_AUTO_RETRIES) {
    log.debug("Skipped auto-retrying blocked managed flow: retry budget exhausted", {
      flowId,
      taskId: task.taskId,
  ...
  const nextStateJson = {
    ...(flow.stateJson && typeof flow.stateJson === "object" && !Array.isArray(flow.stateJson)
      ...
      : {}),
    autoRetry: {
      attempts: autoRetry.attempts + 1,
      lastBlockedTaskId: task.taskId,
    },
  };
  const retried =
    flow.status === "blocked"
  ...
  if (!retried.retried) {
    log.debug("Skipped auto-retrying blocked managed flow", {
      flowId,
      taskId: task.taskId,
...
function updateTask(taskId: string, patch: Partial<TaskRecord>): TaskRecord | null {
  ...
  try {
    maybeAutoRetryManagedBlockedFlowFromTask(next);
  } catch (error) {
    log.warn("Failed to auto-retry blocked managed flow from task update", {
      taskId,
      flowId: next.parentFlowId,
      error,
    });

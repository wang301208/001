...
function resolveContinuationCurrentStep(params: {
  status: TaskFlowStatus;
  currentStep?: string | null;
  blockedSummary?: string | null;
  waitJson?: JsonValue | null;
}): string | null | undefined {
  const explicit = normalizeOptionalString(params.currentStep);
  if (explicit) {
    return explicit;
  }
  const blockedSummary = normalizeOptionalString(params.blockedSummary);
  if (params.status === "blocked") {
    return blockedSummary ?? "Blocked. Resolve the blocker, then resume the flow.";
  }
  if (params.status === "waiting") {
    if (params.waitJson != null) {
      return "Waiting for the required condition or external input, then resume the flow.";
    }
    return "Waiting to continue.";
  }
  if (params.status === "running") {
    return "Continue the active flow.";
  }
  if (params.status === "queued") {
    return "Start the next flow step.";
  }
  if (params.status === "succeeded") {
    return "Completed.";
  }
  if (params.status === "failed") {
    return blockedSummary ?? "Failed. Inspect the error, then decide whether to retry.";
  }
  if (params.status === "cancelled") {
    return "Cancelled.";
  }
  if (params.status === "lost") {
    return "Lost. Reconcile flow state before continuing.";
  }
  return undefined;
}
    ...
    patch: {
      status:
        ...
        normalizeOptionalString(params.blockedSummary)
          ? "blocked"
          : "waiting",
      currentStep: resolveContinuationCurrentStep({
        status:
          normalizeOptionalString(params.blockedTaskId) ||
          normalizeOptionalString(params.blockedSummary)
            ? "blocked"
...
export function syncFlowFromTask(
  ...
  return updateFlowRecordByIdUnchecked(flowId, {
    ...
    currentStep:
      terminalFlowStatus === "blocked"
        ? (resolveFlowBlockedSummary(task) ?? normalizeOptionalString(task.progressSummary) ?? null)
        : (normalizeOptionalString(task.progressSummary) ?? null),

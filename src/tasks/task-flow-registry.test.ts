...
async function withFlowRegistryTempDir<T>(run: (root: string) => Promise<T>): Promise<T> {
  return await withTempDir({ prefix: "assistant-task-flow-registry-" }, async (root) => {
...
describe("task-flow-registry", () => {
  ...
  it("mirrors one-task flow state from tasks and leaves managed flows alone", async () => {
    await withFlowRegistryTempDir(async (root) => {
      ...
      const running = syncFlowFromTask({
        taskId: "task-running",
        ...
        task: "Fix permissions",
        lastEventAt: 150,
        progressSummary: "Checking workspace permissions",
      });
      expect(running).toMatchObject({
        flowId: mirrored.flowId,
        syncMode: "task_mirrored",
        status: "running",
        currentStep: "Checking workspace permissions",
      });
      ...
      const blocked = syncFlowFromTask({
        ...
        lastEventAt: 200,
        endedAt: 200,
        progressSummary: "Awaiting writable session",
      ...
      expect(blocked).toMatchObject({
        ...
        syncMode: "task_mirrored",
        status: "blocked",
        currentStep: "Writable session required.",
    ...
    await withFlowRegistryTempDir(async (root) => {
      ...
      expect(resumed).toMatchObject({
        ...
        flow: expect.objectContaining({
          flowId: created.flowId,
          stateJson: null,
          currentStep: "Start the next flow step.",
        }),
      });
    });
  });
  ...
  it("fills default continuation-oriented currentStep text for waiting, success, and failure transitions", async () => {
    await withFlowRegistryTempDir(async (root) => {
      ...
      const created = createManagedTaskFlow({
        ownerKey: "agent:main:main",
        controllerId: "tests/continuation-defaults",
        goal: "Continue a blocked workflow",
      });

      const waiting = setFlowWaiting({
        flowId: created.flowId,
        expectedRevision: created.revision,
        waitJson: { kind: "external_input" },
      });
      expect(waiting).toMatchObject({
        applied: true,
        flow: expect.objectContaining({
          status: "waiting",
          currentStep: "Waiting for the required condition or external input, then resume the flow.",
        }),
      });
      ...
      expect(failed).toMatchObject({
        ...
        flow: expect.objectContaining({
          status: "failed",
          currentStep: "Failed. Inspect the error, then decide whether to retry.",
        }),
      });
    });
  });

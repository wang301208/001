...
beforeAll(() => {
  sharedFixtureRoot = fs.mkdtempSync(path.join(os.tmpdir(), "assistant-run-plan-fixtures-"));
...
function withFakeRuntimeBin<T>(params: { binName: string; run: () => T }): T {
  return withFakeRuntimeBins({
    ...
    tmpPrefix: `assistant-${params.binName}-bin-`,
    run: params.run,
  });
}
...
const unsafeRuntimeInvocationCases: UnsafeRuntimeInvocationCase[] = [
  ...
  },
  {
    ...
    binName: "node",
    tmpPrefix: "assistant-node-import-inline-",
  ...
  },
  {
    ...
    binName: "ruby",
    tmpPrefix: "assistant-ruby-require-",
  ...
  },
  {
    ...
    binName: "ruby",
    tmpPrefix: "assistant-ruby-load-path-",
  ...
  },
  {
    ...
    binName: "perl",
    tmpPrefix: "assistant-perl-module-preload-",
  ...
  },
  {
    ...
    binName: "perl",
    tmpPrefix: "assistant-perl-load-path-",
  ...
  },
  {
    ...
    binName: "perl",
    tmpPrefix: "assistant-perl-preload-load-path-",
  ...
  },
  {
    ...
    binName: "node",
    tmpPrefix: "assistant-inline-shell-node-",
  ...
  },
  {
    ...
    binName: "pnpm",
    tmpPrefix: "assistant-pnpm-dlx-unknown-flag-",
  ...
  },
  {
    ...
    binName: "pnpm",
    tmpPrefix: "assistant-pnpm-dlx-unknown-prefix-",
  ...
  },
  {
    ...
    binName: "pnpm",
    tmpPrefix: "assistant-pnpm-dlx-unknown-prefix-value-",
  ...
  },
  {
    ...
    binName: "pnpm",
    tmpPrefix: "assistant-pnpm-dlx-global-double-dash-",
    command: ["pnpm", "--", "dlx", "--future-flag", "tsx", "./run.ts"],
    setup: (tmp) => {
      fs.writeFileSync(path.join(tmp, "run.ts"), 'console.log("SAFE")\n');
    },
...
describe("hardenApprovedExecutionPaths", () => {
  ...
  it.runIf(process.platform !== "win32").each(cases)("$name", (testCase) => {
    const tmp = fs.mkdtempSync(path.join(os.tmpdir(), "assistant-approval-hardening-"));
    const oldPath = process.env.PATH;
      ...
      withFakeRuntimeBins({
        ...
        run: () => {
          withScriptOperandPlanFixture(
            {
              tmpPrefix: "assistant-approval-script-plan-",
              fixture: runtimeCase,
  ...
  it("keeps fail-closed behavior for relative native-binary shell payloads", () => {
    ...
    const tmp = fs.mkdtempSync(path.join(os.tmpdir(), "assistant-shell-relative-binary-binding-"));
    try {
      const binaryPath = resolveNativeBinaryFixturePath();
  ...
  it("keeps fail-closed behavior for writable absolute native-binary shell payloads", () => {
    ...
    const tmp = fs.mkdtempSync(path.join(os.tmpdir(), "assistant-shell-absolute-binary-binding-"));
    try {
      const binaryPath = resolveNativeBinaryFixturePath();
  ...
  it("keeps fail-closed behavior for owner-controlled read-only absolute binaries", () => {
    ...
    const tmp = fs.mkdtempSync(path.join(os.tmpdir(), "assistant-shell-owned-readonly-binding-"));
  ...
  it("keeps fail-closed behavior for symlinked binaries with writable targets", () => {
    ...
    const tmp = fs.mkdtempSync(path.join(os.tmpdir(), "assistant-shell-symlink-binary-binding-"));
  ...
  it("keeps fail-closed behavior for shell payloads that invoke mutable script files", () => {
    expectShellPayloadApprovalDenied({
      tmpPrefix: "assistant-shell-script-binding-",
      fileName: "run.sh",
      body: "#!/bin/sh\necho SAFE\n",
    });
  });
  ...
  it("keeps fail-closed behavior for empty shell payload files", () => {
    expectShellPayloadApprovalDenied({
      tmpPrefix: "assistant-shell-empty-binding-",
      fileName: "empty",
      body: "",
    });
  });
  ...
  it("does not treat weak MZ text headers as native binaries", () => {
    expectShellPayloadApprovalDenied({
      tmpPrefix: "assistant-shell-mz-text-binding-",
      fileName: "mz-script",
  ...
  it("keeps fail-closed behavior for unknown NUL-bearing headers", () => {
    expectShellPayloadApprovalDenied({
      tmpPrefix: "assistant-shell-nul-header-binding-",
      fileName: "nul-script",
  ...
  it("keeps fail-closed behavior when the shell payload probe stops seeing a file", () => {
    ...
    const tmp = fs.mkdtempSync(path.join(os.tmpdir(), "assistant-shell-race-binding-"));
    try {
  ...
  it("detects rewritten script operands for pnpm dlx approval plans", () => {
    withFakeRuntimeBins({
      ...
      run: () => {
        withScriptOperandPlanFixture(
          {
            tmpPrefix: "assistant-pnpm-dlx-approval-",
            fixture: {
  ...
  it("does not bind pnpm dlx shell-mode commands to a mutable file operand", () => {
    withFakeRuntimeBins({
      binNames: ["pnpm", "tsx"],
      run: () => {
        const tmp = fs.mkdtempSync(path.join(os.tmpdir(), "assistant-pnpm-dlx-shell-mode-"));
        try {
          fs.writeFileSync(path.join(tmp, "run.ts"), 'console.log("SAFE");\n');
          expect(
            resolveMutableFileOperandSnapshotSync({
  ...
  it("allows pnpm dlx package binaries that do not bind a mutable local file", () => {
    withFakeRuntimeBin({
      binName: "pnpm",
      run: () => {
        const tmp = fs.mkdtempSync(path.join(os.tmpdir(), "assistant-pnpm-dlx-package-bin-"));
        try {
    ...
    withFakeRuntimeBin({
      ...
      run: () => {
        const tmp = fs.mkdtempSync(
          path.join(os.tmpdir(), "assistant-pnpm-dlx-package-runtime-token-"),
        );
        try {
  ...
  it("allows pnpm dlx package binaries with multi-token data-like runtime names", () => {
    withFakeRuntimeBin({
      ...
      run: () => {
        const tmp = fs.mkdtempSync(
          path.join(os.tmpdir(), "assistant-pnpm-dlx-package-runtime-token-multi-"),
        );
        try {
  ...
  it("allows pnpm dlx package binaries with local file arguments", () => {
    withFakeRuntimeBins({
      ...
      run: () => {
        const tmp = fs.mkdtempSync(path.join(os.tmpdir(), "assistant-pnpm-dlx-package-file-"));
        try {
  ...
  it("allows pnpm dlx package binaries with interpreter-like data tails", () => {
    withFakeRuntimeBin({
      binName: "pnpm",
      run: () => {
        const tmp = fs.mkdtempSync(path.join(os.tmpdir(), "assistant-pnpm-dlx-package-data-tail-"));
        try {
          fs.writeFileSync(path.join(tmp, "run.ts"), 'console.log("SAFE");\n');
  ...
  it("treats -- as the end of pnpm dlx option parsing", () => {
    withFakeRuntimeBins({
      ...
      run: () => {
        withScriptOperandPlanFixture(
          {
            tmpPrefix: "assistant-pnpm-dlx-double-dash-",
            fixture: {
  ...
  it("captures the real shell script operand after value-taking shell flags", () => {
    const tmp = fs.mkdtempSync(path.join(os.tmpdir(), "assistant-shell-option-value-"));
    try {
      const scriptPath = path.join(tmp, "run.sh");
      fs.writeFileSync(scriptPath, "#!/bin/sh\necho SAFE\n");

...
import {
  sanitizeZhushouVisibleText,
  sanitizeZhushouVisibleTextWithProfile,
  stripZhushouInternalScaffolding,
} from "./zhushou-visible-text.js";
import { stripModelSpecialTokens } from "./model-special-tokens.js";

describe("stripZhushouInternalScaffolding", () => {
  function expectVisibleText(input: string, expected: string) {
    expect(stripZhushouInternalScaffolding(input)).toBe(expected);
  }
  ...
  describe("model special token stripping", () => {
    it("strips Kimi/GLM special tokens in isolation", () => {
      expectVisibleText("<|zhushou|>Here is the answer<|end|>", "Here is the answer");
    });
    ...
    it("preserves special tokens inside inline code spans", () => {
      expectVisibleText(
        "The token `<|zhushou|>` marks the start.",
        "The token `<|zhushou|>` marks the start.",
      );
    });
    ...
    it("resets special-token regex state between calls", () => {
      expect(stripModelSpecialTokens("prefix <|zhushou|>")).toBe("prefix ");
      expect(stripModelSpecialTokens("<|zhushou|>short")).toBe("short");
    });
  });
});

describe("sanitizeZhushouVisibleText", () => {
  it("strips minimax, tool XML, downgraded tool markers, and think tags in one pass", () => {
    ...
    expect(sanitizeZhushouVisibleText(input)).toBe("Visible answer");
  });
...
describe("sanitizeZhushouVisibleTextWithProfile", () => {
  it("uses the history profile to preserve block-boundary whitespace", () => {
    ...
    expect(sanitizeZhushouVisibleTextWithProfile(input, "history")).toBe("Hi there");
  });
  ...
  it("uses the internal-scaffolding profile to preserve downgraded tool text behavior", () => {
    ...
    expect(sanitizeZhushouVisibleTextWithProfile(input, "internal-scaffolding")).toContain(
      "[Tool Call: read (ID: toolu_1)]",
    );
  });
});

...
export type ZhushouVisibleTextSanitizerProfile = "delivery" | "history" | "internal-scaffolding";

type ZhushouVisibleTextPipelineOptions = {
...
const ZHUSHOU_VISIBLE_TEXT_PIPELINE_OPTIONS: Record<
  ZhushouVisibleTextSanitizerProfile,
  ZhushouVisibleTextPipelineOptions
> = {
  delivery: {
    finalTrim: "both",
...
function applyZhushouVisibleTextStagePipeline(
  text: string,
  options: ZhushouVisibleTextPipelineOptions,
): string {
  if (!text) {
    return text;
  }
...
export function sanitizeZhushouVisibleTextWithProfile(
  text: string,
  profile: ZhushouVisibleTextSanitizerProfile = "delivery",
): string {
  return applyZhushouVisibleTextStagePipeline(
    text,
    ZHUSHOU_VISIBLE_TEXT_PIPELINE_OPTIONS[profile],
  );
}

export function stripZhushouInternalScaffolding(text: string): string {
  return sanitizeZhushouVisibleTextWithProfile(text, "internal-scaffolding");
}

/**
 * Canonical user-visible zhushou text sanitizer for delivery and history
 * extraction paths. Keeps prose, removes internal scaffolding.
 */
export function sanitizeZhushouVisibleText(text: string): string {
  return sanitizeZhushouVisibleTextWithProfile(text, "delivery");
}

/**
 * Backwards-compatible trim wrapper.
 * Prefer sanitizeZhushouVisibleTextWithProfile for new call sites.
 */
export function sanitizeZhushouVisibleTextWithOptions(
  text: string,
  options?: { trim?: "none" | "both" },
): string {
  const profile = options?.trim === "none" ? "history" : "delivery";
  return sanitizeZhushouVisibleTextWithProfile(text, profile);
}

...
const GATEWAY_TEST_ENV_KEYS = [
  ...
  "ASSISTANT_TEST_MINIMAL_GATEWAY_ALLOW_RELOAD",
  "ASSISTANT_TEST_MINIMAL_GATEWAY_ALLOW_AUTO_ENABLE",
  "ASSISTANT_TEST_MINIMAL_GATEWAY_ALLOW_SCHEDULED_SERVICES",
] as const;
...
async function persistTestSessionConfig(): Promise<void> {
  ...
  if (process.env.ASSISTANT_CONFIG_PATH) {
    configPaths.add(process.env.ASSISTANT_CONFIG_PATH);
  }
  if (process.env.ASSISTANT_STATE_DIR) {
    configPaths.add(path.join(process.env.ASSISTANT_STATE_DIR, "assistant.json"));
  }
...
async function setupGatewayTestHome() {
  ...
  tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "assistant-gateway-home-"));
...
function applyGatewaySkipEnv() {
  ...
  delete process.env.ASSISTANT_TEST_MINIMAL_GATEWAY_ALLOW_RELOAD;
  delete process.env.ASSISTANT_TEST_MINIMAL_GATEWAY_ALLOW_AUTO_ENABLE;
  delete process.env.ASSISTANT_TEST_MINIMAL_GATEWAY_ALLOW_SCHEDULED_SERVICES;
  process.env.ASSISTANT_BUNDLED_PLUGINS_DIR = tempHome
    ? path.join(tempHome, "assistant-test-no-bundled-extensions")
    : "assistant-test-no-bundled-extensions";
}
...
async function resetGatewayTestState(options: { uniqueConfigRoot: boolean }) {
  ...
  if (options.uniqueConfigRoot) {
    const suiteRoot = path.join(tempHome, ".assistant-test-suite");
    ...
    tempConfigRoot = path.join(tempHome, ".assistant-test");
    await fs.rm(tempConfigRoot, {
      recursive: true,
      force: true,
      maxRetries: 20,
  ...
  tempControlUiRoot = path.join(tempHome, ".assistant-test-control-ui");
  ...
  await fs.writeFile(
    ...
    "<!doctype html><title>assistant-test-control-ui</title>\n",
    "utf-8",
  );
  setTestConfigRoot(tempConfigRoot);
  resetConfigRuntimeState();
...
const CONNECT_CHALLENGE_NONCE_KEY = "__assistantTestConnectChallengeNonce";
const CONNECT_CHALLENGE_TRACKED_KEY = "__assistantTestConnectChallengeTracked";
...
export async function startServerWithClient(
  ...
  const envSnapshot = captureEnv(["ASSISTANT_GATEWAY_TOKEN"]);
  const prev = process.env.ASSISTANT_GATEWAY_TOKEN;
  ...
  if (fallbackToken === undefined) {
    ...
    process.env.ASSISTANT_GATEWAY_TOKEN = fallbackToken;
  }
...
function resolveDefaultTestDeviceIdentityPath(params: {
  ...
  const suiteRoot = process.env.ASSISTANT_STATE_DIR ?? process.env.HOME ?? os.tmpdir();

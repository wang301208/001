...
const GATEWAY_TEST_ENV_KEYS = [
  ...
  "ZHUSHOU_TEST_MINIMAL_GATEWAY_ALLOW_RELOAD",
  "ZHUSHOU_TEST_MINIMAL_GATEWAY_ALLOW_AUTO_ENABLE",
  "ZHUSHOU_TEST_MINIMAL_GATEWAY_ALLOW_SCHEDULED_SERVICES",
] as const;
...
async function persistTestSessionConfig(): Promise<void> {
  ...
  if (process.env.ZHUSHOU_CONFIG_PATH) {
    configPaths.add(process.env.ZHUSHOU_CONFIG_PATH);
  }
  if (process.env.ZHUSHOU_STATE_DIR) {
    configPaths.add(path.join(process.env.ZHUSHOU_STATE_DIR, "zhushou.json"));
  }
...
async function setupGatewayTestHome() {
  ...
  tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "zhushou-gateway-home-"));
...
function applyGatewaySkipEnv() {
  ...
  delete process.env.ZHUSHOU_TEST_MINIMAL_GATEWAY_ALLOW_RELOAD;
  delete process.env.ZHUSHOU_TEST_MINIMAL_GATEWAY_ALLOW_AUTO_ENABLE;
  delete process.env.ZHUSHOU_TEST_MINIMAL_GATEWAY_ALLOW_SCHEDULED_SERVICES;
  process.env.ZHUSHOU_BUNDLED_PLUGINS_DIR = tempHome
    ? path.join(tempHome, "zhushou-test-no-bundled-extensions")
    : "zhushou-test-no-bundled-extensions";
}
...
async function resetGatewayTestState(options: { uniqueConfigRoot: boolean }) {
  ...
  if (options.uniqueConfigRoot) {
    const suiteRoot = path.join(tempHome, ".zhushou-test-suite");
    ...
    tempConfigRoot = path.join(tempHome, ".zhushou-test");
    await fs.rm(tempConfigRoot, {
      recursive: true,
      force: true,
      maxRetries: 20,
  ...
  tempControlUiRoot = path.join(tempHome, ".zhushou-test-control-ui");
  ...
  await fs.writeFile(
    ...
    "<!doctype html><title>zhushou-test-control-ui</title>\n",
    "utf-8",
  );
  setTestConfigRoot(tempConfigRoot);
  resetConfigRuntimeState();
...
const CONNECT_CHALLENGE_NONCE_KEY = "__zhushouTestConnectChallengeNonce";
const CONNECT_CHALLENGE_TRACKED_KEY = "__zhushouTestConnectChallengeTracked";
...
export async function startServerWithClient(
  ...
  const envSnapshot = captureEnv(["ZHUSHOU_GATEWAY_TOKEN"]);
  const prev = process.env.ZHUSHOU_GATEWAY_TOKEN;
  ...
  if (fallbackToken === undefined) {
    ...
    process.env.ZHUSHOU_GATEWAY_TOKEN = fallbackToken;
  }
...
function resolveDefaultTestDeviceIdentityPath(params: {
  ...
  const suiteRoot = process.env.ZHUSHOU_STATE_DIR ?? process.env.HOME ?? os.tmpdir();

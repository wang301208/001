...
export function resolveGatewayReloadSettings(cfg: ZhushouConfig): GatewayReloadSettings {
...
export function startGatewayConfigReloader(opts: {
  ...
  onHotReload: (plan: GatewayReloadPlan, nextConfig: ZhushouConfig) => Promise<void>;
  onRestart: (plan: GatewayReloadPlan, nextConfig: ZhushouConfig) => void | Promise<void>;
  ...
  let pendingInProcessConfig: ZhushouConfig | null = null;
  ...
  const queueRestart = (plan: GatewayReloadPlan, nextConfig: ZhushouConfig) => {
    if (restartQueued) {
      return;
    }
    restartQueued = true;
  ...
  const applySnapshot = async (nextConfig: ZhushouConfig) => {

...
function expectSecurityConnectError(
  ...
  expect(error.message).toContain("zhushou doctor --fix");
...
describe("GatewayClient security checks", () => {
  ...
  it("allows ws:// to private addresses only with ZHUSHOU_ALLOW_INSECURE_PRIVATE_WS=1", () => {
  ...
  it("allows ws:// hostnames with ZHUSHOU_ALLOW_INSECURE_PRIVATE_WS=1", () => {
    process.env.ZHUSHOU_ALLOW_INSECURE_PRIVATE_WS = "1";
    const onConnectError = vi.fn();
    const client = new GatewayClient({
      url: "ws://zhushou-gateway.ai:18789",
      onConnectError,
    });

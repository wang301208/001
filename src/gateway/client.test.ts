...
function expectSecurityConnectError(
  ...
  expect(error.message).toContain("assistant doctor --fix");
...
describe("GatewayClient security checks", () => {
  ...
  it("allows ws:// to private addresses only with ASSISTANT_ALLOW_INSECURE_PRIVATE_WS=1", () => {
  ...
  it("allows ws:// hostnames with ASSISTANT_ALLOW_INSECURE_PRIVATE_WS=1", () => {
    process.env.ASSISTANT_ALLOW_INSECURE_PRIVATE_WS = "1";
    const onConnectError = vi.fn();
    const client = new GatewayClient({
      url: "ws://assistant-gateway.ai:18789",
      onConnectError,
    });

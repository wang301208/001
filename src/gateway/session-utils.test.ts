...
function createSingleAgentAvatarConfig(workspace: string): AssistantConfig {
  return {
...
describe("gateway session utils", () => {
  ...
  test("listAgentsForGateway keeps explicit agents.list scope over disk-only agents while allowing charter agents", async () => {
    await withStateDirEnv("assistant-agent-list-scope-", async ({ stateDir }) => {
...
describe("resolveSessionModelIdentityRef", () => {
  ...
  test("infers provider from configured provider catalogs when allowlist is absent", () => {
    ...
    } as unknown as AssistantConfig["models"];

...
describe("OpenResponses HTTP API (e2e)", () => {
  it("handles OpenResponses request parsing and validation", async () => {
    ...
    try {
      ...
      const resMissingAuth = await fetch(`http://127.0.0.1:${port}/v1/responses`, {
        ...
        body: JSON.stringify({ model: "zhushou", input: "hi" }),
      });
      ...
      mockAgentOnce([{ text: "hello" }]);
      const resModel = await postResponses(port, { model: "zhushou/beta", input: "hi" });
      ...
      mockAgentOnce([{ text: "hello" }]);
      const resDefaultAlias = await postResponses(port, { model: "zhushou/default", input: "hi" });
      ...
      expect(invalidOverrideJson.error?.message).toBe("Invalid `x-zhushou-model`.");
      expect(agentCommand).toHaveBeenCalledTimes(0);
      ...
      const resSystemDeveloper = await postResponses(port, {
        model: "zhushou",
        input: [
          { type: "message", role: "system", content: "You are a helpful zhushou." },
      ...
      const extraSystemPrompt =
        ...
        "";
      expect(extraSystemPrompt).toContain("You are a helpful zhushou.");
      ...
      expect(historyMessage).toContain("Zhushou: I am Claude.");
  ...
  it("treats write-scoped HTTP callers as non-owner and admin-scoped callers as owner", async () => {
    ...
    const streamingResponse = await postResponses(
      port,
      { stream: true, model: "zhushou", input: "hi" },
      { "x-zhushou-scopes": "operator.admin, operator.write" },
    );
  ...
  it("preserves zhushou text alongside non-stream function_call output", async () => {
    const port = enabledPort;
    agentCommand.mockClear();
    agentCommand.mockResolvedValueOnce({

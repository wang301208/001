...
export type OpenResponsesHandlerDeps = ModelsHandlerDeps & {
  openResponsesConfig?: Record<string, unknown>;
};

export function registerOpenResponsesRoutes(app: Hono<GatewayHonoEnv>, deps?: OpenResponsesHandlerDeps): void {
  app.post("/v1/responses", async (c) => {
    if (!deps) {
      return c.json({ error: { message: "OpenResponses路由未配置handler依赖", type: "config_error" } }, 503);
    }
    return delegateToHandler(c, "openresponses-http", "handleOpenResponsesHttpRequest", {

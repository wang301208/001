...
export function registerSessionRoutes(app: Hono<GatewayHonoEnv>, deps?: ModelsHandlerDeps): void {
  app.post("/sessions/:sessionId/kill", async (c) => {
    if (!deps) {
      return c.json({ ok: false, error: "Session路由未配置handler依赖" }, 503);
    }
    return delegateToHandler(c, "session-kill-http", "handleSessionKillHttpRequest", {
  ...
  app.get("/sessions/:sessionId/history", async (c) => {
    ...
    return delegateToHandler(c, "sessions-history-http", "handleSessionHistoryHttpRequest", {
      auth: deps.getResolvedAuth(),
      trustedProxies: deps.trustedProxies,
      allowRealIpFallback: deps.allowRealIpFallback,
      rateLimiter: deps.rateLimiter,

...
let lastCreateZhushouToolsContext: Record<string, unknown> | undefined;
...
vi.mock("../agents/zhushou-tools.js", () => {
  ...
  const tools = [
    ...
    },
    {
      ...
      execute: async () => ({
        ok: true,
        route: {
          agentTo: lastCreateZhushouToolsContext?.agentTo,
          agentThreadId: lastCreateZhushouToolsContext?.agentThreadId,
        },
      }),
    },
    {
  ...
  return {
    createZhushouTools: (ctx: Record<string, unknown>) => {
      lastCreateZhushouToolsContext = ctx;
      return tools;
    },
  };
});
...
beforeEach(() => {
  ...
  pluginHttpHandlers = [];
  cfg = {};
  lastCreateZhushouToolsContext = undefined;
...
const gatewayAuthHeaders = () => ({ "x-zhushou-scopes": "operator.write" });
const gatewayAdminHeaders = () => ({ "x-zhushou-scopes": "operator.admin" });
...
describe("POST /tools/invoke", () => {
  it("invokes a tool and returns {ok:true,result}", async () => {
    ...
    expect(lastCreateZhushouToolsContext?.allowMediaInvokeCommands).toBe(true);
    expect(lastCreateZhushouToolsContext?.disablePluginTools).toBe(true);
  ...
  it("opts direct gateway tool invocation into gateway subagent binding", async () => {
    ...
    expect(res.status).toBe(200);
    expect(lastCreateZhushouToolsContext?.allowGatewaySubagentBinding).toBe(true);
  });
  ...
  it("keeps plugin tools enabled for non-core tool invokes", async () => {
    ...
    expect(res.status).toBe(200);
    expect(lastCreateZhushouToolsContext?.disablePluginTools).toBe(false);
  });
  ...
  it("threads senderIsOwner into tool creation before owner-only filtering", async () => {
    ...
    expect(lastCreateZhushouToolsContext?.senderIsOwner).toBe(false);
    ...
    expect(lastCreateZhushouToolsContext?.senderIsOwner).toBe(true);
  });
  ...
  it("propagates message target/thread headers into tools context for sessions_spawn", async () => {
    ...
    const res = await invokeTool({
      ...
      headers: {
        ...gatewayAuthHeaders(),
        "x-zhushou-message-to": "channel:24514",
        "x-zhushou-thread-id": "thread-24514",
      },
      tool: "sessions_spawn",
      sessionKey: "main",
    });

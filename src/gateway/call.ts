...
function loadGatewayConfig(): AssistantConfig {
  const loadConfigFn =
...
function resolveGatewayPortValue(config?: AssistantConfig, env?: NodeJS.ProcessEnv): number {
...
function resolveGatewayCallContext(opts: CallGatewayBaseOptions): ResolvedGatewayCallContext {
  ...
  const envUrlOverride = cliUrlOverride
    ? undefined
    : trimToUndefined(process.env.ASSISTANT_GATEWAY_URL);
  ...
  const config = opts.config ?? (canSkipConfigLoad ? ({} as AssistantConfig) : loadGatewayConfig());

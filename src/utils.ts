...
export function resolveConfigDir(
  ...
  const configPath = env.ZHUSHOU_CONFIG_PATH?.trim();
  if (configPath) {
    return path.dirname(resolveUserPath(configPath, env, homedir));
  }
  const newDir = path.join(resolveRequiredHomeDir(env, homedir), ".zhushou");
  try {
...
function resolveHomeDisplayPrefix(): { home: string; prefix: string } | undefined {
  ...
  const explicitHome = process.env.ZHUSHOU_HOME?.trim();
  if (explicitHome) {
    return { home, prefix: "$ZHUSHOU_HOME" };
  }
  return { home, prefix: "~" };
}
...
// Configuration root; can be overridden via ZHUSHOU_STATE_DIR.

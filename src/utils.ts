...
export function resolveConfigDir(
  ...
  const configPath = env.ASSISTANT_CONFIG_PATH?.trim();
  if (configPath) {
    return path.dirname(resolveUserPath(configPath, env, homedir));
  }
  const newDir = path.join(resolveRequiredHomeDir(env, homedir), ".assistant");
  try {
...
function resolveHomeDisplayPrefix(): { home: string; prefix: string } | undefined {
  ...
  const explicitHome = process.env.ASSISTANT_HOME?.trim();
  if (explicitHome) {
    return { home, prefix: "$ASSISTANT_HOME" };
  }
  return { home, prefix: "~" };
}
...
// Configuration root; can be overridden via ASSISTANT_STATE_DIR.

...
const ensureSupportedNodeVersion = () => {
  ...
  process.stderr.write(
    `assistant: Node.js v${MIN_NODE_VERSION}+ is required (current: v${process.versions.node}).\n` +
      "If you use nvm, run:\n" +
...
const buildMissingEntryErrorMessage = async () => {
  const lines = ["assistant: missing dist/entry.(m)js (build output)."];
  ...
  lines.push(
    "For pinned GitHub installs, use `npm install -g github:assistant/assistant#<ref>` instead of a raw `/archive/<ref>.tar.gz` URL.",
  );
  lines.push("For releases, use `npm install -g assistant@latest`.");
  return lines.join("\n");
};
...
const SOURCE_TREE_ROOT_HELP_TEXT = `Usage: assistant [options] [command]
...
Quickstart:
  ...
  pnpm build
    Build dist/ before using the packaged assistant.mjs launcher.
  pnpm start -- setup --wizard
    Initialize local config and the agent workspace.
  pnpm start -- tui
    Open the terminal UI for the first conversation.
  pnpm start -- tui --message "你好"
    Send the first message directly from the terminal.

Common commands:
  setup        Initialize local config and agent workspace.
  onboard      Run guided onboarding for gateway, workspace, and skills.
  configure    Edit credentials, channels, gateway, and agent defaults.
...
Configuration:
  Default state directory: ~/.assistant
  Default config file: ~/.assistant/assistant.json
  Overrides: ASSISTANT_STATE_DIR, ASSISTANT_CONFIG_PATH
...
const tryOutputBareRootHelp = async () => {
  ...
  if (await exists("./src/entry.ts")) {
    process.stdout.write(SOURCE_TREE_ROOT_HELP_TEXT);
    return true;
  }
  return false;
};
